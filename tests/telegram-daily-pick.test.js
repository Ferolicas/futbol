...
test('Telegram publica una sola opción válida cuando ya alcanza cuota 1.50–2.00', () => {
  const result = dailyPickModule.selectTelegramDailyPick([
    { fixtureId: 1, id: 'total_goals_over1_5', name: 'Más de 1.5 goles', probability: 90, odd: 1.72, dailyValidated: true },
...
test('Telegram combina el mínimo de partidos distintos y nunca usa hándicap ni menos de 90%', () => {
  const result = dailyPickModule.selectTelegramDailyPick([
    ...
    { fixtureId: 3, id: 'total_cards_over2_5', name: 'Más de 2.5 tarjetas', probability: 89.9, odd: 1.8, dailyValidated: true },
...
test('Telegram rechaza un 90% cuya familia no sostuvo el umbral fuera de muestra', () => {
  const result = dailyPickModule.selectTelegramDailyPick([
