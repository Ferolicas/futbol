...
import { fetchMultisportBet365Event } from './odds-api.js';
    ...
    const boxTeam = (side) => {
      ...
      return {
        statistics: {
          ...
          hitsByInning: (live?.innings || []).map((inning) => (
            side === 'home' ? inning.homeHits : inning.awayHits
          )),
        },
...
function providerDateForGame(sport, value) {
  const date = new Date(value);
  if (!Number.isFinite(date.getTime())) return null;
  if (sport !== 'baseball') return date.toISOString().slice(0, 10);
  // API-Baseball permite timezone en /games. Sin él, los juegos nocturnos de
  // Colombia caen en el día UTC siguiente y el plan free los rechaza como
  // fecha futura, aunque para el usuario pertenezcan a la jornada de hoy.
  return new Intl.DateTimeFormat('en-CA', {
    timeZone: 'America/Bogota', year: 'numeric', month: '2-digit', day: '2-digit',
  }).format(date);
}

function theOddsApiSportsPayload(event) {
    ...
    for (const market of bookmaker.markets || []) {
      if (market.key === 'h2h') {
        bets.push({
          id: 1,
          name: 'Home/Away',
          values: (market.outcomes || []).map((outcome) => ({
            value: outcome.name === event.home_team ? 'Home'
              : (outcome.name === event.away_team ? 'Away' : 'Draw'),
            odd: String(outcome.price),
          })),
        });
        ...
        bets.push({
          ...
          name: 'Over/Under',
          values: (market.outcomes || []).map((outcome) => ({
            value: `${outcome.name} ${outcome.point}`,
            odd: String(outcome.price),
          })),
        });
        ...
        bets.push({
          ...
          name: 'Asian Handicap',
          values: (market.outcomes || []).map((outcome) => ({
            value: `${outcome.name === event.home_team ? 'Home' : 'Away'} ${Number(outcome.point) >= 0 ? '+' : ''}${outcome.point}`,
            odd: String(outcome.price),
          })),
        });
      }
    }
    return { id: bookmaker.key, name: bookmaker.title, bets };
  }).filter((bookmaker) => bookmaker.bets.length > 0);
  return bookmakers.length ? [{ bookmakers }] : [];
}

async function apiSportsFixtureIdFor(sport, game, options = {}) {
  ...
  const date = providerDateForGame(sport, game.date);
  const provider = sport === 'baseball' ? 'baseball'
    : sport === 'american_football' ? 'american_football' : 'basketball';
  if (!date || !withinApiSportsDateWindow(date)) return null;
  const mappingTtl = options.ttl ?? (sport === 'baseball' ? 10 * 60 : 6 * 3600);
  const response = await getApiSportsGamesByDate(provider, date, {
    ttl: mappingTtl,
    // Las altas tempranas de API-Baseball pueden devolver una cartelera aún
    // incompleta. Un namespace corto evita que ese vacío quede pegado durante
    // seis horas justo cuando Bet365 empieza a publicar sus líneas.
    ...(sport === 'baseball'
      ? {
        params: { timezone: 'America/Bogota' },
        cacheKey: `apisports:${provider}:games:${date}:odds-map-v2-bogota`,
      }
      : {}),
  });
...
export const multisportProviderInternals = {
  fixtureSlug,
  providerDateForGame,
  theOddsApiSportsPayload,
  uniqueGames,
  withinApiSportsDateWindow,
};

function emptyMultisportOdds(source, providerFixtureId = null) {
  return {
    moneyline: {}, totals: {}, spreads: {}, periods: {}, teamTotals: { home: {}, away: {} },
    statistics: { hits: { home: {}, away: {}, total: {} } }, playerProps: {},
    specials: {}, catalog: [], rawBookmakers: [], source, providerFixtureId,
  };
}

// Una sola llamada trae la cartelera COMPLETA del sport-key del día (3
// créditos), sin importar si hay 5 o 150 partidos — nunca partido por
// partido. Usado como respaldo/único intento en varios puntos de
// getSportOdds.
async function tryFeaturedOdds(config, game, options) {
  const featured = await fetchMultisportBet365Event(config.key, game, {
    ttl: options.ttl ?? 3 * 3600,
    emptyTtl: options.emptyTtl,
  }).catch((error) => {
    console.warn(`[${config.key}:odds] The Odds API no disponible: ${error.message}`);
    return { event: null, source: 'the-odds-api' };
  });
  if (!featured.event) return null;
  return {
    ...normalizeApiSportsOdds(theOddsApiSportsPayload(featured.event), game, {
      sport: config.key,
      bookmakers: ['Bet365'],
    }),
    source: 'the-odds-api',
    providerFixtureId: featured.event.id,
    providerSportKey: featured.sportKey,
  };
}
...
export async function getSportOdds(sport, game, options = {}) {
  ...
  // Defensa para payloads/colas antiguas: MLB (1) y Triple-A (11, Bet365 SÍ
  // publica cuota ahí) son las únicas ligas de béisbol activas. El resto de
  // MiLB (AA/A+/A/Rookie) sigue fuera y nunca debe provocar una consulta.
  if (config.key === 'baseball' && !['1', '11'].includes(String(game?.league?.id || '1'))) {
    return emptyMultisportOdds('sin-cuota-publicada');
  }
  ...
  const hasContractualEmbeddedOdds = hasEmbeddedOdds
    && (embedded.rawBookmakers || []).some((bookmaker) => (
      String(bookmaker?.name || '').toLowerCase().replace(/[^a-z0-9]/g, '').includes('bet365')
    ));
  // Las jornadas universitarias pueden superar ampliamente los 100 partidos:
  // NUNCA pasan por API-Sports partido a partido (agotaría su cupo diario,
  // compartido con NBA/NCAAB o NFL). Primero la cuota ya incluida en el
  // calendario de ESPN; si ESPN no la trae, una sola llamada a The Odds API
  // trae la cartelera COMPLETA del día (mismo costo con 5 o 150 partidos, vía
  // tryFeaturedOdds) en vez de depender exclusivamente de ESPN.
  if (/^espn-ncaa/.test(String(game?.dataProvider || ''))) {
    if (hasContractualEmbeddedOdds) {
      return { ...embedded, source: 'espn-odds', providerFixtureId: game.providerFixtureId };
    }
    const featuredNcaa = await tryFeaturedOdds(config, game, options);
    if (featuredNcaa) return featuredNcaa;
    return emptyMultisportOdds('sin-cuota-bet365', game.providerFixtureId);
  }
  ...
  if (hasEmbeddedOdds && !withinApiSportsDateWindow(date)) {
    return hasContractualEmbeddedOdds
      ? { ...embedded, source: 'espn-odds', providerFixtureId: game.espnProviderFixtureId || game.providerFixtureId }
      : emptyMultisportOdds('sin-cuota-bet365', game.espnProviderFixtureId || game.providerFixtureId);
  }

  // API-Sports (API-Baseball/API-Basketball/API-NFL) es la fuente PRIMARIA:
  // el mismo proveedor que ya entrega Bet365 en fútbol, con 100 llamadas/día
  // por deporte — muy por encima del cupo compartido de The Odds API (~16/día
  // entre béisbol, NBA, NCAAB, NFL y NCAAF). Un partido cuesta como máximo dos
  // llamadas (mapeo de ID del día + cuotas del partido), lejos del límite
  // diario salvo NCAA (ya protegida arriba).
  const providerFixtureId = await apiSportsFixtureIdFor(config.key, game, { ttl: options.mappingTtl }).catch((error) => {
    console.warn(`[${config.key}:odds] mapeo API-Sports falló: ${error.message}`);
    return null;
  });
  if (providerFixtureId) {
    const payload = await getApiSportsOddsForGame(provider, providerFixtureId, { ttl: options.ttl ?? 6 * 3600 }).catch((error) => {
      console.warn(`[${config.key}:odds] API-Sports no disponible: ${error.message}`);
      return null;
    });
    if (payload) {
      const normalized = normalizeApiSportsOdds(payload, game, {
        sport: config.key,
        // Los tres productos se presentan contra la misma casa contractual. No
        // se mezcla DraftKings/Bwin ni se toma una línea que el cliente no vaya
        // a encontrar exactamente en Bet365.
        bookmakers: ['Bet365'],
      });
      if (normalized.rawBookmakers.length > 0) {
        return { ...normalized, source: `api-${provider}`, providerFixtureId };
      }
    }
  }

  // The Odds API como respaldo — sólo si API-Sports no tenía la línea todavía.
  const featured = await tryFeaturedOdds(config, game, options);
  if (featured) return featured;

  return emptyMultisportOdds('sin-cuota-bet365', providerFixtureId || null);
}
