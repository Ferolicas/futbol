...
export function normalizeNflGame(raw, requestedDate) {
  ...
  return {
    ...
    league: { id: String(raw.league?.id || 1), name: raw.league?.name || 'NFL', logo: raw.league?.logo || null },
...
export function normalizeMlbGame(raw) {
  ...
  return {
    ...
    league: { id: '1', name: 'MLB', logo: null },
    country: { name: 'USA' },
    status,
    teams: {
...
export async function getSportGamesByDate(sport, date, options = {}) {
  ...
  if (config.key === 'baseball') {
    const games = await getMlbScheduleByDate(date, 1);
    return games.map(normalizeMlbGame);
  }
  if (config.key === 'basketball') return getBasketballGamesByDate(date, options);
  const response = await getApiSportsGamesByDate('american_football', date, {
    ttl: options.ttl ?? 600,
    params: { timezone: options.timeZone || 'America/Bogota' },
  });
  const leagueIds = new Set((options.leagueIds || config.apiSportsLeagueIds).map(Number));
  return response.response.filter((game) => leagueIds.has(Number(game.league?.id))).map((game) => normalizeNflGame(game, date));
}
...
async function apiSportsFixtureIdFor(sport, game) {
  if (sport === 'american_football' || (sport === 'basketball' && game.dataProvider === 'api-basketball')) {
    return game.providerFixtureId;
  }
  const date = new Date(game.date).toISOString().slice(0, 10);
  const provider = sport === 'baseball' ? 'baseball' : 'basketball';
...
export async function getSportOdds(sport, game, options = {}) {
  ...
  if (!providerFixtureId) return { moneyline: {}, totals: {}, spreads: {}, periods: {}, rawBookmakers: [], source: `api-${provider}`, providerFixtureId: null };
  const payload = await getApiSportsOddsForGame(provider, providerFixtureId, { ttl: options.ttl ?? 6 * 3600 });
  return { ...normalizeApiSportsOdds(payload, game), source: `api-${provider}`, providerFixtureId };
}
