...
export async function getSportOdds(sport, game, options = {}) {
  ...
  // Va PRIMERO: protege el cupo diario de API-Sports (compartido entre NBA y
  // NCAAB) de una sola jornada universitaria que por sí sola lo agotaría.
  if (/^espn-ncaa/.test(String(game?.dataProvider || ''))) {
    return hasContractualEmbeddedOdds
      ? { ...embedded, source: 'espn-odds', providerFixtureId: game.providerFixtureId }
      : emptyMultisportOdds('sin-cuota-bet365', game.providerFixtureId);
  }
