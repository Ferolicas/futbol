...
export async function getSportOdds(sport, game, options = {}) {
  ...
  // API-Baseball no ofrece un cruce fiable para los IDs oficiales de MiLB.
  // La predicción sigue calculándose con sus hechos reales, pero no se fabrica
  // una cuota ni se consumen decenas de llamadas inútiles por jornada.
  if (config.key === 'baseball' && String(game?.league?.id || '1') !== '1') {
    return { moneyline: {}, totals: {}, spreads: {}, periods: {}, rawBookmakers: [], source: 'sin-cuota-publicada', providerFixtureId: null };
  }
  const date = new Date(game.date).toISOString().slice(0, 10);
