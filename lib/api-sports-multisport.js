...
export async function getApiSportsGamesByDate(provider, date, options = {}) {
  return apiSportsRequest(provider, '/games', { date, ...(options.params || {}) }, {
    ttl: options.ttl ?? 600,
    cacheKey: options.cacheKey || `apisports:${provider}:games:${date}:${stableParams(options.params || {})}`,
  });
}
...
function genericPeriodDefinition(marketName) {
  const market = exactMarketName(marketName);
  if (/1st half|first half|half time|halftime/.test(market)) return { key: 'firstHalf', label: '1.ª mitad' };
  if (/2nd half|second half/.test(market)) return { key: 'secondHalf', label: '2.ª mitad' };
  const quarter = market.match(/(?:quarter|q)\s*([1-4])|([1-4])(?:st|nd|rd|th) quarter/);
  const number = Number(quarter?.[1] || quarter?.[2]);
  if (number >= 1 && number <= 4) return { key: `quarter${number}`, label: `${number}.º cuarto` };
  return null;
}

function ensureGenericPeriod(result, definition) {
  result.periods[definition.key] ||= {
    label: definition.label,
    moneyline: {}, totals: {}, spreads: { home: {}, away: {} },
...
// API-Baseball expresa el Asian Handicap sobre el eje del equipo local: en
// una selección Away el signo publicado por el feed es el inverso del que ve
// el cliente en Bet365. Ejemplo real: `Away -4.5 @1.12` corresponde a que el
// visitante recibe +4.5; `Away +4.5 @7.25`, a visitante -4.5. Home conserva
// el signo. Esta regla también aplica a los hándicaps por tramos.
function canonicalBaseballHandicap(selectionTeam, providerLine) {
  const line = selectionTeam === 'away' ? -Number(providerLine) : Number(providerLine);
  return {
    line,
    selectionName: `${selectionTeam === 'away' ? 'Away' : 'Home'} ${signedLine(line)}`,
  };
}
...
function normalizeBaseballBet365Item(result, bookmaker, bet, item, odd, fixture) {
  ...
  if (isExactMarket(2, 'asian handicap')) {
    ...
    const canonical = canonicalBaseballHandicap(selectionTeam, line);
    result.spreads[selectionTeam] ||= {};
    const extra = {
      team: selectionTeam,
      line: canonical.line,
      selectionName: canonical.selectionName,
      providerLine: line,
      providerSelectionName: label,
    };
    const metadata = marketMetadata(bookmaker, bet, item, 'asian_handicap', extra);
    bestSet(result.spreads[selectionTeam], canonical.line, odd, bookmaker.name, metadata);
    rememberCatalog(result, bookmaker, bet, item, odd, 'asian_handicap', extra);
    return true;
  }
  ...
  if (period && /asian handicap/.test(market)) {
    ...
    bestSet(target.spreads[selectionTeam], canonical.line, odd, bookmaker.name, metadata);
...
// ── Coherencia interna del espejo de cuotas ────────────────────────────────
//
// API-Baseball no publica las cuotas de Bet365: publica un espejo suyo, y ese
// espejo trae familias mal etiquetadas. Caso comprobado (Boston–Chicago White
// Sox, 6 de agosto de 2026), misma casa, mismo partido, mismo suceso:
//
//   [38] A Run (1st Inning)       Yes = 2.00  → 47% de que se anote en la 1.ª
//   [11] Over/Under (1st Inning)  Over 0.5 = 1.16 → 86% de que se anote en la 1.ª
//
// Ambas no pueden ser ciertas. La escalera del mercado 11 (0.5→86%, 1.5→60%,
// 2.5→36%, 3.5→22%) es la de un total de varias entradas, no la de una: de ahí
// salía el "menos de 3.5 carreras a 1.20" que en Bet365 no existe.
//
// La regla no adivina cuál es la buena: cuando dos mercados de la misma casa
// describen el mismo suceso y se contradicen, se descarta el ambiguo (la
// escalera) y se conserva el inequívoco (sí/no). Así nunca se publica un precio
// que el cliente no vaya a encontrar en la casa.
const SIBLING_TOLERANCE = 0.15;

function impliedProbability(entry) {
  const odd = Number(entry?.odd ?? entry);
  return Number.isFinite(odd) && odd > 1 ? 1 / odd : null;
}

function dropIncoherentBaseballMarkets(result) {
  const inning1 = result.periods?.inning1;
  if (!inning1) return;
  // "Over 0.5" y "A Run — Yes" son literalmente el mismo suceso.
  const ladder = impliedProbability(inning1.totals?.['0.5']?.over);
  const explicit = impliedProbability(inning1.specials?.run_yes);
  if (ladder == null || explicit == null) return;
  if (Math.abs(ladder - explicit) <= SIBLING_TOLERANCE) return;

  const dropped = Object.keys(inning1.totals || {}).length;
  inning1.totals = {};
  result.catalog = result.catalog.filter((entry) => entry.family !== 'inning1_total');
  console.warn(
    `[odds:coherencia] 1.ª entrada descartada: la escalera dice ${Math.round(ladder * 100)}% `
    + `y "A Run" dice ${Math.round(explicit * 100)}% para el mismo suceso (${dropped} líneas fuera)`,
  );
}
...
export function normalizeApiSportsOdds(payload, fixture = {}, options = {}) {
    ...
    for (const bookmaker of row?.bookmakers || []) {
      ...
      for (const bet of bookmaker.bets || []) {
        ...
        for (const item of bet.values || []) {
          ...
          const periodDefinition = genericPeriodDefinition(market);
          const period = periodDefinition ? ensureGenericPeriod(result, periodDefinition) : null;
          const marketTeam = /home team|home total|total home/.test(market)
            ? 'home'
            : (/away team|away total|total away/.test(market) ? 'away' : null);
          const selectionTeam = /^home\b|^1\b|local/.test(label) || (homeName && label.includes(homeName))
            ? 'home'
            : (/^away\b|^2\b|visit/.test(label) || (awayName && label.includes(awayName)) ? 'away' : marketTeam);
          const drawSelection = /draw|tie|empate|^x$/.test(label);
          const isWinner = /home\/away|moneyline|money line|match winner|winner|ganador|1x2/.test(market)
            && !/run line|handicap|spread/.test(market);
          if (isWinner) {
            const target = period?.moneyline || result.moneyline;
            const family = period ? 'period_moneyline' : 'moneyline';
            if (selectionTeam) bestSet(target, selectionTeam, odd, bookmaker.name, marketMetadata(bookmaker, bet, item, family, { team: selectionTeam, period: periodDefinition?.key || null }));
            else if (drawSelection) bestSet(target, 'draw', odd, bookmaker.name, marketMetadata(bookmaker, bet, item, family, { team: 'draw', period: periodDefinition?.key || null }));
            rememberCatalog(result, bookmaker, bet, item, odd, family, { team: selectionTeam || (drawSelection ? 'draw' : null), period: periodDefinition?.key || null });
            continue;
          }
          ...
          const teamTotal = /team total|home total|away total|total.*(?:home|away)|(?:home|away).*total/.test(market);
          if (/total|over\/under/.test(market) && side && teamTotal && selectionTeam) {
            const target = period?.teamTotals || result.teamTotals;
            target[selectionTeam][line] ||= {};
            const family = period ? 'period_team_total' : 'team_total';
            const metadata = marketMetadata(bookmaker, bet, item, family, { team: selectionTeam, side, line, period: periodDefinition?.key || null });
            bestSet(target[selectionTeam][line], side, odd, bookmaker.name, metadata);
            rememberCatalog(result, bookmaker, bet, item, odd, family, metadata);
          } else if (/total|over\/under/.test(market) && side && !teamTotal) {
            const target = period?.totals || result.totals;
            target[line] ||= {};
            const family = period ? 'period_total' : 'game_total';
            const metadata = marketMetadata(bookmaker, bet, item, family, { side, line, period: periodDefinition?.key || null });
            bestSet(target[line], side, odd, bookmaker.name, metadata);
            rememberCatalog(result, bookmaker, bet, item, odd, family, metadata);
          } else if (/handicap|spread|run line/.test(market)) {
            if (selectionTeam) {
              const target = period?.spreads || result.spreads;
              const family = period ? 'period_spread' : 'spread';
              const metadata = marketMetadata(bookmaker, bet, item, family, { team: selectionTeam, line, period: periodDefinition?.key || null });
              target[selectionTeam] ||= {};
              bestSet(target[selectionTeam], line, odd, bookmaker.name, metadata);
              rememberCatalog(result, bookmaker, bet, item, odd, family, metadata);
            }
          }
        }
  ...
  if (sport === 'baseball') dropIncoherentBaseballMarkets(result);
  return result;
}
