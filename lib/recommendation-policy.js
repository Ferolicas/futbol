...
export function sanitizeFootballCombinada(combinada) {
  if (!combinada || typeof combinada !== 'object') return combinada || null;
  const canonical = combinada.source === 'context-engine';
  const reliable = (items) => (canonical && Array.isArray(items)
    ? items.filter((item) => meetsFootballReliability(item?.confidence))
    : []);
  ...
  // sí podemos reutilizar el subconjunto principal que demuestra >=90.
  const selectable = reliableSelectable.length > 0 ? reliableSelectable : selections;
