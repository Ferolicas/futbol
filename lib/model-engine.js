...
// Aplica H2H al 1X2 según H2H_1X2_MODE (env TEMPORAL para comparar en backtest).
// Venue: solo duelos con el local de hoy en casa (is_home en su perspectiva).
//   default    = actual: modo (a) plano 0.6 → luego modo (b) curva.
//   merge      = fusiona venue de (a)+(b) en UN conteo y un solo blend (peso 0.6).
//   softweight = separa a/b, pero peso de (a) = 0.6·n_cur/(n_cur+3) (suave si n_cur chico).
function apply1x2H2H(mk, cur, hist) {
  if (!mk) return;
  const mode = process.env.H2H_1X2_MODE;
