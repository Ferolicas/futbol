...
function applyPlayer(markets, pctx, ctx) {
  ...
  // 1X2 — canal merma posicional: el equipo mermado "marca" gol_equiv(merma) goles menos.
  // La merma cruda (lineal) se acota con saturación suave antes de entrar al recuento (no 1:1).
  const h1 = homeVenue.map((r) => shiftRow(r, -mermaToGoals(H.merma), 0));
  const a1 = awayVenue.map((r) => shiftRow(r, -mermaToGoals(A.merma), 0));
