...
const PLAYER_CAP = 1;           // tope ±1 gol del shift agregado por lado
...
// ── capa de jugador (4C-2) ────────────────────────────────────────────────
// Se aplica DESPUÉS de applyH2H, solo a familias de GOL. Un titular habitual del
// equipo que hoy NO arranca desplaza la expectativa de gol del equipo (delta_gf
// ofensivo, delta_ga defensivo, de player_impact 4A), ponderado por confianza de
// muestra y topado ±1. El ladder se RECUENTA sobre los valores desplazados (pRate,
// igual que el núcleo) y el efecto se compone en MOMIOS sobre la prob pos-H2H.
// Pura, sin escrituras. No toca córners/tarjetas/tiros/faltas/offsides.
...
// lee lineup + player_impact + ratio de titularidad reciente + filas de hechos.
async function fetchPlayerContext(pool, fixtureId, ctx) {
  const cutoff = ctx.cutoff || new Date();
  const teams = [Number(ctx.homeTeamId), Number(ctx.awayTeamId)];
  const [lu, imp, sr, homeRows, awayRows] = await Promise.all([
    pool.query(`SELECT player_id, is_starter FROM model.lineups WHERE fixture_id = $1`, [fixtureId]),
    pool.query(`SELECT player_id, team_id, delta_gf, delta_ga, n_with, n_without FROM model.player_impact WHERE team_id = ANY($1::bigint[])`, [teams]),
    // ratio de titularidad desde model.lineups (is_starter fiable distingue XI vs banca;
    // player_match_stats.is_starter está inflado y marca titular a suplentes).
    pool.query(
  ...
  return { hasLineup: lu.rows.length > 0, starters, impact: imp.rows, ratio, homeRows, awayRows };
}

// ausentes con impacto de un equipo: titular habitual (ventana/umbral/min) + tiene
// par en player_impact + hoy NO arranca. Devuelve {player_id, c, delta_gf, delta_ga}.
function teamAbsentees(pctx, teamId) {
  ...
  for (const ir of pctx.impact) {
    ...
    // titular habitual: sin registro en lineups, o napp<5 (muestra insuficiente,
    // p.ej. napp=1 ratio=1.0), o ratio<60% → NO cuenta como ausente con impacto.
    if (!sr || sr.napp < PLAYER_MIN_APP || sr.ratio < PLAYER_STARTER_RATIO) continue;
    if (pctx.starters.has(pid)) continue;                                              // hoy arranca → presente
    const nEff = Math.min(Number(ir.n_with), Number(ir.n_without));
    out.push({ player_id: pid, c: nEff / (nEff + PLAYER_K), delta_gf: Number(ir.delta_gf), delta_ga: Number(ir.delta_ga) });
  }
  return out;
}
function teamShift(absentees) {
  let gf = 0, ga = 0;
  for (const a of absentees) { gf += a.c * a.delta_gf; ga += a.c * a.delta_ga; }
  const cap = (x) => Math.max(-PLAYER_CAP, Math.min(PLAYER_CAP, -x)); // se QUITA el aporte del ausente
  return { shift_gf: round(cap(gf)), shift_ga: round(cap(ga)), ausentes: absentees.map((a) => a.player_id) };
}
// resumen de shifts (también lo usa la sonda para mostrarlos).
function computePlayerShifts(pctx, ctx) {
  return { hasLineup: pctx.hasLineup, home: teamShift(teamAbsentees(pctx, ctx.homeTeamId)), away: teamShift(teamAbsentees(pctx, ctx.awayTeamId)) };
}

function playerStep(side, n) { // side = {home?, away?} con {shift_gf,shift_ga,ausentes}
  const s = { step: 'player', n };
  if (side.home && side.away) { s.home = side.home; s.away = side.away; }
  else { const t = side.home || side.away; s.shift_gf = t.shift_gf; s.shift_ga = t.shift_ga; s.ausentes = t.ausentes; }
  return s;
}
function modPlayerOU(mk, orig, shifted, mkFn, side) {
  if (!mk) return;
  for (const ln of mk.lines) {
    ...
    const st = playerStep(side, pO.n); st.before = ln.prob; st.after = after;
    ln.chain.push(st); ln.prob = after;
  }
}
function modPlayerBool(mk, orig, shifted, fn, side) {
  if (!mk) return;
  const pO = pRate(orig, fn), pS = pRate(shifted, fn);
  if (pO.n === 0 || pO.p == null || pS.p == null) return;
  const after = oddsCompose(mk.prob, pO.p, pS.p);
  if (after == null || after === mk.prob) return;
  const st = playerStep(side, pO.n); st.before = mk.prob; st.after = after;
  (mk.chain = mk.chain || []).push(st); mk.prob = after;
}
function modPlayer1x2(mk, hOrig, hShift, aOrig, aShift, side) {
  if (!mk) return;
  ...
  const s = home + draw + away || 1;
  mk.home = round(home / s); mk.draw = round(draw / s); mk.away = round(away / s);
  const st = playerStep(side, n); st.before = before; st.after = { home: mk.home, draw: mk.draw, away: mk.away };
  (mk.chain = mk.chain || []).push(st);
}

// aplica la capa de jugador a las familias de gol. Muta in-place y devuelve markets.
function applyPlayer(markets, pctx, ctx) {
  ...
  if (H.shift_gf === 0 && H.shift_ga === 0 && A.shift_gf === 0 && A.shift_ga === 0) return markets;
  ...
  const unionOrig = homeVenue.concat(awayVenue);
  const unionShift = homeShift.concat(awayShift);
  const both = { home: H, away: A };

  modPlayerOU(markets.goals_total, unionOrig, unionShift, rcOverTotal, both);
  modPlayerOU(markets.goals_home, homeVenue, homeShift, rcOverFor, { home: H });
  modPlayerOU(markets.goals_away, awayVenue, awayShift, rcOverFor, { away: A });
  modPlayerBool(markets.btts, unionOrig, unionShift, rcBtts, both);
  modPlayerBool(markets.clean_sheet_home, homeVenue, homeShift, rcClean, { home: H });
  modPlayerBool(markets.clean_sheet_away, awayVenue, awayShift, rcClean, { away: A });
  modPlayer1x2(markets['1x2'], homeVenue, homeShift, awayVenue, awayShift, both);
  return markets;
}

module.exports = { computeBaseMarkets, fetchH2HRows, applyH2H, fetchPlayerContext, applyPlayer, computePlayerShifts, tier, K };
