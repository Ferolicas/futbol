...
const MERMA_CAP = 2.0;          // tope de la merma agregada (penalización de margen en 1X2)
...
//   · 1X2 (merma posicional = piso por rol + K·|delta_canal|) → menos victoria del mermado
...
// canales agregados de un equipo con AGREGACIÓN AMORTIGUADA; clamp ±1 como red final.
function teamChannels(absentees) {
  ...
  return {
    ...
    merma: round(Math.min(MERMA_CAP, Math.max(0, dampedAggregate(absentees.map((a) => a.merma))))),
...
function applyPlayer(markets, pctx, ctx) {
  ...
  // 1X2 — canal merma posicional: el equipo mermado "marca" merma goles menos (menos victoria)
  const h1 = homeVenue.map((r) => shiftRow(r, -H.merma, 0));
  const a1 = awayVenue.map((r) => shiftRow(r, -A.merma, 0));
