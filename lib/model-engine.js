...
const { posteriorReliabilityPercent } = require('./reliability-core.cjs');
...
function teamWeightedReliability(est, inverse = false) {
  ...
  const values = teams
    .filter((team) => Number(team?.n) > 0)
    .map((team) => footballReliability(inverse ? Number(team.n) - Number(team.hits) : team.hits, team.n));
