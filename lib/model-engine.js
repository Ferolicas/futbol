...
function mod1x2(mk, rows, mode) {
  ...
  let home = h2hCombine(mk.home, W.p, n, mode);   // H gana = local hoy
  let draw = h2hCombine(mk.draw, D.p, n, mode);
  let away = h2hCombine(mk.away, L.p, n, mode);   // H pierde = visita hoy
  const s = home + draw + away || 1;
  mk.home = round(home / s); mk.draw = round(draw / s); mk.away = round(away / s);
  (mk.chain = mk.chain || []).push({ step: 'h2h', mode, before, after: { home: mk.home, draw: mk.draw, away: mk.away }, n });
}
...
function modulateH2H(markets, rows, mode) {
  ...
  // 1X2 y CLEAN SHEET: "quién gana / quién deja la valla a cero" depende de DÓNDE.
  // Solo duelos del mismo venue que hoy: H jugando en casa (is_home en su perspectiva).
  // Mezclar casa+visita rompe la predicción (p.ej. City 3W-1L en casa vs 0W-2L fuera).
