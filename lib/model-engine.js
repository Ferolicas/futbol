...
const BT_GOALS = [   // goles por mitad: total/home/away (gf_Xh con for+against disponibles)
  { key: 'goals_1h', total: (r) => add2(r.gf_1h, r.ga_1h), team: (r) => num(r.gf_1h) },
  { key: 'goals_2h', total: (r) => add2(r.gf_2h, r.ga_2h), team: (r) => num(r.gf_2h) },
];
...
function goalPMF(rows, valFn, maxK) {
  const vals = rows.map(valFn).filter((v) => v != null);
  if (!vals.length) return null;
  const pmf = new Array(maxK + 1).fill(0);
  for (const v of vals) pmf[Math.min(maxK, Math.max(0, Math.round(v)))]++;
  return pmf.map((c) => c / vals.length);
}
...
function buildDerived(markets, homeL2, awayL2, unionL2) {
  const MAXG = 6;
  const x = markets['1x2'];
  if (x) markets.double_chance = { kind: 'multi', '1X': round(x.home + x.draw), '12': round(x.home + x.away), 'X2': round(x.draw + x.away) };
  const tot = unionL2.map((r) => num(r.total_goals)).filter((v) => v != null);
  if (tot.length) { const even = tot.filter((v) => v % 2 === 0).length / tot.length; markets.odd_even = { kind: 'multi', even: round(even), odd: round(1 - even), n: tot.length }; }
  const hp = goalPMF(homeL2, (r) => num(r.goals_for), MAXG);   // marginal local
  const ap = goalPMF(awayL2, (r) => num(r.goals_for), MAXG);   // marginal visitante
  if (!hp || !ap) return;
  const exact = [], diff = {};
  ...
  markets.exact_score = { kind: 'list', n: homeL2.length + awayL2.length, lines: exact.filter((e) => e.prob >= 0.02).slice(0, 12) };
...
async function computeBaseMarkets(pool, ctx, opts = {}) {
  ...
  const rH = buildBool(homeLevels, (r) => r.had_red_for, 'home', base);  if (rH) markets.red_card_home = rH;
  const rA = buildBool(awayLevels, (r) => r.had_red_for, 'away', base);   if (rA) markets.red_card_away = rA;
  const rAny = buildBool(totalLevels, (r) => (r.had_red_for || r.had_red_against), 'all', base); if (rAny) markets.red_card_any = rAny;
  const fg = buildBool(totalLevels, (r) => (r.first_goal_minute != null && r.first_goal_minute <= 45), 'all', base); if (fg) markets.first_goal_1h = fg;
  const l2 = (levels) => (levels.find((l) => l.name === 'L2') || { rows: [] }).rows;
  buildDerived(markets, l2(homeLevels), l2(awayLevels), l2(totalLevels));
...
function apply1x2H2H(mk, cur) {
  if (!mk) return;
  // ETAPA 1: solo temporada actual; sin histórico. Venue-filtrado (H de local hoy).
  const curHV = cur.filter((r) => r.is_home === true);
  if (!curHV.length) return;
  const mode = process.env.H2H_1X2_MODE || 'softweight';   // softweight = peso escalado por n; merge/default = plano 0.6
  const w = mode === 'softweight' ? H2H_A_WEIGHT * curHV.length / (curHV.length + 3) : H2H_A_WEIGHT;
  blend1x2(mk, curHV, w, 'a');
}
...
function applyH2H(markets, h2hRows, ctx) {
  ...
  // ETAPA 1 (4E): SOLO modo (a) = duelos de la TEMPORADA ACTUAL (cualquier competición, sin
  // filtrar comp_id). El modo (b) histórico se ELIMINÓ del serving — el backtest mostró que su
  // ruido de muestra chica (duelos viejos, otras plantillas) empeoraba el ECE. fetchH2HRows y
  // modulateH2H se conservan (el histórico podría reusarse en un análisis segmentado futuro).
  const cur = h2hRows.filter((r) => Number(r.comp_season) === Number(ctx.season));
  if (!cur.length) return markets;
  modulateH2H(markets, cur, 'a');
  apply1x2H2H(markets['1x2'], cur);   // solo temporada actual (sin histórico)
  return markets;
}
...
//   • H2H ENCENDIDO por defecto (MODEL_H2H_ENABLED==='false' para apagar), RESTRINGIDO a la
//     TEMPORADA ACTUAL (Etapa 1): el backtest 4E mostró que el H2H histórico (modo b) empeoraba
//     el ECE por ruido de muestra chica; eliminado el histórico, solo la temporada actual modula.
...
async function predict(pool, ctx, opts = {}) {
  ...
  const h2hOn = opts.h2h != null ? !!opts.h2h : process.env.MODEL_H2H_ENABLED !== 'false';             // default ON (Etapa 1: H2H solo temporada actual)
