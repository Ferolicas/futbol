  ...
  const [lu, imp, sr, homeRows, awayRows] = await Promise.all([
    ...
    pool.query(
      `SELECT player_id, team_id, avg(is_starter::int) AS ratio, count(*) AS napp FROM (
         SELECT player_id, team_id, is_starter,
                row_number() OVER (PARTITION BY player_id, team_id ORDER BY kickoff DESC) rn
         FROM model.player_match_stats
         WHERE team_id = ANY($1::bigint[]) AND minutes > 0 AND kickoff < $2
