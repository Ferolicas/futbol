...
} = require('./reliability-core.cjs');
const { normalizeDistribution } = require('./prediction-math.cjs');

const K = 12;               // soporte muestral de 1X2; NO modifica probabilidades
const CURRENT_SEASON_SHARE = 0.65;
const FOOTBALL_RELIABILITY_BASELINE = 0.70;
...
const DEFAULT_ENGINE_CONFIG = Object.freeze({
  currentShare: CURRENT_SEASON_SHARE,
  venueBoost: 1.18,
  stadiumBoost: 1.00,
  ...
  h2hBoost: 1.22,
  refereeBoost: 1.10,
  // Neutro hasta que el walk-forward valide que comparar congestión mejora.
  restBoost: 1.00,
...
// resultado 'W'/'D'/'L' de una métrica for/against cualquiera (goles por
// mitad, córners, tiros, faltas...), misma lectura que r.result para 1x2.
const derivedResult = (forVal, againstVal) => {
  if (forVal == null || againstVal == null) return null;
  const f = Number(forVal), a = Number(againstVal);
...
const sampleSupport = (n) => n > 0 ? n / (n + K) : 0;
const footballReliability = (hits, n) => {
  const percent = posteriorReliabilityPercent(hits, n, FOOTBALL_RELIABILITY_BASELINE);
...
function normalizeEngineConfig(raw = {}) {
  return {
    // Contrato de producto: actualidad 65%, histórico 35%. El entrenamiento
    // optimiza contexto, pero no puede rebajar la actualidad por una muestra
    // histórica más grande al inicio de una liga.
    currentShare: CURRENT_SEASON_SHARE,
    venueBoost: bounded(raw.venueBoost, DEFAULT_ENGINE_CONFIG.venueBoost, 1, 2),
    stadiumBoost: bounded(raw.stadiumBoost, DEFAULT_ENGINE_CONFIG.stadiumBoost, 1, 1.6),
    ...
    restBoost: bounded(raw.restBoost, DEFAULT_ENGINE_CONFIG.restBoost, 1, 1.6),
...
// Cada equipo aporta una estimación propia y ambos lados pesan lo mismo en el
// partido de hoy. Así 100 partidos de un club nunca aplastan los 10 del rival:
// primero se calcula el cumplimiento de cada equipo (incluido su 65/35) y luego
// se ponderan las dos señales disponibles al 50/50.
function teamWeightedEmpiricalRate(rows, fn, currentShare = DEFAULT_ENGINE_CONFIG.currentShare) {
  const groups = new Map();
  for (const row of rows) {
    const teamId = Number(row?.team_id);
    const key = Number.isFinite(teamId) && teamId > 0 ? teamId : '__unknown__';
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(row);
  }
  const teams = [...groups.entries()]
    .map(([teamId, teamRows]) => ({ teamId: teamId === '__unknown__' ? null : teamId, ...empiricalRate(teamRows, fn, currentShare) }))
    .filter((estimate) => estimate.p != null);
  if (!teams.length) return empiricalRate(rows, fn, currentShare);
  const pooled = empiricalRate(rows, fn, currentShare);
  return {
    ...pooled,
    p: teams.reduce((sum, estimate) => sum + estimate.p, 0) / teams.length,
    currentShare,
    teams,
  };
}

function teamWeightedReliability(est, inverse = false) {
  const teams = Array.isArray(est?.teams) && est.teams.length ? est.teams : [est];
  const teamWeight = 1 / teams.length;
  const currentShare = Number(est?.currentShare ?? CURRENT_SEASON_SHARE);
  const samples = [];
  const add = (segment, weight) => {
    const n = Number(segment?.n || 0);
    if (!n) return;
    const hits = inverse ? n - Number(segment.hits || 0) : Number(segment.hits || 0);
    samples.push({ hits, n, weight });
  };
  for (const team of teams) {
    const hasCurrent = Number(team?.current?.n || 0) > 0;
    const hasHistorical = Number(team?.historical?.n || 0) > 0;
    if (hasCurrent && hasHistorical) {
      add(team.current, teamWeight * currentShare);
      add(team.historical, teamWeight * (1 - currentShare));
    } else if (hasCurrent) add(team.current, teamWeight);
    else if (hasHistorical) add(team.historical, teamWeight);
    else add(team, teamWeight);
  }
  const percent = weightedPosteriorReliabilityPercent(samples, FOOTBALL_RELIABILITY_BASELINE);
  return percent == null ? 0 : percent / 100;
}
...
function auditChain(est, currentShare) {
  ...
  if (Array.isArray(est.teams)) {
    step.teamWeight = round(1 / est.teams.length);
    step.teams = est.teams.map((team) => ({
      teamId: team.teamId,
      p: round(team.p), n: team.n, hits: team.hits,
      reliability: round(teamWeightedReliability({ teams: [team] })),
      current: { p: round(team.current.p), n: team.current.n, hits: team.current.hits },
      historical: { p: round(team.historical.p), n: team.historical.n, hits: team.historical.hits },
    }));
  }
  return [step];
}
...
async function fetchTeamRows(pool, teamId, cutoff) {
  const { rows } = await pool.query(
    `SELECT ${COLS}, m.referee,m.venue_id,
            otms.corners_1h AS corners_1h_against, otms.corners_2h AS corners_2h_against,
            CASE WHEN tms.is_home THEN m.away_rank_before ELSE m.home_rank_before END AS opp_rank,
            EXTRACT(EPOCH FROM (tms.kickoff - LAG(tms.kickoff) OVER (
              PARTITION BY tms.team_id ORDER BY tms.kickoff
            ))) / 86400.0 AS rest_days
     FROM model.team_match_stats tms
     JOIN model.matches m ON m.fixture_id = tms.fixture_id
     LEFT JOIN model.team_match_stats otms ON otms.fixture_id = tms.fixture_id AND otms.team_id = tms.opponent_id
     WHERE tms.team_id = $1 AND tms.kickoff < $2
     ORDER BY tms.kickoff DESC`, [teamId, cutoff]);
  return rows.map(rowMap);
}

function restBucket(value) {
  const days = Number(value);
  if (!Number.isFinite(days) || days <= 0) return null;
  if (days <= 3.5) return 'congested';
  if (days <= 7.5) return 'normal';
  return 'long';
}
...
function contextualizeRows(rows, contrib, ctx, config) {
  ...
  return rows.map((r) => {
    ...
    if (contrib.targetVenueId != null && Number(r.venue_id) === Number(contrib.targetVenueId)) {
      weight *= config.stadiumBoost;
    }
    ...
    if (restBucket(contrib.targetRestDays)
        && restBucket(r.rest_days) === restBucket(contrib.targetRestDays)) weight *= config.restBoost;
...
async function annotateLineupSimilarity(pool, rawRows, currentLineups, ctx) {
  ...
  const continuity = (items, teamId) => {
    const values = items
      .filter((row) => ctx.season == null || Number(row.season) === Number(ctx.season))
      .sort((left, right) => right.kickoff - left.kickoff)
      .map((row) => similarity.get(`${row.fixture_id}:${teamId}`))
      .filter((value) => value != null)
      .slice(0, 5);
    return values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : null;
  };
  const homeContinuity = continuity(rawRows.homeRaw || [], Number(ctx.homeTeamId));
  const awayContinuity = continuity(rawRows.awayRaw || [], Number(ctx.awayTeamId));
  return {
    ...
    context: {
      ...
      homeContinuity,
      awayContinuity,
      rosterContinuity: homeContinuity == null && awayContinuity == null
        ? null
        : [homeContinuity, awayContinuity].filter((value) => value != null)
          .reduce((sum, value, _, values) => sum + value / values.length, 0),
    },
  };
}

// XI probable reproducible: los 11 jugadores que más titularidades acumulan
// en los cinco partidos anteriores de cada equipo. Nunca consulta el fixture
// objetivo ni una alineación posterior al cutoff.
async function inferExpectedLineups(pool, ctx) {
  if (!pool) return [];
  const { rows } = await pool.query(
    `WITH recent_fixtures AS (
       SELECT team_id,fixture_id,kickoff,
              dense_rank() OVER (PARTITION BY team_id ORDER BY kickoff DESC) AS match_rank
       FROM (
         SELECT DISTINCT lu.team_id,lu.fixture_id,m.kickoff
         FROM model.lineups lu
         JOIN model.matches m ON m.fixture_id=lu.fixture_id
         WHERE lu.team_id=ANY($1::bigint[]) AND lu.is_starter=TRUE AND m.kickoff<$2
       ) fixtures
     ), candidates AS (
       SELECT lu.team_id,lu.player_id,count(*)::int appearances,max(rf.kickoff) last_start
       FROM recent_fixtures rf
       JOIN model.lineups lu ON lu.fixture_id=rf.fixture_id AND lu.team_id=rf.team_id AND lu.is_starter=TRUE
       WHERE rf.match_rank<=5
       GROUP BY lu.team_id,lu.player_id
     ), ranked AS (
       SELECT *,row_number() OVER (PARTITION BY team_id ORDER BY appearances DESC,last_start DESC,player_id) AS player_rank
       FROM candidates
     )
     SELECT team_id,player_id FROM ranked WHERE player_rank<=11 ORDER BY team_id,player_rank`,
    [[Number(ctx.homeTeamId), Number(ctx.awayTeamId)], ctx.cutoff || new Date()],
  );
  const teams = new Map();
  for (const row of rows) {
    const teamId = Number(row.team_id);
  ...
  return [...teams.entries()]
    .filter(([, startXI]) => startXI.length === 11)
    .map(([teamId, startXI]) => ({ team: { id: teamId }, startXI, substitutes: [] }));
}

// ── constructores de mercado ──────────────────────────────────────────────
function buildOU(rows, valFn, config, requiredMaxLine = 0.5) {
  ...
  // El informe personal exige ciertas escaleras aunque estén por encima del
  // p95 observado. No se inventa nada: empiricalRate cuenta los mismos hechos
  // reales y puede devolver legítimamente 0%/100% en esas líneas.
  const hi = Math.max(
    0.5,
    Math.min(Math.max(percentile(vals, LINE_PCT), requiredMaxLine), LINE_HARD_CAP),
  );
  const lines = [];
  ...
  for (const L of lines) {
    ...
    const est = teamWeightedEmpiricalRate(rows, fn, config.currentShare);
    if (est.p == null) continue;
    out.push({
      line: L, prob: round(est.p), level: 'empirical', n: est.n, hits: est.hits,
      conf: round(teamWeightedReliability(est)),
      underConf: round(teamWeightedReliability(est, true)),
      chain: auditChain(est, config.currentShare),
    });
  }
  return out.length ? { kind: 'ou', lines: out } : null;
}

function requiredReportLine(family) {
  if (family === 'corners') return 10.5;
  if (family === 'corners_1h' || family === 'corners_2h') return 4.5;
  if (family === 'goals' || family === 'goals_1h' || family === 'goals_2h') return 4.5;
  if (family === 'cards') return 5.5;
  return 0.5;
}
...
function buildBool(rows, valFn, config) {
  ...
  return {
    kind: 'bool', prob: round(est.p), level: 'empirical', n: est.n, hits: est.hits,
    conf: round(teamWeightedReliability(est)),
    inverseConf: round(teamWeightedReliability(est, true)),
    chain: auditChain(est, config.currentShare),
  };
}
...
// Mercado 1X2 genérico (mismo patrón que markets['1x2']) sobre CUALQUIER
// resultado W/D/L derivado — mitad de gol, córners, tiros, faltas... Cada
// equipo pesa su propio patrón (teamWeightedEmpiricalRate), nunca el
// promedio general de la liga.
function buildResultMarket(homeRows, awayRows, resultFn, config) {
  const outcomeEstimate = (homeCode, awayCode) => teamWeightedEmpiricalRate(unionRows([
    homeRows.map((row) => ({ ...row, _resultHit: resultFn(row) == null ? null : (resultFn(row) === homeCode ? 1 : 0) })),
    awayRows.map((row) => ({ ...row, _resultHit: resultFn(row) == null ? null : (resultFn(row) === awayCode ? 1 : 0) })),
  ]), (row) => row._resultHit, config.currentShare);
  const resultEstimates = {
    home: outcomeEstimate('W', 'L'),
    draw: outcomeEstimate('D', 'D'),
    away: outcomeEstimate('L', 'W'),
  };
  if (!Object.values(resultEstimates).some((estimate) => estimate.p != null)) return null;
  const distribution = normalizeDistribution(Object.fromEntries(
    Object.entries(resultEstimates).map(([key, estimate]) => [key, estimate.p || 0]),
  ));
  return {
    kind: 'result', level: 'empirical',
    home: round(distribution.home), draw: round(distribution.draw), away: round(distribution.away),
    n: Math.max(...Object.values(resultEstimates).map((estimate) => estimate.n || 0)),
    homeConf: round(teamWeightedReliability(resultEstimates.home)),
    drawConf: round(teamWeightedReliability(resultEstimates.draw)),
    awayConf: round(teamWeightedReliability(resultEstimates.away)),
  };
}
...
// Distribución empírica COMPLETA de goles. Nunca colapsa 7, 8, 9... en el
// marcador 6: el soporte llega hasta el máximo realmente observado (con el
// mismo tope de seguridad de las demás familias). La proyección cruza ataque y
// concesión dando a ambos equipos el mismo peso, igual que los O/U del núcleo.
function goalPMF(rows, valFn, config) {
  const values = rows.map(valFn).filter((value) => value != null)
    .map((value) => Math.max(0, Math.round(Number(value))))
    .filter(Number.isFinite);
  if (!values.length) return null;
  if (values.some((value) => value > LINE_HARD_CAP)) {
    throw new Error('prediction_goal_support_exceeds_cap');
  }
  const maxK = Math.min(LINE_HARD_CAP, Math.max(...values));
  const pmf = new Array(maxK + 1).fill(0);
  for (let k = 0; k <= maxK; k++) {
    const est = teamWeightedEmpiricalRate(rows, (r) => {
      const value = valFn(r);
      return value == null ? null : (Math.max(0, Math.round(Number(value))) === k ? 1 : 0);
    }, config.currentShare);
    pmf[k] = est.p || 0;
  }
...
function buildDerived(markets, homeRows, awayRows, union, config) {
  const x = markets['1x2'];
  if (x) markets.double_chance = {
    kind: 'multi', '1X': round(x.home + x.draw), '12': round(x.home + x.away), 'X2': round(x.draw + x.away),
    n: x.doubleChanceEvidence?.n || x.n,
    conf1X: x.doubleChanceEvidence?.['1X']?.conf,
    conf12: x.doubleChanceEvidence?.['12']?.conf,
    confX2: x.doubleChanceEvidence?.X2?.conf,
  };
  const parity = empiricalRate(union, (r) => r.total_goals == null ? null : (Number(r.total_goals) % 2 === 0 ? 1 : 0), config.currentShare);
  if (parity.p != null) markets.odd_even = {
    kind: 'multi', even: round(parity.p), odd: round(1 - parity.p), n: parity.n, hits: parity.hits,
    evenConf: round(teamWeightedReliability(parity)),
    oddConf: round(teamWeightedReliability(parity, true)),
  };
  const hp = goalPMF(homeRows, (r) => num(r._metric), config);   // ataque local + concesión visitante
  const ap = goalPMF(awayRows, (r) => num(r._metric), config);   // ataque visitante + concesión local
  if (!hp || !ap) return;
  const exact = [], diff = {}, totals = {};
  for (let h = 0; h < hp.length; h++) for (let a = 0; a < ap.length; a++) {
    ...
    const total = h + a;
    const bucket = total >= 7 ? '7plus' : String(total);
    totals[bucket] = (totals[bucket] || 0) + p;
  }
  exact.sort((u, v) => v.prob - u.prob);
  markets.exact_score = {
    kind: 'list', n: homeRows.length + awayRows.length,
    // `lines` es solo presentación/mercado de marcador exacto. Los totales se
    // derivan de la matriz completa antes de este recorte visual.
    lines: exact.filter((e) => e.prob >= 0.02).slice(0, 12),
    totalProbabilities: normalizeDistribution(totals),
    support: { homeMax: hp.length - 1, awayMax: ap.length - 1 },
  };
...
async function computeBaseMarkets(pool, ctx, opts = {}) {
  ...
  const baseConfig = normalizeEngineConfig(opts.config || await loadEngineConfig(pool));
  ...
  const currentRows = homeRaw.filter((row) => ctx.season != null && Number(row.season) === Number(ctx.season)).length
    + awayRaw.filter((row) => ctx.season != null && Number(row.season) === Number(ctx.season)).length;
  const continuity = Number(lineupContext?.rosterContinuity);
  const regimeShare = Number.isFinite(continuity) && currentRows >= 3
    ? Math.min(0.85, CURRENT_SEASON_SHARE + (1 - Math.max(0, Math.min(1, continuity))) * 0.20 * Math.min(1, currentRows / 10))
    : CURRENT_SEASON_SHARE;
  const config = { ...baseConfig, currentShare: regimeShare };
  ...
  const homeRows = contextualizeRows(homeRaw, {
    ...
    targetRestDays: ctx.homeRestDays, targetVenueId: ctx.venueId,
  }, cctx, config);
  const awayRows = contextualizeRows(awayRaw, {
    ...
    targetRestDays: ctx.awayRestDays, targetVenueId: ctx.venueId,
  }, cctx, config);
  ...
  for (const fam of OU_FAMILIES) {
    ...
    const requiredLine = requiredReportLine(fam.key);
    const t = buildOU(familyTotalRows, fam.total, config, requiredLine); if (t) markets[`${fam.key}_total`] = t;
    ...
    const h = buildOU(homeEvidence, (r) => r._metric, config, requiredLine); if (h) markets[`${fam.key}_home`] = h;
    const a = buildOU(awayEvidence, (r) => r._metric, config, requiredLine); if (a) markets[`${fam.key}_away`] = a;
  ...
  const outcomeEstimate = (homeCode, awayCode) => teamWeightedEmpiricalRate(unionRows([
    homeRows.map((row) => ({ ...row, _resultHit: row.result == null ? null : (row.result === homeCode ? 1 : 0) })),
    awayRows.map((row) => ({ ...row, _resultHit: row.result == null ? null : (row.result === awayCode ? 1 : 0) })),
  ]), (row) => row._resultHit, config.currentShare);
  const resultEstimates = {
    home: outcomeEstimate('W', 'L'),
    draw: outcomeEstimate('D', 'D'),
  ...
  const binaryOutcomeEstimate = (homeCodes, awayCodes) => teamWeightedEmpiricalRate(unionRows([
    homeRows.map((row) => ({ ...row, _resultHit: row.result == null ? null : (homeCodes.includes(row.result) ? 1 : 0) })),
    awayRows.map((row) => ({ ...row, _resultHit: row.result == null ? null : (awayCodes.includes(row.result) ? 1 : 0) })),
  ]), (row) => row._resultHit, config.currentShare);
  const doubleChanceEstimates = {
    '1X': binaryOutcomeEstimate(['W', 'D'], ['L', 'D']),
    '12': binaryOutcomeEstimate(['W', 'L'], ['W', 'L']),
    X2: binaryOutcomeEstimate(['D', 'L'], ['D', 'W']),
  };
  if (Object.values(resultEstimates).some((estimate) => estimate.p != null)) {
    const distribution = normalizeDistribution(Object.fromEntries(
      Object.entries(resultEstimates).map(([key, estimate]) => [key, estimate.p || 0]),
    ));
    markets['1x2'] = {
      ...
      drawConf: round(teamWeightedReliability(resultEstimates.draw)),
      awayConf: round(teamWeightedReliability(resultEstimates.away)),
      doubleChanceEvidence: {
        n: Math.max(...Object.values(doubleChanceEstimates).map((estimate) => estimate.n || 0)),
        '1X': { conf: round(teamWeightedReliability(doubleChanceEstimates['1X'])) },
        '12': { conf: round(teamWeightedReliability(doubleChanceEstimates['12'])) },
        X2: { conf: round(teamWeightedReliability(doubleChanceEstimates.X2)) },
      },
  ...
  for (const fam of BT_GOALS) {
    const requiredLine = requiredReportLine(fam.key);
    const t = buildOU(totalRows, fam.total, config, requiredLine); if (t) markets[`${fam.key}_total`] = t;
    ...
    const h = buildOU(hRows, (r) => r._metric, config, requiredLine); if (h) markets[`${fam.key}_home`] = h;
    const a = buildOU(aRows, (r) => r._metric, config, requiredLine); if (a) markets[`${fam.key}_away`] = a;
  }
  for (const fam of BT_TEAMONLY) {   // solo home/away (sin total: no hay against por mitad)
    const requiredLine = requiredReportLine(fam.key);
    const h = buildOU(homeRows, fam.team, config, requiredLine); if (h) markets[`${fam.key}_home`] = h;
    const a = buildOU(awayRows, fam.team, config, requiredLine); if (a) markets[`${fam.key}_away`] = a;
  }
  ...
  // Ganador por mitad y "más córners/tiros/faltas": mismo patrón 1X2, sobre
  // el resultado W/D/L derivado de esa métrica específica en cada antecedente.
  const w1h = buildResultMarket(homeRows, awayRows, (r) => derivedResult(r.gf_1h, r.ga_1h), config);
  if (w1h) markets.winner_1h = w1h;
  const w2h = buildResultMarket(homeRows, awayRows, (r) => derivedResult(r.gf_2h, r.ga_2h), config);
  if (w2h) markets.winner_2h = w2h;
  const mCorners = buildResultMarket(homeRows, awayRows, (r) => derivedResult(r.corners_for, r.corners_against), config);
  if (mCorners) markets.most_corners = mCorners;
  const mShots = buildResultMarket(homeRows, awayRows, (r) => derivedResult(r.shots_for, r.shots_against), config);
  if (mShots) markets.most_shots = mShots;
  const mFouls = buildResultMarket(homeRows, awayRows, (r) => derivedResult(r.fouls_for, r.fouls_against), config);
  if (mFouls) markets.most_fouls = mFouls;
  const mCorners1h = buildResultMarket(homeRows, awayRows, (r) => derivedResult(r.corners_1h, r.corners_1h_against), config);
  if (mCorners1h) markets.most_corners_1h = mCorners1h;
  const mCorners2h = buildResultMarket(homeRows, awayRows, (r) => derivedResult(r.corners_2h, r.corners_2h_against), config);
  if (mCorners2h) markets.most_corners_2h = mCorners2h;
...
async function predict(pool, ctx, opts = {}) {
  ...
  const probableLineups = hasConfirmedLineups ? [] : await inferExpectedLineups(pool, ctx).catch(() => []);
  const activeLineups = hasConfirmedLineups ? opts.currentLineups : probableLineups;
  const hasProbableLineups = !hasConfirmedLineups && activeLineups.length > 0;
  const loadedConfig = opts.config || await loadEngineConfig(pool);
  const activeConfig = normalizeEngineConfig(loadedConfig);
  const horizon = hasConfirmedLineups ? 'confirmed' : (hasProbableLineups ? 'probable' : 'early');
  const horizonMetrics = loadedConfig?.metrics?.candidate?.horizons?.[horizon] || null;
  const validationFamilies = opts.validationFamilies
    || horizonMetrics?.families
    || loadedConfig?.metrics?.candidate?.families
    || null;
  const calibrationFamilies = opts.calibrationFamilies
    || horizonMetrics?.calibrationFamilies
    || loadedConfig?.metrics?.candidate?.calibrationFamilies
    || null;
  ...
  const res = await computeBaseMarkets(pool, ctx, {
    config: activeConfig,
    currentLineups: activeLineups.length ? activeLineups : null,
  });
  let h2hRows = null;
  ...
  const lineupApplied = activeLineups.length > 0 && Number(res.fixture?.lineupContext?.historicalRows || 0) > 0
    && Number(activeConfig.lineupBoost) > 1;
  return {
    fixture: res.fixture,
    modelVersion: Number(loadedConfig?.version || 0),
    markets: res.markets,
    h2hRows,
    pctx: null,
    calibrationFamilies,
    validationFamilies,
    applied: { h2h: h2hOn, player: lineupApplied, lineup: lineupApplied, horizon },
  };
}

module.exports = {
  computeBaseMarkets, fetchH2HRows, applyH2H, predict, tier, K,
  CURRENT_SEASON_SHARE, FOOTBALL_RELIABILITY_BASELINE, footballReliability,
  DEFAULT_ENGINE_CONFIG, normalizeEngineConfig, empiricalRate,
  teamWeightedEmpiricalRate, teamWeightedReliability,
  loadEngineConfig, resetEngineConfigCache, lineupRowsFromApi, annotateLineupSimilarity,
  inferExpectedLineups, restBucket,
};
