...
// Validación deliberadamente lineal: evita expresiones regulares con backtracking
// sobre entrada pública y aplica los límites prácticos de una dirección de email.
...
export async function signupUser(email, password, options = {}) {
  ...
  // M7 FIX: transacción REAL (antes eran 2 pgQuery sueltos → si el 2º fallaba
  // quedaba un user sin perfil). Un solo cliente con BEGIN/COMMIT/ROLLBACK.
  let user;
  ...
  try {
    ...
    console.error('[signupUser] tx fallo:', e.message);
    return { error: { code: 'SIGNUP_FAILED', message: 'No se pudo crear la cuenta' } };
  } finally {
    client.release();
  }
...
export async function getCurrentUser() {
  ...
  // M6 FIX: solo escribe si pasaron >5 min desde el último bump. Antes escribía
  // en CADA page-load (un write + WAL + actualización de índice por request).
  // El WHERE condicional convierte el 99% de las visitas en un no-op de escritura.
  pgQuery(
    "UPDATE public.auth_sessions SET last_seen = NOW() WHERE id = $1 AND (last_seen IS NULL OR last_seen < NOW() - interval '5 minutes')",
    [payload.sid],
  ).catch(() => {});
