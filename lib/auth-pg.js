...
export async function signupUser(email, password, options = {}) {
  ...
  const exists = await pgQuery('SELECT id FROM users WHERE LOWER(email) = $1', [normalizedEmail]);
  ...
  const userResult = await pgQuery(
    `INSERT INTO users (email, password_hash, email_verification_token, email_verification_expires, display_name)
     VALUES ($1, $2, $3, $4, $5)
...
export async function loginUser(email, password, options = {}) {
  ...
  if (!valid) {
    ...
    params.push(user.id);
    await pgQuery(`UPDATE users SET ${updates.join(', ')} WHERE id = $${params.length}`, params);
    return { error: { code: 'INVALID_CREDENTIALS', message: 'Email o password incorrectos' } };
  }
...
export async function logoutUser() {
  ...
  if (token) {
    ...
    if (payload?.sid) {
      await pgQuery('DELETE FROM auth_sessions WHERE id = $1', [payload.sid]).catch(() => {});
    }
  }
  clearSessionCookie();
  return true;
...
export async function getCurrentUser() {
  ...
  pgQuery('UPDATE auth_sessions SET last_seen = NOW() WHERE id = $1', [payload.sid]).catch(() => {});
...
async function createSession(userId, userAgent, ip) {
  ...
  const r = await pgQuery(
    `INSERT INTO auth_sessions (user_id, expires_at, user_agent, ip)
     VALUES ($1, $2, $3, $4)
     RETURNING id`,
...
export async function revokeSession(userId, sessionId) {
  await pgQuery('DELETE FROM auth_sessions WHERE id = $1 AND user_id = $2', [sessionId, userId]);
}
...
export async function createPasswordResetToken(email) {
  const normalizedEmail = normalizeEmail(email);
  const r = await pgQuery('SELECT id FROM users WHERE LOWER(email) = $1', [normalizedEmail]);
  if (r.rows.length === 0) {
...
export async function changePassword(userId, oldPassword, newPassword) {
  ...
  const r = await pgQuery('SELECT password_hash FROM users WHERE id = $1', [userId]);
  ...
  await pgQuery('UPDATE users SET password_hash = $1 WHERE id = $2', [newHash, userId]);
  return { ok: true };
}
...
export async function cleanupExpiredAuth() {
  const sess = await pgQuery('DELETE FROM auth_sessions WHERE expires_at < NOW()');
