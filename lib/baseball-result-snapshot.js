...
export function buildBaseballResultRow(game, existing = {}, boxscore = null, date, now = new Date()) {
  ...
  return {
    ...
    // Estado en vivo (cuenta y corredores en base) — solo tiene sentido
    // mientras el juego sigue en curso; un juego final lo pierde (no hay
    // "outs" ni "bases ocupadas" de un partido que ya terminó).
    outs: live ? numberOrNull(game.outs) : null,
    balls: live ? numberOrNull(game.balls) : null,
    strikes: live ? numberOrNull(game.strikes) : null,
    bases: live && game.bases ? game.bases : null,
...
export function baseballResultRowChanged(existing, next) {
  ...
  const keys = [
    ...
    'outs', 'balls', 'strikes', 'bases', 'finished_at',
  ];
