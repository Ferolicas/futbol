...
const STAT_ALIASES = {
  ...
  personalFouls: ['pfouls', 'personalfouls', 'fouls'],
  fieldGoalsMade: ['fgm', 'fieldgoalsmade', 'fieldgoalsmadefieldgoalsattempted'],
  fieldGoalsAttempted: ['fga', 'fieldgoalsattempted', 'fieldgoalsmadefieldgoalsattempted'],
  freeThrowsMade: ['ftm', 'freethrowsmade', 'freethrowsmadefreethrowsattempted'],
  freeThrowsAttempted: ['fta', 'freethrowsattempted', 'freethrowsmadefreethrowsattempted'],
  threePointersMade: ['threepointersmade', 'threepointsmade', 'threepointgoalstotal', 'tpm', '3pointsmade', 'threepoints', 'threepointfieldgoalsmadethreepointfieldgoalsattempted'],
  turnovers: ['turnoverstotal', 'turnovers', 'turnover'],
  totalYards: ['yardstotal', 'totalyards', 'yards'],
  passingYards: ['passingtotal', 'passingyards', 'passyards', 'netpassingyards'],
  ...
  penalties: ['penaltiestotal', 'penalties', 'totalpenaltiesyards'],
...
export function normalizeTeamStatistics(statistics) {
  ...
  for (const [canonical, aliases] of Object.entries(STAT_ALIASES)) {
    ...
    if (hit) {
      const compound = String(hit[1] ?? '').match(/^\s*(\d+)\s*[-/]\s*(\d+)/);
      const compoundCount = ['penalties', 'fieldGoalsMade', 'freeThrowsMade', 'threePointersMade'].includes(canonical)
        ? compound?.[1]
        : ['fieldGoalsAttempted', 'freeThrowsAttempted'].includes(canonical) ? compound?.[2] : null;
...
function factsForGame(game, details, side) {
  ...
  const ownHitsByInning = teamDetail?.statistics?.hitsByInning || teamDetail?.stats?.hitsByInning;
  const opponentHitsByInning = opponentDetail?.statistics?.hitsByInning || opponentDetail?.stats?.hitsByInning;
  if (Array.isArray(ownHitsByInning)) stats.hitsByInning = ownHitsByInning.map(numberOrNull);
  if (Array.isArray(opponentHitsByInning)) opponentStats.hitsByInning = opponentHitsByInning.map(numberOrNull);
...
export async function persistSportGames(pool, sport, games) {
  ...
  const rows = games.map((game) => ({
    ...
    provider_fixture_id: String(game.providerFixtureId || game.id),
    competition_id: String(game.league?.id || ''),
    season: String(game.season || ''),
    kickoff: game.date,
    status: game.status?.short || 'NS',
    home_team_id: String(game.teams?.home?.id || ''),
    away_team_id: String(game.teams?.away?.id || ''),
    home_team: game.teams?.home?.name || null,
    away_team: game.teams?.away?.name || null,
    home_logo: game.teams?.home?.logo || game.teams?.home?.fallbackLogo || null,
    away_logo: game.teams?.away?.logo || game.teams?.away?.fallbackLogo || null,
    home_score: numberOrNull(game.scores?.home?.total),
    away_score: numberOrNull(game.scores?.away?.total),
    periods: game.periods || {},
    raw: game.raw || {},
    finalized_at: game.status?.isFinal ? new Date().toISOString() : null,
  }));
  const chunkSize = 500;
  for (let index = 0; index < rows.length; index += chunkSize) {
    ...
    await pool.query(
      ...
      SELECT x.fixture_id,x.provider,x.provider_fixture_id,x.competition_id,x.season,x.kickoff,x.status,
             x.home_team_id,x.away_team_id,x.home_team,x.away_team,x.home_logo,x.away_logo,
             x.home_score,x.away_score,x.periods,x.raw,x.finalized_at,now()
      FROM jsonb_to_recordset($1::jsonb) AS x(
        fixture_id text,provider text,provider_fixture_id text,competition_id text,season text,
        kickoff timestamptz,status text,home_team_id text,away_team_id text,home_team text,away_team text,
        home_logo text,away_logo text,home_score numeric,away_score numeric,periods jsonb,raw jsonb,finalized_at timestamptz
      )
...
// Ingesta masiva para backfills de ligas con miles de partidos (NCAA/MiLB).
// Conserva exactamente el mismo contrato de hechos que ingestFinalSportGame,
// pero evita abrir una transacción y hacer varios round-trips por partido.
export async function ingestFinalSportGames(pool, sport, games, options = {}) {
  const config = getMultisportConfig(sport);
  const finals = (games || []).filter((game) => game?.status?.isFinal
    && numberOrNull(game.scores?.home?.total) != null
    && numberOrNull(game.scores?.away?.total) != null);
  if (!finals.length) return { ingested: 0, fixtures: [] };
  if (!options.skipPersist) await persistSportGames(pool, config.key, finals);

  const statsTable = ident(`${config.tablePrefix}_engine_team_stats`);
  const matchesTable = ident(`${config.tablePrefix}_engine_matches`);
  const rows = finals.flatMap((game) => [
    factsForGame(game, null, 'home'),
    factsForGame(game, null, 'away'),
  ]).map((fact) => ({
    fixture_id: fact.fixtureId,
    kickoff: fact.kickoff,
    team_id: fact.teamId,
    opponent_id: fact.opponentId,
    competition_id: fact.competitionId,
    season: fact.season,
    is_home: fact.isHome,
    score_for: fact.scoreFor,
    score_against: fact.scoreAgainst,
    period_scores: fact.periodScores,
    period_scores_against: fact.periodScoresAgainst,
    stats: fact.stats,
    result: fact.result,
  }));
  const chunkSize = Math.max(50, Math.min(1_000, Number(options.chunkSize) || 500));
  for (let index = 0; index < rows.length; index += chunkSize) {
    const chunk = rows.slice(index, index + chunkSize);
    await pool.query(
      `INSERT INTO ${statsTable}(
      ...
      SELECT x.fixture_id,x.kickoff,x.team_id,x.opponent_id,x.competition_id,x.season,x.is_home,
             x.score_for,x.score_against,x.period_scores,x.period_scores_against,x.stats,x.result,now()
      FROM jsonb_to_recordset($1::jsonb) AS x(
        fixture_id text,kickoff timestamptz,team_id text,opponent_id text,
        competition_id text,season text,is_home boolean,score_for numeric,score_against numeric,
        period_scores jsonb,period_scores_against jsonb,stats jsonb,result text
      )
      ON CONFLICT(fixture_id,team_id) DO UPDATE SET
        kickoff=EXCLUDED.kickoff,opponent_id=EXCLUDED.opponent_id,competition_id=EXCLUDED.competition_id,
        season=EXCLUDED.season,is_home=EXCLUDED.is_home,score_for=EXCLUDED.score_for,
        score_against=EXCLUDED.score_against,period_scores=EXCLUDED.period_scores,
        period_scores_against=EXCLUDED.period_scores_against,
        stats=CASE
          WHEN COALESCE((${statsTable}.stats->>'_detailsAvailable')::boolean,FALSE)
            THEN ${statsTable}.stats
          ELSE EXCLUDED.stats
        END,
        result=EXCLUDED.result,updated_at=now()`,
      [JSON.stringify(chunk)],
  ...
  const fixtureIds = finals.map((game) => String(game.id));
  await pool.query(
    `UPDATE ${matchesTable}
  ...
  return { ingested: finals.length, fixtures: fixtureIds };
}
...
export async function saveSportAnalysis(pool, sport, game, payload) {
  ...
  await pool.query(
    ...
    [
      ...
      // La columna `odds` guarda el censo de casas vistas. Antes leía
      // `payload.odds?.raw`, clave que el objeto de cuotas nunca ha tenido
      // (se llama `rawBookmakers`), así que quedaba `[]` en el 100% de las
      // filas y no había forma de auditar de qué casa salió cada precio.
      JSON.stringify(payload.odds?.rawBookmakers || []), JSON.stringify(payload.odds || {}), JSON.stringify(payload.probabilities || {}),
