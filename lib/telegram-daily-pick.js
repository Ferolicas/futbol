 ...
 *
 * Ya no se arma una combinada: se publican todos los partidos que tengan al
 * menos una opción válida y, de cada uno, sus tres mejores opciones como
 * máximo. Así, un partido puede aportar una, dos o tres opciones al envío.
 *
...
/**
 * Devuelve todos los partidos publicables, cada uno con entre una y tres de sus
 * mejores opciones. Ningún partido válido desplaza a otro por un límite global.
 */
export function selectTelegramDailyPick(selections) {
  ...
  const matches = candidates.sort(compareMatches).map(match => ({
    ...match,
