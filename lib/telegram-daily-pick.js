 ...
 *
 * Ya no se arma una combinada: se publican hasta tres partidos y, de cada uno,
 * sus tres mejores opciones. Un partido solo entra si reúne al menos tres
 * opciones que cumplan probabilidad y fiabilidad mínimas.
 *
...
function compareMatches(a, b) {
  ...
  for (let index = 0; index < OPTIONS_PER_MATCH; index += 1) {
...
/**
 * Devuelve hasta tres partidos publicables, cada uno con sus tres mejores
 * opciones. Si solo hay dos partidos con tres opciones válidas, devuelve dos;
 * si solo hay uno, devuelve uno; si ninguno reúne tres, devuelve vacío.
 */
export function selectTelegramDailyPick(selections) {
  ...
  for (const options of byFixture.values()) {
    if (options.length < OPTIONS_PER_MATCH) continue;
    const best = [...options].sort(compareOptions).slice(0, OPTIONS_PER_MATCH);
...
export const TELEGRAM_DAILY_PICK_RULES = Object.freeze({
  ...
  optionsPerMatch: OPTIONS_PER_MATCH,
