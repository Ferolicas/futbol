...
// endpoint que fixtures) y se filtra a los juegos en curso: ni por empezar
// (NS) ni en estado terminal. Fecha en hora Colombia, consistente con
// baseball-fixtures/analyze.
const BASEBALL_NON_LIVE_STATUS = new Set([
  'NS', 'FT', 'AOT', 'POST', 'CANC', 'INTR', 'ABD', 'PST', 'TBD',
]);
...
export async function getBaseballLiveGames() {
  ...
  return (response || []).filter(g =>
    ...
    g.status?.short && !BASEBALL_NON_LIVE_STATUS.has(g.status.short)
  );
}
