...
export async function getBaseballFixturesByDate(date, { forceApi = false } = {}) {
  ...
  // Layer 3: API
  const { response } = await apiCall(`/games?date=${date}&timezone=America/Bogota`);
...
// ===================== LIVE GAMES =====================
// La API de baseball NO soporta `?live=all` (eso es de la API de futbol y
// devuelve "The Live field do not exist"). Se consulta /games?date= (mismo
// endpoint que fixtures) y se filtran los juegos cuyo marcador hay que
// SINCRONIZAR. Fecha en hora Colombia, consistente con baseball-fixtures/analyze.
//
// ⚠ BUG corregido: antes este filtro excluía FT/AOT, así que cuando un partido
// terminaba DESAPARECÍA de la lista y el job live nunca escribía su marcador
// final en baseball_match_results — el front se quedaba mostrando el último
// parcial (a menudo 0-0) "al finalizar". Ahora FT/AOT SÍ se incluyen: son
// partidos terminados CON marcador real, y el live los devuelve una última vez
// para persistir el resultado final. Solo se excluyen los estados sin marcador
// util: no empezados (NS/TBD) o no jugados (postpuesto/cancelado/abandonado).
const BASEBALL_SKIP_STATUS = new Set([
  'NS', 'TBD', 'PST', 'POST', 'CANC', 'INTR', 'ABD',
]);
...
export async function getBaseballLiveGames() {
  ...
  const { response } = await apiCall(`/games?date=${date}&timezone=America/Bogota`);
  return (response || []).filter(g =>
    BASEBALL_LEAGUE_IDS.includes(g.league?.id) &&
    g.status?.short && !BASEBALL_SKIP_STATUS.has(g.status.short)
  );
}

// ===================== FIXTURE PLAYERS — bloque E/F (stub) =====================
//
// api-baseball v1 NO expone /fixtures/players (sí lo hace api-football).
// Esta funcion existe para mantener la interfaz limpia: el worker la
// llama, ella retorna null, y el modelo cae a fallback (team-level
// pitching) sin error.
//
// Cuando se integre MLB Stats API (gratuita, sin key):
//   1. fetch a `https://statsapi.mlb.com/api/v1/game/{gamePk}/feed/live`
//   2. extraer probablePitchers de gameData
//   3. fetch a `/api/v1/people/{pitcherId}/stats?stats=season&season={year}`
//   4. devolver { game, probablePitchers: { home, away }, players: { home: [], away: [] } }
export async function getBaseballFixturePlayers(_fixtureId) {
  return null;
}

// ===================== PLAYER STATS — bloque E/F (stub) =====================
// Para cuando se integre MLB Stats API. Devolver season stats del player.
export async function getBaseballPlayerStats(_playerId, _season) {
  return null;
}
