...
/**
 * Segmento acumulativo que debe respaldar una probabilidad concreta. Mantener
 * esta frontera en un solo sitio evita que entrenamiento y serving comparen
 * una cifra del 74% contra una cohorte distinta (por ejemplo, >=80%).
 */
function probabilityValidationBand(value) {
  const probability = clampProbability(value);
  if (probability == null) return null;
  if (probability >= 0.95) return 'elite95';
  if (probability >= 0.90) return 'daily90';
  if (probability >= 0.80) return 'high';
  if (probability >= 0.70) return 'selectable70';
  return 'all';
}
...
// Solo probabilidad, fiabilidad y cuota real bloquean una recomendación — las
// tres surgen exclusivamente del cruce de los dos equipos/jugadores del
// enfrentamiento (H2H, historial propio, alineación). El ledger de
// calibración general (validation.eligible, agregado entre TODOS los
// equipos) y los filtros económicos (EV, market edge) quedan como datos
// informativos en el resultado — nunca ocultan una recomendación real. Un
// filtro externo a los dos equipos jamás debe decidir si se muestra algo que
// el motor, con toda su evidencia específica, ya determinó que cumple.
function recommendationDecision(input, options = {}) {
  ...
  // validation/expectedValue/marketEdge se calculan y devuelven siempre (para
  // /rendimiento y para que el apostador los vea), pero ninguno entra en
  // `reasons` — no pueden ocultar una recomendación real.
