...
function recommendationDecision(input, options = {}) {
  ...
  const enforceExpectedValue = options.enforceExpectedValue !== false;
  const enforceMarketEdge = options.enforceMarketEdge !== false;
  // Por defecto SIEMPRE exige calibración del ledger (validation.eligible).
  // Solo multideporte, y solo durante su ventana de arranque sin historial
  // liquidado, pasa enforceValidation:false — fútbol nunca lo toca.
  const enforceValidation = options.enforceValidation !== false;
  const reasons = [];
  ...
  if (enforceValidation && !validation.eligible) reasons.push(validation.status);
  if (odd == null || odd <= 1) reasons.push('odds');
  if (enforceExpectedValue
      && (expectedValue == null || expectedValue + EPSILON < minimumExpectedValue)) reasons.push('expected-value');
  if (enforceMarketEdge
      && marketFairProbability != null
      && marketEdge + EPSILON < minimumMarketEdge) reasons.push('market-edge');
  return {
