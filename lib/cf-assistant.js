...
export async function searchRecommendations({ sport, date, minProbability = 0, marketNameLike = null, limit = 30, paidAccess }, pool = pgPool) {
  ...
  const nameFilter = marketNameLike ? String(marketNameLike).trim().slice(0, 80) : null;
  ...
  const { rows } = await pool.query(
    `SELECT * FROM (
       ...
       WHERE r.kickoff::date=$1 AND ($2::text IS NULL OR r.sport=$2)
         ...
         AND ($4::text IS NULL OR o.output->>'name' ILIKE '%'||$4||'%')
       ORDER BY r.sport, r.fixture_id, o.market_key, r.predicted_at DESC
     ) latest
     ORDER BY probability_raw DESC LIMIT $5`,
    [safeDate, normalizedSport, minProb, nameFilter, safeLimit],
  );
  if (!rows.length) {
    return {
      exists: false, date: safeDate, filter: { minProbability: Math.round(minProb * 100), marketNameLike: nameFilter },
...
export const CF_ASSISTANT_TOOLS = [{
  ...
  type: 'function', function: {
    ...
    parameters: { type: 'object', additionalProperties: false, properties: {
      ...
      marketNameLike: { type: ['string', 'null'], description: 'Texto libre a buscar en el nombre del mercado, ej. "2.5 goles", "córners", "1ª parte". null si no hay filtro de mercado.' },
