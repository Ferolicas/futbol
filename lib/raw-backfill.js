...
const { footballApiRequest, footballApiPath, payloadQuality } = require('./football-api-client.cjs');
let isYouthTeam;
...
function makeApi(counter) {
  ...
  return async function apiGet(path) {
    counter.calls++;
    const result = await footballApiRequest(path, { apiKey: key, timeoutMs: 20_000, retries: 2 });
    return result.payload; // payload CRUDO completo (response + paging + parameters)
  };
}

async function recordCaptureFailure(pool, { endpoint, refId, subKey = '', error, empty = false }) {
  try {
    await pool.query(
      `INSERT INTO api_capture_failures (endpoint,ref_id,sub_key,attempts,last_error,last_attempt_at,next_retry_at,status)
       VALUES ($1,$2,$3,1,$4,NOW(),NOW()+INTERVAL '24 hours',$5)
       ON CONFLICT (endpoint,ref_id,sub_key) DO UPDATE SET
         attempts=api_capture_failures.attempts+1,last_error=EXCLUDED.last_error,last_attempt_at=NOW(),
         next_retry_at=NOW()+INTERVAL '24 hours',status=EXCLUDED.status,resolved_at=NULL`,
      [endpoint, refId, subKey, String(error || (empty ? 'respuesta vacía' : 'error')).slice(0, 500), empty ? 'empty' : 'retry'],
    );
  } catch { /* migración aún no aplicada: nunca convertir logging en fallo de captura */ }
}

async function clearCaptureFailure(pool, endpoint, refId, subKey = '') {
  try {
    await pool.query(
...
// Guarda solo payloads válidos. Una respuesta vacía jamás reemplaza datos que
// ya existen; un body con error/rate-limit no entra en la tabla.
async function saveValue(pool, { endpoint, refType, refId, season, subKey = '', payload }) {
  const nextQuality = payloadQuality(payload);
  if (nextQuality === 0) return { saved: false, invalid: true };
  const { rows } = await pool.query(
    `SELECT payload FROM raw_api_payloads WHERE endpoint=$1 AND ref_id=$2 AND sub_key=$3`,
    [endpoint, refId, subKey],
  );
  const oldQuality = rows.length ? payloadQuality(rows[0].payload) : 0;
  if (oldQuality === 2 && nextQuality < 2) return { saved: false, preserved: true };
  await pool.query(
  ...
  return { saved: true, empty: nextQuality === 1 };
}

// Captura si falta dato útil. Los vacíos válidos se reintentan tras 24h; los
// payloads con datos se saltan. Así una ausencia temporal puede curarse.
async function captureOnce(pool, apiGet, counter, { endpoint, refType, refId, season, subKey = '', path }) {
  const { rows } = await pool.query(
    `SELECT payload, fetched_at FROM raw_api_payloads WHERE endpoint=$1 AND ref_id=$2 AND sub_key=$3`,
    [endpoint, refId, subKey]
  );
  const existingQuality = rows.length ? payloadQuality(rows[0].payload) : 0;
  const ageMs = rows[0]?.fetched_at ? Date.now() - new Date(rows[0].fetched_at).getTime() : Infinity;
  if (existingQuality === 2 || (existingQuality === 1 && ageMs < 24 * 3600 * 1000)) { counter.skipped++; return; }
  let payload;
  try { payload = await apiGet(path); }
  catch (error) {
    counter.failed = (counter.failed || 0) + 1;
    await recordCaptureFailure(pool, { endpoint, refId, subKey, error: error?.message || error });
    return;
  }
  const quality = payloadQuality(payload);
  if (quality === 0) {
    counter.failed = (counter.failed || 0) + 1;
    await recordCaptureFailure(pool, { endpoint, refId, subKey, error: 'payload inválido' });
    return;
  }
  const saved = await saveValue(pool, { endpoint, refType, refId, season, subKey, payload });
  if (saved.saved) counter.saved++;
  if (saved.preserved) counter.preserved = (counter.preserved || 0) + 1;
  if (quality === 1) {
    counter.empty = (counter.empty || 0) + 1;
    await recordCaptureFailure(pool, { endpoint, refId, subKey, empty: true });
  } else {
    await clearCaptureFailure(pool, endpoint, refId, subKey);
  }
}
...
//
// Idempotente: salta lo ya capturado. H2H ya existe en opponent_id, por eso
// captureH2H es opt-in y solo se usa para auditorías históricas específicas.
// ────────────────────────────────────────────────────────────────────────
...
// Consume el ledger durable de vacíos/errores. Se ejecuta cada noche con un
// límite acotado; lo que no quepa queda para la siguiente corrida.
async function retryDueCaptureFailures(opts = {}) {
  const { pool: extPool = null, limit = 300, concurrency = 3 } = opts;
  const pool = extPool || makePool();
  const ownPool = !extPool;
  const counter = { calls: 0, due: 0, resolved: 0, empty: 0, failed: 0, unroutable: 0 };
  const resolvedFixtureIds = new Set();
  try {
    const { rows } = await pool.query(
      `SELECT f.endpoint,f.ref_id,f.sub_key,f.metadata->>'season' AS metadata_season,
              r.ref_type,r.season AS raw_season
       FROM api_capture_failures f
       LEFT JOIN raw_api_payloads r
         ON r.endpoint=f.endpoint AND r.ref_id=f.ref_id AND r.sub_key=f.sub_key
       WHERE f.resolved_at IS NULL AND f.status IN ('retry','empty')
         AND COALESCE(f.next_retry_at,NOW()) <= NOW()
       ORDER BY COALESCE(f.next_retry_at,f.last_attempt_at) ASC
       LIMIT $1`, [Number(limit)]);
    counter.due = rows.length;
    const apiGet = makeApi(counter);
    await mapPool(rows, Math.max(1, Math.min(5, Number(concurrency))), async (row) => {
      const seasonFromKey = String(row.sub_key || '').match(/^(?:s|team):(\d+)/)?.[1];
      const seasonRaw = row.metadata_season ?? row.raw_season ?? seasonFromKey;
      const season = seasonRaw == null ? null : Number(seasonRaw);
      const path = footballApiPath(row.endpoint, Number(row.ref_id), row.sub_key || '', season);
      if (!path) { counter.unroutable++; return; }
      try {
        const payload = await apiGet(path);
        const quality = payloadQuality(payload);
        if (quality === 0) throw new Error('payload inválido');
        await saveValue(pool, {
          endpoint: row.endpoint, refType: row.ref_type || (String(row.sub_key || '').startsWith('fx:') ? 'fixture' : 'retry'),
          refId: Number(row.ref_id), season, subKey: row.sub_key || '', payload,
        });
        if (quality === 2) {
          await clearCaptureFailure(pool, row.endpoint, row.ref_id, row.sub_key || '');
          if (row.ref_type === 'fixture' || String(row.sub_key || '').startsWith('fx:') || String(row.endpoint).startsWith('fixtures/')) {
            resolvedFixtureIds.add(Number(row.ref_id));
          }
          counter.resolved++;
        }
        else { await recordCaptureFailure(pool, { endpoint: row.endpoint, refId: row.ref_id, subKey: row.sub_key || '', empty: true }); counter.empty++; }
      } catch (error) {
        await recordCaptureFailure(pool, { endpoint: row.endpoint, refId: row.ref_id, subKey: row.sub_key || '', error: error?.message || error });
        counter.failed++;
      }
    });
    counter.fixtureIds = [...resolvedFixtureIds];
    return counter;
  } finally {
    if (ownPool) await pool.end();
  }
...
module.exports = { runRawBackfill, runRawBackfillLeagues, getTeamsForHalf, SELECCIONES_LEAGUES, captureFinalizedFixturesRaw, retryDueCaptureFailures };
