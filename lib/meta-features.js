...
// Hándicap asiático (medias líneas, sin push). ah_home_{key} / ah_away_{key}.
const AH_LINES = [-3.5, -2.5, -1.5, -0.5, 0.5, 1.5, 2.5, 3.5];
const ahKey = (L) => (L < 0 ? 'm' : 'p') + String(Math.abs(L)).replace('.', '_');
...
function buildMarketDefs() {
  ...
  for (const L of AH_LINES) {
    const k = ahKey(L);
    const gateG = a => a?.goals?.home != null && a?.goals?.away != null;
    defs[`ah_home_${k}`] = { baseProb: nullProb, setProb: noop, outcome: a => gateG(a) && (a.goals.home + L) > a.goals.away, gate: gateG, family: 'asian_handicap', side: 'home', line: L };
    defs[`ah_away_${k}`] = { baseProb: nullProb, setProb: noop, outcome: a => gateG(a) && (a.goals.away + L) > a.goals.home, gate: gateG, family: 'asian_handicap', side: 'away', line: L };
  }
