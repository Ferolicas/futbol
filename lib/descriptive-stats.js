...
export function calculateGoalTimingProbabilities(goalTimingData) {
  ...
  function computeTeamTiming(teamData) {
    ...
    return periods.map(period => {
      ...
      // Cuenta PARTIDOS con al menos un gol en el tramo, no cantidad de goles.
      // Así dos goles en un mismo encuentro siguen siendo una sola ocurrencia.
      const legacyGoals = periodData
        ? (periodData.scored || 0) + (periodData.conceded || 0)
        : (teamData[period] || 0);
      const count = periodData?.matchesWithGoal ?? Math.min(n, legacyGoals);
      const prob = clamp(Math.round(((count / n) * 100 + Number.EPSILON) * 100) / 100);
  ...
  // Frecuencia conjunta directa: partidos de ambos equipos con al menos un gol
  // en el tramo / todos sus partidos registrados. No se asume independencia.
  const combined = periods.map((period, i) => {
    const homeN = Number(goalTimingData?.home?.totalMatches || 0);
    const awayN = Number(goalTimingData?.away?.totalMatches || 0);
    const totalN = homeN + awayN;
    const homeHits = Number(goalTimingData?.home?.periods?.[period]?.matchesWithGoal
      ?? Math.min(homeN, (goalTimingData?.home?.periods?.[period]?.scored || 0) + (goalTimingData?.home?.periods?.[period]?.conceded || 0)));
    const awayHits = Number(goalTimingData?.away?.periods?.[period]?.matchesWithGoal
      ?? Math.min(awayN, (goalTimingData?.away?.periods?.[period]?.scored || 0) + (goalTimingData?.away?.periods?.[period]?.conceded || 0)));
    const pC = totalN
      ? clamp(Math.round((((homeHits + awayHits) / totalN) * 100 + Number.EPSILON) * 100) / 100)
      : 0;
