...
export async function activatePaidAccess({
  ...
  try {
    ...
    // Inicio del plan: distinto de "último pago" (que se pisa en cada
    // renovación). Se reinicia solo en una activación REAL — el usuario no
    // venía activo, o cambió de plan — nunca en una simple renovación del
    // mismo plan, para que "Inicio del plan" no salte cada vez que se cobra.
    const isNewActivation = !['active', 'trialing'].includes(current.subscription_status)
    ...
    profile = (await client.query(
      `UPDATE public.user_profiles SET
         ...
         plan_started_at = CASE WHEN $12 THEN NOW() ELSE COALESCE(plan_started_at, NOW()) END,
