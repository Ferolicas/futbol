...
import { canonicalJsonBytes, recommendationSealPayload, sha512Hex } from './prediction-seal-core.js';
...
// `combinada.selections` (≥80%) es exclusivamente la Recomendación estadística
// publicada. `combinada.selectable` (≥70%) es el acordeón que MEZCLA esa misma
// Recomendación estadística con el Dato estadístico (70–79%, `recommended:
// false` en cada ítem — ver buildModelCombinada). Usar `selectable` sin
// filtrar por `.recommended===true` marcaba como recomendación cualquier cosa
// con solo aparecer ahí, contando también los datos estadísticos.
function recommendationById(analysis) {
  const selections = analysis?.combinada?.selections;
  if (Array.isArray(selections) && selections.length) {
    return new Map(selections.map((item) => [String(item.id), item]));
  }
  const selectable = Array.isArray(analysis?.combinada?.selectable) ? analysis.combinada.selectable : [];
  return new Map(selectable.filter((item) => item?.recommended === true).map((item) => [String(item.id), item]));
}
...
function footballOutputs(analysis) {
  ...
  const teamOutputs = Object.entries(analysis?._scored || {}).map(([key, row]) => {
    ...
    return {
      ...
      // `row` es la entrada cruda de _scored (sin nombre legible). El nombre
      // en español ya existe en el ítem de recomendación (buildModelCombinada
      // lo calcula con marketLabel); si no hay recomendación, se calcula igual
      // aquí para que ningún consumidor (p.ej. /rendimiento) tenga que mostrar
      // la clave interna cruda (ej. "ah_home_p1").
      output: {
        ...(row || {}),
        name: recommendation?.name || marketLabel(key, { home: analysis?.homeTeam, away: analysis?.awayTeam }),
      },
    };
  });
  const playerOutputs = [];
  ...
  try {
    ...
    const inserted = await client.query(
      ...
      [sport, fixtureId, horizon, predictedAt, kickoff, dataCutoff,
        ...
        JSON.stringify({
          source: input?.source || 'analysis-cache', engine: meta.engine || null,
          league: input?.game?.league?.name || analysis?.league || input?.payload?.league_name || null,
          homeTeam: input?.game?.teams?.home?.name || analysis?.homeTeam || input?.payload?.home_team || null,
          awayTeam: input?.game?.teams?.away?.name || analysis?.awayTeam || input?.payload?.away_team || null,
        })],
    );
    ...
    for (const output of outputs) {
      const marketRow = await client.query(
        `WITH inserted AS (INSERT INTO prediction_market_outputs(
         ...
         ON CONFLICT(run_id,market_key) DO NOTHING RETURNING id)
         SELECT id FROM inserted UNION ALL
         SELECT id FROM prediction_market_outputs WHERE run_id=$1 AND market_key=$2 LIMIT 1`,
      ...
      if (output.recommended && marketRow.rows[0]?.id) {
        const canonical = canonicalJsonBytes(recommendationSealPayload({
          run: {
            sport, fixtureId, predictedAt, kickoff, dataCutoff,
            modelVersion: snapshot.modelVersion, analysisVersion: input?.analysisVersion || analysis.cacheVersion || 0,
          },
          output,
        }));
        const proof = await client.query(
          `INSERT INTO prediction_seal_proofs(run_id,market_output_id,canonical_payload,content_hash,status)
           VALUES($1,$2,$3,$4,'pending') ON CONFLICT(market_output_id) DO UPDATE SET market_output_id=EXCLUDED.market_output_id
           RETURNING public_id,status`,
          [runId, marketRow.rows[0].id, canonical, sha512Hex(canonical)],
        );
        sealProofs[output.key] = { publicId: proof.rows[0].public_id, status: proof.rows[0].status };
      }
    }
    await client.query('COMMIT');
    return { runId, payloadHash, markets: outputs.length, sealProofs };
  } catch (error) {
