...
function init() {
  ...
  if (!pub || !priv) {
    // Log UNA SOLA VEZ (no inundar logs si se llama 200 veces por tick).
    if (!vapidMissingLogged) {
      console.error(
        '[PUSH] VAPID_PUBLIC_KEY/VAPID_PRIVATE_KEY no están en el env del proceso. ' +
        'web-push NO enviará nada. Confirma que el .env del worker tiene ambas claves ' +
        'y que el proceso se reinició tras añadirlas.',
      );
  ...
  initialized = true;
  console.log('[PUSH] VAPID inicializado OK (pub=...' + String(pub).slice(-8) + ')');
}
