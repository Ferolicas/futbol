...
function displayPercent(p) {
  ...
  // El dato conserva `raw`; solo 100% se presenta como 95% para no comunicar
  // garantía. 9/10 sigue mostrando 90 y 1/2 sigue mostrando 50.
  if (p >= 1) return 95;
  // Una frecuencia de 99.99% no puede redondearse visualmente a 100%; conserva
  // 99.9 sin convertirla artificialmente en el caso especial de 95%.
  return Math.min(99.9, Math.round(Math.max(0, p) * 1000) / 10);
}
