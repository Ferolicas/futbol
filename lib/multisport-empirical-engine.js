...
function genericPeriodScore(row, key, own = true) {
...
function buildGenericPeriodPrediction(homeRows, awayRows, key, odds, currentShare) {
  const own = row => genericPeriodScore(row, key, true);
  const against = row => genericPeriodScore(row, key, false);
...
export async function computeMultisportEmpiricalPrediction(pool, input) {
  ...
  if (config.key === 'baseball') {
    ...
    const periodKeys = [...new Set(['firstHalf', ...Object.keys(input.odds?.periods || {})])];
    for (const key of periodKeys) {
      periods[key] = buildGenericPeriodPrediction(homeRows, awayRows, key, input.odds?.periods?.[key], share);
    }
