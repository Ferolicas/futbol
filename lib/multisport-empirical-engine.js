...
export const MULTISPORT_CURRENT_SEASON_SHARE = 0.65;

export const DEFAULT_MULTISPORT_ENGINE_CONFIG = Object.freeze({
  currentShare: MULTISPORT_CURRENT_SEASON_SHARE,
  venueBoost: 1.18,
  opponentBoost: 1.22,
...
export function normalizeMultisportEngineConfig(raw = {}) {
  return {
    // Contrato común: el entrenamiento optimiza semejanza, pero nunca cambia
    // el reparto entre la temporada vigente y el histórico.
    currentShare: MULTISPORT_CURRENT_SEASON_SHARE,
...
function participantKey(row) {
  if (row?._side === 'home' || row?._side === 'away') return row._side;
  const teamId = String(row?.team_id || '').trim();
  return teamId || '__unknown__';
}

// Cada equipo/jugador obtiene primero su propia tasa. Los dos participantes
// del encuentro pesan 50/50 aunque uno tenga mucha más historia almacenada.
function participantWeightedEmpiricalRate(rows, predicate, currentShare) {
  ...
  const teams = [...groups.entries()].map(([participant, participantRows]) => ({
    participant,
    teamId: participant === 'home' || participant === 'away'
      ? String(participantRows[0]?.team_id || '') || null
      : (participant === '__unknown__' ? null : participant),
    ...empiricalRate(participantRows, predicate, currentShare),
  })).filter((rate) => rate.p != null);
  if (!teams.length) return empiricalRate(rows, predicate, currentShare);
  const pooled = empiricalRate(rows, predicate, currentShare);
  return {
    ...pooled,
    p: teams.reduce((sum, rate) => sum + rate.p, 0) / teams.length,
    currentShare,
    teams,
  };
}
...
function participantWeightedEmpiricalMean(rows, currentShare) {
  const groups = new Map();
  for (const row of rows) {
    const key = participantKey(row);
    if (!groups.has(key)) groups.set(key, []);
  ...
  const means = [...groups.values()]
    .map((participantRows) => empiricalMean(participantRows, currentShare))
    .filter((mean) => mean != null && Number.isFinite(mean));
  return means.length ? means.reduce((sum, mean) => sum + mean, 0) / means.length : null;
}
...
function audit(rate, currentShare) {
  const evidence = {
    ...
    n: rate.n,
    hits: rate.hits,
    currentShareContract: currentShare,
  ...
  if (Array.isArray(rate.teams)) {
    evidence.participantWeight = 1 / rate.teams.length;
    evidence.teams = rate.teams.map((team) => ({
      participant: team.participant,
      teamId: team.teamId,
      p: team.p,
      n: team.n,
      hits: team.hits,
      current: { p: team.current.p, n: team.current.n, hits: team.current.hits, weight: team.current.weight },
      historical: { p: team.historical.p, n: team.historical.n, hits: team.historical.hits, weight: team.historical.weight },
    }));
  }
  return evidence;
}
...
function probability(rows, predicate, currentShare) {
  const rate = participantWeightedEmpiricalRate(rows, predicate, currentShare);
...
export function displayFrequencyLadder(rows, currentShare, step = 1) {
  const values = rows.map(row => row._value).filter(value => value != null && Number.isFinite(Number(value))).map(Number);
  if (!values.length) return {};
  const max = Math.ceil(Math.max(...values) / step) * step;
  const result = {};
  // Integer scoring has no fixed maximum; the observed range sets the upper
  // bound. Keep strict > / < semantics (pushes are neither side).
  for (let line = step === 10 ? 10 : 0; line <= max; line += step) {
    result[line] = { over: probability(rows, value => value > line ? 1 : 0, currentShare), under: probability(rows, value => value < line ? 1 : 0, currentShare) };
  }
  return result;
}
...
function genericPeriodScore(row, key, own = true, periodMode = 'quarters') {
  const values = periodArray(row, own);
  if (periodMode === 'halves') {
    if (key === 'firstHalf' || key === 'halves') return sumPeriodValues(values, 1, 0);
    if (key === 'secondHalf') return sumPeriodValues(values, 1, 1);
    return null;
  }
  if (key === 'firstHalf' || key === 'halves') return sumPeriodValues(values, 2, 0);
  if (key === 'secondHalf') return sumPeriodValues(values, 2, 2);
  const quarter = String(key).match(/^quarter([1-4])$/)?.[1];
  return quarter ? sumPeriodValues(values, 1, Number(quarter) - 1) : null;
}

function buildGenericPeriodPrediction(homeRows, awayRows, key, odds, currentShare, periodMode) {
  const own = row => genericPeriodScore(row, key, true, periodMode);
  const against = row => genericPeriodScore(row, key, false, periodMode);
  const totalRows = dedupeObservations([
    homeRows.map(row => {
      const left = own(row), right = against(row);
      return { ...row, _value: left == null || right == null ? null : left + right };
    }),
    awayRows.map(row => {
      const left = own(row), right = against(row);
      return { ...row, _value: left == null || right == null ? null : left + right };
  ...
  const homeScore = project(homeRows, own, awayRows, against);
  const awayScore = project(awayRows, own, homeRows, against);
  ...
  return {
    kind: key,
    label: odds?.label || key,
    moneyline: {
      home: probability(margin, value => value > 0 ? 1 : 0, currentShare),
      away: probability(margin, value => value < 0 ? 1 : 0, currentShare),
      draw: probability(margin, value => value === 0 ? 1 : 0, currentShare),
    },
    displayFrequencies: { totals: displayFrequencyLadder(totalRows, currentShare, 10), home: displayFrequencyLadder(homeScore, currentShare, 10), away: displayFrequencyLadder(awayScore, currentShare, 10) },
    totals: ladder(totalRows, Object.keys(odds?.totals || {}).map(Number), [], currentShare),
    teamTotals: {
      home: ladder(homeScore, Object.keys(odds?.teamTotals?.home || {}).map(Number), [], currentShare),
      away: ladder(awayScore, Object.keys(odds?.teamTotals?.away || {}).map(Number), [], currentShare),
    },
    spreads: spreadLadder(margin, odds?.spreads || {}, [], currentShare),
    expected: {
      home: participantWeightedEmpiricalMean(homeScore, currentShare),
      away: participantWeightedEmpiricalMean(awayScore, currentShare),
      total: participantWeightedEmpiricalMean(totalRows, currentShare),
    },
    samples: { total: totalRows.length, home: homeScore.length, away: awayScore.length },
  };
}

function periodArray(row, own = true) {
  const value = own ? row.period_scores : row.period_scores_against;
  if (Array.isArray(value)) return value;
  return own ? (row.period_scores?.for || []) : (row.period_scores?.against || []);
}

function sumPeriodValues(values, count, start = 0) {
  if (!Array.isArray(values) || values.length < start + count) return null;
  const selected = values.slice(start, start + count).map(numberOrNull);
  if (selected.some((value) => value == null)) return null;
  return selected.reduce((sum, value) => sum + value, 0);
}

function baseballPeriodScore(row, key, own = true) {
  const values = periodArray(row, own);
  const inningMatch = String(key).match(/^inning(\d+)$/);
  if (inningMatch) return sumPeriodValues(values, 1, Number(inningMatch[1]) - 1);
  if (key === 'first4_5') {
    const belongsToHomeTeam = own ? row.is_home === true : row.is_home !== true;
    return sumPeriodValues(values, belongsToHomeTeam ? 4 : 5);
  }
  const count = { first3: 3, first5: 5, first7: 7 }[key];
  return count ? sumPeriodValues(values, count) : null;
}

function baseballPeriodStat(row, key, stat, own = true) {
  const directKey = own ? `${stat}ByInning` : `opponent${stat[0].toUpperCase()}${stat.slice(1)}ByInning`;
  const values = row?.stats?.[directKey];
...
function standardBaseballPeriodHitLines(key, scope) {
  if (/^inning\d+$/.test(key)) return scope === 'total' ? [0.5, 1.5, 2.5, 3.5] : [0.5, 1.5];
  if (key === 'first3') return scope === 'total' ? [2.5, 3.5, 4.5, 5.5] : [1.5, 2.5, 3.5];
  if (key === 'first5') return scope === 'total' ? [4.5, 5.5, 6.5, 7.5, 8.5] : [2.5, 3.5, 4.5];
  return [];
}

const BASEBALL_PERIOD_LABELS = Object.freeze({
  inning1: '1.ª entrada', inning2: '2.ª entrada', inning3: '3.ª entrada',
  inning4: '4.ª entrada', inning5: '5.ª entrada', inning6: '6.ª entrada',
  inning7: '7.ª entrada', inning8: '8.ª entrada', inning9: '9.ª entrada',
  first3: 'primeras 3 entradas', first4_5: 'primeras 4,5 entradas',
  first5: 'primeras 5 entradas', first7: 'primeras 7 entradas',
});

function invertSample(sample) {
  if (!sample || typeof sample !== 'object') return sample || null;
  const n = Number(sample.n);
  const hits = Number(sample.hits);
  return {
    ...sample,
    p: sample.p == null ? sample.p : 1 - Number(sample.p),
    rawProbability: sample.rawProbability == null ? sample.rawProbability : 1 - Number(sample.rawProbability),
    hits: Number.isFinite(n) && Number.isFinite(hits) ? n - hits : sample.hits,
  };
}

export function invertEmpiricalEvidence(evidence) {
  if (!evidence || typeof evidence !== 'object') return evidence || null;
  return {
    ...invertSample(evidence),
    current: invertSample(evidence.current),
    historical: invertSample(evidence.historical),
    teams: Array.isArray(evidence.teams) ? evidence.teams.map((team) => ({
      ...invertSample(team),
      current: invertSample(team.current),
      historical: invertSample(team.historical),
    })) : evidence.teams,
  };
}

function probabilityComplement(entry) {
  if (!entry || entry.rawProbability == null) return { probability: null, rawProbability: null, evidence: entry?.evidence || null };
  const rawProbability = 1 - Number(entry.rawProbability);
  return { probability: displayPercent(rawProbability), rawProbability, evidence: invertEmpiricalEvidence(entry.evidence) };
}

function spreadLadder(rows, offered = {}, required = [], currentShare) {
  const result = { home: {}, away: {} };
  for (const team of ['home', 'away']) {
    const lines = [...new Set([...Object.keys(offered?.[team] || {}), ...required].map(Number).filter(Number.isFinite))]
      .sort((left, right) => left - right);
    for (const line of lines) {
      result[team][line] = probability(
        rows,
        team === 'home'
          ? (value) => value + line > 0 ? 1 : 0
          : (value) => -value + line > 0 ? 1 : 0,
        currentShare,
      );
    }
...
function buildBaseballPeriodPrediction(homeRows, awayRows, key, odds, currentShare) {
  const own = (row) => baseballPeriodScore(row, key, true);
  const against = (row) => baseballPeriodScore(row, key, false);
  const totalRows = dedupeObservations([
    homeRows.map((row) => {
      const left = own(row), right = against(row);
      return { ...row, _value: left == null || right == null ? null : left + right };
  ...
  const offeredTotals = Object.keys(odds?.totals || {}).map(Number);
  const standardTotals = /^inning\d+$/.test(key) ? [0.5, 1.5] : [];
  const runYes = probability(totalRows, (value) => value > 0 ? 1 : 0, currentShare);
  const homeHits = project(
    homeRows, (row) => baseballPeriodStat(row, key, 'hits', true),
    awayRows, (row) => baseballPeriodStat(row, key, 'hits', false),
  );
  const awayHits = project(
    awayRows, (row) => baseballPeriodStat(row, key, 'hits', true),
    homeRows, (row) => baseballPeriodStat(row, key, 'hits', false),
  );
  const totalHits = dedupeObservations([
    homeRows.map((row) => {
      const left = baseballPeriodStat(row, key, 'hits', true);
      const right = baseballPeriodStat(row, key, 'hits', false);
      return { ...row, _value: left == null || right == null ? null : left + right };
    }),
    awayRows.map((row) => {
      const left = baseballPeriodStat(row, key, 'hits', true);
  ...
  const hitOdds = odds?.statistics?.hits || {};
  return {
    kind: key,
    label: odds?.label || BASEBALL_PERIOD_LABELS[key] || key,
    moneyline: {
      home: probability(margin, (value) => value > 0 ? 1 : 0, currentShare),
      away: probability(margin, (value) => value < 0 ? 1 : 0, currentShare),
      draw: probability(margin, (value) => value === 0 ? 1 : 0, currentShare),
    },
    displayFrequencies: { totals: displayFrequencyLadder(totalRows, currentShare), home: displayFrequencyLadder(homeScore, currentShare), away: displayFrequencyLadder(awayScore, currentShare) },
    totals: ladder(totalRows, offeredTotals, standardTotals, currentShare),
    teamTotals: {
      home: ladder(homeScore, Object.keys(odds?.teamTotals?.home || {}).map(Number), /^inning\d+$/.test(key) ? [0.5] : [], currentShare),
      away: ladder(awayScore, Object.keys(odds?.teamTotals?.away || {}).map(Number), /^inning\d+$/.test(key) ? [0.5] : [], currentShare),
    },
    spreads: spreadLadder(margin, odds?.spreads || {}, [0], currentShare),
    run: { yes: runYes, no: probabilityComplement(runYes) },
    statistics: {
      hits: {
        label: 'hits',
        total: ladder(totalHits, Object.keys(hitOdds.total || {}).map(Number), standardBaseballPeriodHitLines(key, 'total'), currentShare),
        home: ladder(homeHits, Object.keys(hitOdds.home || {}).map(Number), standardBaseballPeriodHitLines(key, 'team'), currentShare),
        away: ladder(awayHits, Object.keys(hitOdds.away || {}).map(Number), standardBaseballPeriodHitLines(key, 'team'), currentShare),
      },
    },
    expected: {
      home: participantWeightedEmpiricalMean(homeScore, currentShare),
    ...
    samples: {
      ...
      home: homeScore.length,
      away: awayScore.length,
    },
  };
}

function baseballScoringSide(row, mode) {
  const own = periodArray(row, true);
  const against = periodArray(row, false);
  const length = Math.max(own.length, against.length);
  const indexes = Array.from({ length }, (_, index) => index);
  if (mode === 'last') indexes.reverse();
  for (const index of indexes) {
    const actualHome = Number(row.is_home ? own[index] : against[index]) || 0;
    const actualAway = Number(row.is_home ? against[index] : own[index]) || 0;
    let actualSide = null;
    if (mode === 'first') actualSide = actualAway > 0 ? 'away' : (actualHome > 0 ? 'home' : null);
    else actualSide = actualHome > 0 ? 'home' : (actualAway > 0 ? 'away' : null);
    if (!actualSide) continue;
    const ownActualSide = row.is_home ? 'home' : 'away';
    return actualSide === ownActualSide ? 'own' : 'against';
  }
  return null;
}

function orientedBaseballGames(homeRows, awayRows) {
  const orient = (row, currentSide) => {
    const ownScore = numberOrNull(row.score_for), otherScore = numberOrNull(row.score_against);
    if (ownScore == null || otherScore == null) return { ...row, _value: null };
    const ownFirst5 = baseballPeriodScore(row, 'first5', true);
    const otherFirst5 = baseballPeriodScore(row, 'first5', false);
    const firstScorer = baseballScoringSide(row, 'first');
    const lastScorer = baseballScoringSide(row, 'last');
    const ownIsCurrentHome = currentSide === 'home';
    const mapScorer = (scorer) => scorer == null ? null
      : ((scorer === 'own') === ownIsCurrentHome ? 'home' : 'away');
    return {
      ...row,
      _value: 1,
      _homeScore: ownIsCurrentHome ? ownScore : otherScore,
      _awayScore: ownIsCurrentHome ? otherScore : ownScore,
      _homeFirst5: ownFirst5 == null || otherFirst5 == null ? null : (ownIsCurrentHome ? ownFirst5 : otherFirst5),
      _awayFirst5: ownFirst5 == null || otherFirst5 == null ? null : (ownIsCurrentHome ? otherFirst5 : ownFirst5),
      _firstScorer: mapScorer(firstScorer),
      _lastScorer: mapScorer(lastScorer),
      _extraInnings: Math.max(periodArray(row, true).length, periodArray(row, false).length) > 9,
    };
  };
  return dedupeObservations([
    homeRows.map((row) => orient(row, 'home')),
    awayRows.map((row) => orient(row, 'away')),
  ]);
}

function baseballOutcome(home, away) {
  return home > away ? 'home' : (away > home ? 'away' : 'draw');
}

function buildBaseballSpecials(homeRows, awayRows, offered = {}, currentShare) {
  const games = orientedBaseballGames(homeRows, awayRows);
  const rate = (predicate) => probability(games, (_value, row) => predicate(row), currentShare);
  const output = {
    totalParity: {
      odd: rate((row) => (row._homeScore + row._awayScore) % 2 === 1 ? 1 : 0),
      even: rate((row) => (row._homeScore + row._awayScore) % 2 === 0 ? 1 : 0),
    },
    teamParity: {
      home: {
        odd: rate((row) => row._homeScore % 2 === 1 ? 1 : 0),
        even: rate((row) => row._homeScore % 2 === 0 ? 1 : 0),
      },
      away: {
        odd: rate((row) => row._awayScore % 2 === 1 ? 1 : 0),
        even: rate((row) => row._awayScore % 2 === 0 ? 1 : 0),
      },
    },
    firstTeamScore: {
      home: rate((row) => row._firstScorer == null ? null : (row._firstScorer === 'home' ? 1 : 0)),
      away: rate((row) => row._firstScorer == null ? null : (row._firstScorer === 'away' ? 1 : 0)),
    },
    lastTeamScore: {
      home: rate((row) => row._lastScorer == null ? null : (row._lastScorer === 'home' ? 1 : 0)),
      away: rate((row) => row._lastScorer == null ? null : (row._lastScorer === 'away' ? 1 : 0)),
    },
    extraInnings: {
      yes: rate((row) => row._extraInnings ? 1 : 0),
      no: rate((row) => row._extraInnings ? 0 : 1),
    },
    highestScoring: {
      home: rate((row) => baseballOutcome(row._homeScore, row._awayScore) === 'home' ? 1 : 0),
      away: rate((row) => baseballOutcome(row._homeScore, row._awayScore) === 'away' ? 1 : 0),
      draw: rate((row) => baseballOutcome(row._homeScore, row._awayScore) === 'draw' ? 1 : 0),
    },
    correctScore: {},
    halfFull: {},
    resultTotals: {},
  };
  for (const [selection, price] of Object.entries(offered.correctScore || {})) {
    const homeScore = Number(price.homeScore), awayScore = Number(price.awayScore);
    if (!Number.isFinite(homeScore) || !Number.isFinite(awayScore)) continue;
    output.correctScore[selection] = rate((row) => row._homeScore === homeScore && row._awayScore === awayScore ? 1 : 0);
  }
  for (const [selection, price] of Object.entries(offered.halfFull || {})) {
    output.halfFull[selection] = rate((row) => {
      if (row._homeFirst5 == null || row._awayFirst5 == null) return null;
      return baseballOutcome(row._homeFirst5, row._awayFirst5) === price.first5
        && baseballOutcome(row._homeScore, row._awayScore) === price.final ? 1 : 0;
    });
  }
  for (const [selection, price] of Object.entries(offered.resultTotals || {})) {
    output.resultTotals[selection] = rate((row) => {
      const resultMatches = baseballOutcome(row._homeScore, row._awayScore) === price.outcome;
      const total = row._homeScore + row._awayScore;
      const totalMatches = price.side === 'over' ? total > Number(price.line) : total < Number(price.line);
      return resultMatches && totalMatches ? 1 : 0;
    });
  }
  return output;
}
...
function standardLines(sport, kind) {
  ...
  if (kind === 'hits') return [6.5, 8.5, 10.5, 18.5];
  return [];
}
...
export async function computeMultisportEmpiricalPrediction(pool, input) {
  ...
  const tie = probability(homeMargin, (value) => value === 0 ? 1 : 0, share);
  const draw = config.drawAllowed ? tie : null;
  const offeredTotals = Object.keys(input.odds?.totals || {}).map(Number);
  const offeredHomeTotals = Object.keys(input.odds?.teamTotals?.home || {}).map(Number);
  const offeredAwayTotals = Object.keys(input.odds?.teamTotals?.away || {}).map(Number);
  const totals = ladder(totalScore, offeredTotals, standardLines(config.key, 'totals'), share);
  const homeTotals = ladder(homeScore, offeredHomeTotals, standardLines(config.key, 'team'), share);
  const awayTotals = ladder(awayScore, offeredAwayTotals, standardLines(config.key, 'team'), share);

  let periods = {};
  let innings = {};
  let legacyPeriod = null;
  if (config.key === 'baseball') {
    const keys = [...new Set([
      'inning1', 'inning2', 'inning3', 'inning4', 'inning5', 'inning6', 'inning7', 'inning8', 'inning9',
      'first3', 'first4_5', 'first5', 'first7',
      ...Object.keys(input.odds?.periods || {}),
    ])];
    for (const key of keys) {
      const computed = buildBaseballPeriodPrediction(homeRows, awayRows, key, input.odds?.periods?.[key], share);
      periods[key] = computed;
      const inning = key.match(/^inning(\d+)$/)?.[1];
      if (inning) innings[inning] = computed;
    }
    legacyPeriod = periods.first5;
  } else {
    // NCAA Basketball se disputa en dos mitades; NBA y fútbol americano
    // entregan cuartos. Siempre calculamos todos los periodos reglamentarios,
    // aunque la casa no publique cuota, para alimentar el análisis completo.
    const periodMode = config.key === 'basketball' && String(fixture.league?.id) === '116'
      ? 'halves'
      : 'quarters';
    const canonicalPeriods = periodMode === 'halves'
      ? ['firstHalf', 'secondHalf']
      : ['firstHalf', 'secondHalf', 'quarter1', 'quarter2', 'quarter3', 'quarter4'];
    const periodKeys = [...new Set([...canonicalPeriods, ...Object.keys(input.odds?.periods || {})])];
    for (const key of periodKeys) {
      periods[key] = buildGenericPeriodPrediction(
        homeRows,
        awayRows,
        key,
        input.odds?.periods?.[key],
        share,
        periodMode,
      );
    }
    legacyPeriod = periods.firstHalf;
  }
  ...
  for (const [key, label] of statFamilies(config.key)) {
    ...
    if (home.length || away.length || total.length) {
      const offered = input.odds?.statistics?.[key] || {};
      const required = standardLines(config.key, key);
      statMarkets[key] = {
        label,
        home: ladder(home, Object.keys(offered.home || {}).map(Number), required, share),
        away: ladder(away, Object.keys(offered.away || {}).map(Number), required, share),
        total: ladder(total, Object.keys(offered.total || {}).map(Number), required, share),
        expected: {
          home: participantWeightedEmpiricalMean(home, share),
          away: participantWeightedEmpiricalMean(away, share),
          total: participantWeightedEmpiricalMean(total, share),
        },
      };
    }
  }
  ...
  const spread = 1.5;
  const allSpreads = spreadLadder(homeMargin, input.odds?.spreads || {}, [-spread, spread], share);
  const specialMarkets = config.key === 'baseball'
    ? buildBaseballSpecials(homeRows, awayRows, input.odds?.specials || {}, share)
    : {};
  const output = {
    sport: config.key,
    engine: {
    ...
    moneyline: {
      home: publicProbability(homeWin), away: publicProbability(awayWin),
      ...(draw ? { draw: publicProbability(draw) } : { push: publicProbability(tie) }),
    },
    displayFrequencies: {
      totals: displayFrequencyLadder(totalScore, share, config.key === 'baseball' ? 1 : 10),
      home: displayFrequencyLadder(homeScore, share, config.key === 'baseball' ? 1 : 10),
      away: displayFrequencyLadder(awayScore, share, config.key === 'baseball' ? 1 : 10),
    },
    totals: { lines: totals },
    ...
    expected: {
      home: participantWeightedEmpiricalMean(homeScore, share),
      away: participantWeightedEmpiricalMean(awayScore, share),
      total: participantWeightedEmpiricalMean(totalScore, share),
    },
  };
  output.bothScore.no = probabilityComplement(output.bothScore.yes);
  return output;
}
...
function bareProbabilityTree(value) {
  if (!value || typeof value !== 'object') return value ?? null;
  if ('probability' in value || 'rawProbability' in value) return bare(value);
  return Object.fromEntries(Object.entries(value).map(([key, child]) => [key, bareProbabilityTree(child)]));
}
...
function standardPlayerLines(category) {
  if (category === 'strikeouts') return [2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5];
  if (category === 'totalBases') return [0.5, 1.5, 2.5, 3.5];
  if (category === 'hits') return [0.5, 1.5, 2.5];
  return [0.5, 1.5];
}

function offeredPlayerLines(playerProps, category, playerName) {
  const player = playerProps?.[category]?.[normalizedPlayerName(playerName)];
  return Object.keys(player?.lines || {}).map(Number).filter(Number.isFinite);
}

export function buildEmpiricalPlayerProbabilities(playerHighlights, playerProps = null) {
  ...
  for (const [category, players] of Object.entries(playerHighlights)) {
    ...
    result[category] = players.map((player) => {
      ...
      const lines = [...new Set([
        ...dynamic,
        ...standardPlayerLines(category),
        ...offeredPlayerLines(playerProps, category, player.name),
      ])].sort((a, b) => a - b);
      for (const line of lines) {
        ...
        lineSides[line] = {
          over: { probability: displayPercent(raw), rawProbability: raw, evidence: evidence[line] },
          under: {
            probability: displayPercent(1 - raw), rawProbability: 1 - raw,
            evidence: { n: history.length, hits: history.length - hits, rawProbability: 1 - raw },
          },
        };
      }
      return {
...
export function toBaseballProbabilityShape(prediction, { playerHighlights = null, playerProbabilities, pitcherMatchup = null } = {}) {
  ...
  return {
    ...
    runLines: {
      home: Object.fromEntries(Object.entries(prediction.spreads?.home || {}).map(([line, value]) => [line, bare(value)])),
      away: Object.fromEntries(Object.entries(prediction.spreads?.away || {}).map(([line, value]) => [line, bare(value)])),
    },
    f5: {
      moneyline: {
    ...
    periods: Object.fromEntries(Object.entries(prediction.periods || {}).map(([key, period]) => [key, {
      kind: period.kind,
      label: period.label,
      moneyline: {
        home: bare(period.moneyline?.home), away: bare(period.moneyline?.away), tie: bare(period.moneyline?.draw),
      },
      totals: bareLadder(period.totals),
      teamTotals: { home: bareLadder(period.teamTotals?.home), away: bareLadder(period.teamTotals?.away) },
      run: { yes: bare(period.run?.yes), no: bare(period.run?.no) },
      statistics: Object.fromEntries(Object.entries(period.statistics || {}).map(([metric, values]) => [metric, {
        label: values.label,
        home: bareLadder(values.home), away: bareLadder(values.away), total: bareLadder(values.total),
      }])),
      expected: period.expected,
      samples: period.samples,
    }])),
    innings: Object.fromEntries(Object.entries(prediction.innings || {}).map(([key, period]) => [key, {
      label: period.label,
      totals: bareLadder(period.totals),
      teamTotals: { home: bareLadder(period.teamTotals?.home), away: bareLadder(period.teamTotals?.away) },
      run: { yes: bare(period.run?.yes), no: bare(period.run?.no) },
    ...
    statistics: Object.fromEntries(Object.entries(prediction.statistics || {}).map(([key, values]) => [key, {
      label: values.label,
      home: bareLadder(values.home), away: bareLadder(values.away), total: bareLadder(values.total),
    }])),
    specials: bareProbabilityTree(prediction.specials || {}),
...
export const multisportEngineInternals = {
  empiricalRate, participantWeightedEmpiricalRate, participantWeightedEmpiricalMean,
  displayPercent, dedupeObservations, project, ladder, percentile,
};
