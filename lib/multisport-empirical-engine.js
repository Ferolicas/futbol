...
export async function computeMultisportEmpiricalPrediction(pool, input) {
  ...
  if (config.key === 'baseball') {
    ...
    const periodCount = 2;
    const periodTotals = dedupeObservations([
      homeRows.map((row) => {
        const own = periodTotal(row, periodCount);
        const against = periodArray(row, false);
        const other = sumPeriodValues(against, periodCount);
        return { ...row, _value: own == null || other == null ? null : own + other };
      }),
      awayRows.map((row) => {
        const own = periodTotal(row, periodCount);
        const against = periodArray(row, false);
    ...
    const periodMargin = project(homeRows,
      (row) => {
        const own = periodTotal(row, periodCount), other = sumPeriodValues(periodArray(row, false), periodCount);
        return own == null || other == null ? null : own - other;
      },
      awayRows,
      (row) => {
        const own = periodTotal(row, periodCount), other = sumPeriodValues(periodArray(row, false), periodCount);
    ...
    const offeredPeriodTotals = Object.keys(input.odds?.periods?.[config.periodKind] || {}).map(Number);
    legacyPeriod = {
      ...
      totals: ladder(periodTotals, offeredPeriodTotals, [], share),
    };
    periods[config.periodKind] = legacyPeriod;
  }
