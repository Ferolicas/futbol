...
export async function computeMultisportEmpiricalPrediction(pool, input) {
  ...
  const offeredPeriodTotals = config.periodKind === 'first5'
    ? Object.keys(input.odds?.periods?.first5?.totals || {}).map(Number)
    : [];
  ...
  const output = {
    ...
    period: {
      ...
      totals: ladder(historicalPeriodTotals, offeredPeriodTotals, standardLines(config.key, 'period'), share),
    },
    bothScore: {
