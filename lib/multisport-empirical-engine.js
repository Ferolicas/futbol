...
function displayPercent(p) {
  ...
  // La frecuencia cruda se conserva en `rawProbability`; visualmente se limita
  // a 95% para no presentar 96–100% como una garantía. 9/10 sigue mostrando
  // 90 y 1/2 sigue mostrando 50.
  return Math.min(95, Math.round(Math.max(0, p) * 1000) / 10);
}

export function validationSupportsDisplayedProbability(entry, displayedProbability, floor = 0.8) {
  if (!entry) return false;
  const n = Number(entry.n ?? entry.samples ?? 0);
  const accuracy = Number(entry.hitRate ?? entry.accuracy ?? entry.rate);
  const shown = Number(displayedProbability);
  if (n <= 0 || !Number.isFinite(accuracy) || !Number.isFinite(shown)) return false;
  // No se altera la probabilidad calculada. Este es un gate independiente:
  // una línea solo puede recomendarse si esa familia, fuera de muestra, ha
  // acertado al menos el porcentaje exacto que se enseña al usuario.
  const target = Math.max(Number(floor) || 0, Math.min(0.95, shown / 100));
  return accuracy + 1e-12 >= target;
}
