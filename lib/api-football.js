...
// ===================== RATE LIMITER =====================
// API-Football per-minute caps depend on plan tier. The Mega 150k/day plan
// allows 900 req/min ≈ 15/s sustained, but bursts are tolerated thanks to the
// per-second smoothing the API uses internally.
//
...
async function computeRefereeFactor(refereeRaw) {
  ...
  try {
    ...
    if (!(globalAvg > 0)) return { factor: 1, refereeName: name, refereeMatches: matches, refereeAvg: refAvg, refereeYellows, refereeReds };
    ...
    return { factor, refereeName: name, refereeMatches: matches, refereeAvg: refAvg, refereeYellows, refereeReds };
  } catch (e) {
...
export async function analyzeMatch(fixture, { date: requestDate, force } = {}) {
  ...
  const modelCtx = await buildModelCtx(pgPool, {
    ...
    round: fixture.league.round, homeId, awayId,
  ...
  // ── Suficiencia de datos: count de model.team_match_stats (la MISMA fuente del motor) ──
  // <5 partidos por equipo ⇒ tasas empíricas por mercado = ruido → insufficient y SIN picks.
  const MIN_TEAM_HISTORY = 5;
  const homeFinished = descriptives.homeAvg.n;
  const awayFinished = descriptives.awayAvg.n;
  const dataQuality = (homeFinished >= MIN_TEAM_HISTORY && awayFinished >= MIN_TEAM_HISTORY) ? 'good' : 'insufficient';
  analysis.dataQuality = dataQuality;
  if (dataQuality === 'insufficient') {
    console.warn(`[ANALYSIS] crudo insuficiente fixture=${fixtureId}: homeFinished=${homeFinished} awayFinished=${awayFinished} (min=${MIN_TEAM_HISTORY}) → análisis sin recomendaciones`);
  }

  const pr = await predict(pgPool, modelCtx, {});           // núcleo → H2H(temp. actual) → jugador (flags por env)
  const scored = modelToScored(pr.markets);                  // mercados del motor → shape `scored`
  ...
  try {
    if (!Object.keys(playerMarkets).length && analysis.lineups?.available && Array.isArray(analysis.lineups.data)) {
      ...
      if (startXI.length) { playerMarkets = await buildPlayerMarkets(pgPool, startXI, { cutoff: new Date() }); analysis.playerMarkets = playerMarkets; }
    }
  } catch (e) { console.error(`[ANALYSIS] playerMarkets ${fixtureId}:`, e.message); }

  // 6º arg: analysis.cornerCardData — fuente fiable de qué categorías tienen muestra real
  // (veto). Sin historial suficiente NO se emiten recomendaciones: combinada vacía.
  const combinadaResult = dataQuality === 'insufficient'
    ? { selections: [], selectable: [], _funnel: { scored: Object.keys(scored).length, ge70: 0, ge70_sinCuota: 0, sinDatos: 0, teamSelectable: 0, playerSelectable: 0, selectable: 0, insufficientData: true }, combinedOdd: null, combinedProbability: 0, highRisk: false, hasRealOdds: false, threshold: 90, source: 'context-engine', insufficientData: true }
    : buildModelCombinada(scored, analysis.odds, teamNames, playerMarkets, calculatedProbabilities, analysis.cornerCardData);
...
function extractPlayerHighlights(homeFixtureIds, awayFixtureIds, matchPlayersMap, homeId, awayId, homeTeamName, awayTeamName) {
  ...
  const MIN_HISTORY = 8;                 // partidos minimos requeridos
