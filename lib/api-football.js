...
export async function analyzeMatch(fixture, { date: requestDate, force } = {}) {
  ...
  if (CONTEXT_ENGINE_ENABLED) {
    try {
      ...
      const { loadContextInputs, computeContext, scoreContext, persistFixtureContext, loadRuptureModels, loadAdverseModel, isKeyInjury } = await import('./context-engine');
      ...
      const adverseModel = mlEnabled ? await loadAdverseModel(pgPool) : null;
      const scored = scoreContext(computeContext(inputs), {
        ...
        mlModels, mlEnabled, adverseModel,
      });
