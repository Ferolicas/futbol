...
// API calls (stats, players, events for 10-20 fixtures). Without throttling,
// Promise.all fires them all at once → instant 429 flood.
const MAX_CONCURRENT = 6;
const MIN_DELAY_MS = 170; // ~6 req/s — stays under 10/s limit with headroom
...
async function apiCall(endpoint) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    // Acquire concurrency slot (max 6 in-flight)
    await _acquireSlot();
