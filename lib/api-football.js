...
// ===================== RATE LIMITER =====================
// API-Football allows ~10 requests/second. Each match analysis can trigger 40-70+
// API calls (stats, players, events for 10-20 fixtures).
//
// Two-layer protection:
// 1. _throttleChain: serial promise chain — each request waits MIN_DELAY_MS after
//    the previous one STARTS. No race condition possible (vs timestamp approach).
// 2. _activeRequests: caps in-flight connections to MAX_CONCURRENT.
//
// Tuned for the VPS worker: 110ms (~9 req/s) leaves ~10% headroom under the
// 10/s ceiling and the 429 retry handler below absorbs any overshoot.
const MAX_CONCURRENT = 10;
const MIN_DELAY_MS = 110; // ~9 req/s — within docs limit of 10/s
...
function extractOdds(oddsData) {
  ...
  const aggregateOdds = (betMatcher, keyFn) => {
    const result = {};
    for (const bk of bookmakers) {
      const bet = (bk.bets || []).find(betMatcher);
  ...
  // Match Winner (1X2)
  const mw = aggregateOdds(
    b => b.name === 'Match Winner',
    v => ({ Home: 'home', Draw: 'draw', Away: 'away' })[v.value] || null
  );
  if (mw) result.matchWinner = mw;

  // Goals Over/Under
  const ou = aggregateOdds(
    b => b.name === 'Goals Over/Under',
    v => v.value?.replace(/[^a-zA-Z0-9_-]/g, '_') || null
  );
  if (ou) result.overUnder = ou;

  // Both Teams Score
  const btts = aggregateOdds(
    b => b.name === 'Both Teams Score',
    v => v.value?.toLowerCase() || null
  );
  if (btts) result.btts = btts;

  // Corners Over/Under
  const corners = aggregateOdds(
    b => b.name === 'Total - Corners' || b.name === 'Corners Over Under' || b.name === 'Total Corners' || b.name?.toLowerCase().includes('corner'),
    v => v.value?.replace(/[^a-zA-Z0-9_-]/g, '_') || null
  );
  if (corners) result.corners = corners;

  // Cards Over/Under
  const cards = aggregateOdds(
    b => b.name === 'Total - Cards' || b.name === 'Cards Over Under' || b.name === 'Total Cards' || b.name?.toLowerCase().includes('card'),
    v => v.value?.replace(/[^a-zA-Z0-9_-]/g, '_') || null
  );
