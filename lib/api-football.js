...
function extractMatchStatsData(homeMatchStats, awayMatchStats, homeId, awayId) {
  ...
  // 'period' = '' | '_1h' | '_2h' y rellena los buckets de un equipo.
  const processMatch = (matchStats, teamId, sideBucket) => {
    ...
    for (const m of metrics) {
      ...
      sideBucket[m].for.push(tFor);
      sideBucket[m].against.push(tAg);
      sideBucket[m].total.push(tFor + tAg);
      ...
      sideBucket[m].for1H.push(forH1);
      sideBucket[m].for2H.push(forH2);
      sideBucket[m].against1H.push(agH1);
      sideBucket[m].against2H.push(agH2);
      sideBucket[m].total1H.push(forH1 + agH1);
      sideBucket[m].total2H.push(forH2 + agH2);
      if (synthFor || synthAg) {
        sideBucket[m].synth1H++;
        sideBucket[m].synth2H++;
      }
    }
  };

  // Procesar ambos equipos
  for (const matchStats of (homeMatchStats || [])) processMatch(matchStats, homeId, data);
  // Re-procesar pero asignando al bucket "away" cuando el contexto es away history
  const dataAway = {};
  for (const m of metrics) dataAway[m] = { home: makeBucket(), away: makeBucket() };
  for (const matchStats of (awayMatchStats || [])) processMatch(matchStats, awayId, dataAway);

  // Combinar: del home-history sale el bucket .home, del away-history sale el bucket .away
  const out = {};
  for (const m of metrics) {
    out[m] = {
      home: data[m].home,
      away: dataAway[m].away,
    };
  }
...
function extractPlayerHighlights(homeFixtureIds, awayFixtureIds, matchPlayersMap, homeId, awayId, homeTeamName, awayTeamName) {
  ...
  // Consistent assisters: ≥1 asistencia en 8+ de los últimos 10 partidos
  const assisters = allPlayers
    .filter(p => p.assists.length >= 8 && p.assists.filter(a => a >= 1).length >= 8)
  ...
  // Consistent total-shooters (incluye tiros fuera y bloqueados, mercado Player Shots Total):
  // ≥2 tiros en 8+ de los ultimos 10 partidos. Threshold mas estricto porque el
  // mercado tipico es Over 1.5-2.5 tiros.
  const shotsTotalists = allPlayers
    .filter(p => p.shotsTotal.length >= 8 && p.shotsTotal.filter(s => s >= 2).length >= 8)
...
const BET_NAMES = {
  ...
  // Renombrado: 'shots' → 'playerShots' (mas explicito; era player-level)
  playerShots: ['Player Shots on Target', 'Player Shots On Target', 'Shots On Target', 'Player Total Shots on Target', 'Player Shots on Goal',
                'Player Shots On Target Total', 'Player Shots Total', 'Home Player Shots', 'Away Player Shots',
...
function extractOdds(oddsData) {
  ...
  // Algunos mercados player vienen con value "PlayerName - 3.5" (linea por jugador).
  // Quitamos la parte ' - X.X' antes de normalizar para que el lookup por nombre
  // funcione igual que antes; perdemos la linea exacta (no se usa en este modelo
  // — el player market es "anytime" tipo, no exact-line).
  const playerKey = v => {
    let raw = (v.value || '').toString();
    raw = raw.replace(/\s*-\s*\d+(\.\d+)?\s*$/, '');
    const norm = normalizePlayerName(raw);
    return norm || null;
  };
  const scorer       = aggregateOdds('scorer',        playerKey);
  const playerShots  = aggregateOdds('playerShots',   playerKey);
  const playerFouls  = aggregateOdds('fouls',         playerKey);
  const playerBooked = aggregateOdds('booked',        playerKey);
  const playerAssist = aggregateOdds('playerAssists', playerKey);
  if (scorer || playerShots || playerFouls || playerBooked || playerAssist) {
    result.players = {};
    if (scorer)       result.players.scorer  = scorer;
    if (playerShots)  result.players.shots   = playerShots;
    if (playerFouls)  result.players.fouls   = playerFouls;
