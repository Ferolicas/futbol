...
async function apiCall(endpoint) {
  ...
  const result = await footballApiRequest(endpoint, { apiKey: key, timeoutMs: 30_000, retries: 2 });
...
export async function getFixtures(date, { forceApi } = {}) {
  if (forceApi) {
    ...
    const { response: all } = await apiCall(`/fixtures?date=${date}&timezone=America/Bogota`);
