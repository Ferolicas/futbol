...
export async function analyzeMatch(fixture, { date: requestDate, force } = {}) {
  ...
  // Motor de contexto = ÚNICA ruta de cálculo (Dixon-Coles purgado). Mercados =
  // contexto específico (prob_final) desde el crudo real; estadísticas descriptivas
  // = promedio GENERAL del equipo. La capa ML (ruptura + familias direccionales)
  // está gated por CONTEXT_ML_ENABLED (kill switch). Si el motor falla, propagamos
  // el error: NO hay fallback obsoleto que devolvería números incorrectos.
  const teamNames = { home: analysis.homeTeam, away: analysis.awayTeam, homeId, awayId };
  const { pgPool } = await import('./db');
  const { loadContextInputs, computeContext, scoreContext, persistFixtureContext, loadRuptureModels, loadFamilyModels, loadMarketBaseRates, isKeyInjury } = await import('./context-engine');
  const { buildProbabilitiesFromContext, buildTodayCtx, buildContextCombinada } = await import('./context-probabilities');
  const inputs = await loadContextInputs(pgPool, homeId, awayId);

  // ── Suficiencia de datos: FUENTE = el crudo que el motor REALMENTE usa ──
  // _counts.{home,away}Finished = partidos finalizados por equipo cargados de
  // raw_api_payloads (lo que alimenta el ADN y las probabilidades). Si un equipo no
  // llega al mínimo, el motor estaría calculando sobre la nada → análisis insufficient
  // y SIN picks. N=5: por debajo de 5 partidos las tasas empíricas por mercado son
  // ruido; más estricto que el viejo umbral de 2 (que ni se aplicaba al crudo) y
  // alineado con la convención de "últimos 5".
  const MIN_TEAM_HISTORY = 5;
  const homeFinished = inputs?._counts?.homeFinished ?? 0;
  const awayFinished = inputs?._counts?.awayFinished ?? 0;
  ...
  const todayCtx = buildTodayCtx(analysis);
  // key_injury con el MISMO criterio que el entrenamiento: ¿un lesionado de hoy
  // pertenece al XI habitual (modal XI) de su equipo? (no "cualquier lesión").
  todayCtx.keyInjury = isKeyInjury(analysis.injuries, inputs.modalXIByTeam);
  const mlEnabled = process.env.CONTEXT_ML_ENABLED === 'true';
  const mlModels = mlEnabled ? await loadRuptureModels(pgPool) : null;
  const familyModels = mlEnabled ? await loadFamilyModels(pgPool) : null;
  // Calibración por shrink hacia la tasa base (gated por CONTEXT_SHRINK_ENABLED).
  // Si la flag está off o no hay tasas computadas → baseRates=null → sin shrink.
  const shrinkEnabled = process.env.CONTEXT_SHRINK_ENABLED === 'true';
  const baseRates = shrinkEnabled ? await loadMarketBaseRates(pgPool) : null;
  // Recencia del ADN (gated por CONTEXT_RECENCY_ENABLED): los partidos recientes
  // pesan más en la tasa empírica. refMs = ahora. OFF → recency=null → idéntico.
  const recencyEnabled = process.env.CONTEXT_RECENCY_ENABLED === 'true';
  const recency = recencyEnabled ? { halfLifeDays: Number(process.env.CONTEXT_RECENCY_HALFLIFE_DAYS) || 180, refMs: Date.now() } : null;
  const scored = scoreContext(computeContext({ ...inputs, recency }), {
    ...
    mlModels, mlEnabled, familyModels, baseRates,
  });
  const calculatedProbabilities = buildProbabilitiesFromContext(scored, inputs, analysis);
  // 6º arg: analysis.cornerCardData (de extractMatchStatsData) — la FUENTE FIABLE
  // de qué categorías de stats tienen muestra real (arrays *PerMatch). El veto de
  // mercados sin datos la usa; los promedios mienten cuando la liga no cubre stats.
  // Sin historial suficiente en el crudo NO se emiten recomendaciones: combinada
  // vacía (el partido se muestra, pero sin picks construidos sobre un lado vacío).
  const combinadaResult = dataQuality === 'insufficient'
    ...
    : buildContextCombinada(scored, analysis.odds, teamNames, analysis.playerHighlights, calculatedProbabilities, analysis.cornerCardData);
  // Persistir los 1133 mercados (auditable). Fire-and-forget.
  persistFixtureContext(pgPool, fixtureId, date, scored).catch(e => console.error('[context-engine] persist:', e.message));
  console.log('[context-engine] fixture=%s: %d mercados con datos, %d recomendados (h2h=%d/adn=%d cruces)',
    fixtureId, Object.keys(scored).length, combinadaResult.selections.length, inputs._counts.meetings, inputs._counts.meetingsReconstructed);
...
const BET_NAMES = {
  ...
  // Asian Handicap
  // (Hándicap eliminado del catálogo — no se extrae ni se recomienda.)
...
export function extractOdds(oddsData) {
  ...
  // (Hándicap asiático ELIMINADO del catálogo — ya no se extrae ni se recomienda.)
  ...
  const AB_SPECS = [
    ...
    // Asian Handicap NO va aquí: se enruta con classifyAH (ver ahFromBets) para que
    // full/1H/2H jamás colisionen aunque compartan línea (p.ej. away_p3_5).
