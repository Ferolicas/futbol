...
export async function analyzeMatch(fixture, { date: requestDate, force } = {}) {
  ...
  // Fase 6a: con CONTEXT_ENGINE_ENABLED='true' las probs (mercados Y estadísticas
  // descriptivas) salen del MOTOR DE CONTEXTO desde el crudo real — sin Dixon-Coles
  // ni isotónica. Mercados = contexto específico (prob_final); estadísticas
  // descriptivas = promedio GENERAL del equipo. Si el flag está OFF (o el motor
  // falla), cae a la ruta Dixon-Coles + isotónica actual (producción intacta).
  ...
  let calculatedProbabilities = null;
  let combinadaResult = null;
  ...
  if (CONTEXT_ENGINE_ENABLED) {
    try {
      ...
      // key_injury con el MISMO criterio que el entrenamiento: ¿un lesionado de
      // hoy pertenece al XI habitual (modal XI) de su equipo? (no "cualquier lesión").
      todayCtx.keyInjury = isKeyInjury(analysis.injuries, inputs.modalXIByTeam);
      // Capa ML (detector de ruptura) — gated por CONTEXT_ML_ENABLED (OFF por defecto).
      ...
      calculatedProbabilities = buildProbabilitiesFromContext(scored, inputs, analysis);
      combinadaResult = buildContextCombinada(scored, analysis.odds, teamNames, analysis.playerHighlights, calculatedProbabilities);
      usedEngine = true;
      ...
      console.error('[context-engine] FALLÓ, fallback a Dixon-Coles:', e.message);
      calculatedProbabilities = null;
    }
  }

  if (!calculatedProbabilities) {
    // ── Ruta Dixon-Coles + isotónica (actual) ──
    const { computeAllProbabilities } = await import('./calculations');
    calculatedProbabilities = computeAllProbabilities(analysis);
    const { calibrateProbabilities } = await import('./calibration');
    await calibrateProbabilities(calculatedProbabilities);
    try {
      const { calibrateContextual } = await import('./contextual-calibration');
      await calibrateContextual(calculatedProbabilities, analysis);
    } catch (e) { console.error('[contextual]', e.message); }
  }
  analysis.calculatedProbabilities = calculatedProbabilities;

  if (!combinadaResult) {
    const { buildCombinada } = await import('./combinada');
    const tournamentContext = await getTournamentContext(homeLeagueId, season).catch(() => null);
    combinadaResult = buildCombinada(
      calculatedProbabilities, analysis.odds, analysis.playerHighlights,
      teamNames, tournamentContext,
    );
  }
  ...
  // Save prediction snapshot to match_predictions (fire-and-forget). Solo en la
  // ruta DC — con el motor de contexto la persistencia va a market_context_analysis.
  if (!usedEngine) _savePrediction(analysis, calculatedProbabilities).catch(() => {});
...
async function _savePrediction(analysis, probs) {
  try {
    const { supabaseAdmin } = await import('./supabase');
    const { buildFeatureSnapshot } = await import('./feature-snapshot');
    ...
    // Snapshot completo de todas las probs calculadas — para calibracion
    // futura de mercados nuevos (corners 4.5-14.5, cards 1.5-7.5, per-team,
    // shots, sot, fouls, half-time, etc.). NO incluimos cornerCardData raw
    // (perMatch arrays) que son grandes y no se usan para calibrar.
    const predictionsFull = {
      // meta.contextual_status: applied | flag_off | no_models_loaded | error
      // (lo escribe calibrateContextual) → visible en BD sin depender de logs.
      meta:            probs.meta || null,
      confidence:      probs.confidence || null,
      contextualMarkets: probs.contextualMarkets || null,
      winner:          probs.winner,
      btts:            probs.btts,
      bttsNo:          probs.bttsNo,
      overUnder:       probs.overUnder,
      corners:         probs.corners,
      cards:           probs.cards,
      shots:           probs.shots,
      sot:             probs.sot,
      fouls:           probs.fouls,
      firstGoal:       probs.firstGoal,
      goalTiming:      probs.goalTiming,
      halfGoals:       probs.halfGoals,
      halfWinner:      probs.halfWinner,
      perTeam:         probs.perTeam,
      perTeamShots:    probs.perTeamShots,
      perTeamFouls:    probs.perTeamFouls,
      perTeamHalfGoals: probs.perTeamHalfGoals,
      asianHandicap:   probs.asianHandicap,
      mostCorners:     probs.mostCorners,
      mostShots:       probs.mostShots,
      mostFouls:       probs.mostFouls,
      lambdaHome:      probs.lambdaHome,
      lambdaAway:      probs.lambdaAway,
      halfSplit1H:     probs.halfSplit1H,
      halfSplit2H:     probs.halfSplit2H,
    };
    ...
    await supabaseAdmin.from('match_predictions').upsert({
      ...
      // Columnas legacy (compat con calibration vieja + queries existentes)
      ...
      // Nuevo: snapshot completo para calibracion dinamica (todos los mercados)
      predictions_full: predictionsFull,
      // Fase 1 modelo contextual: snapshot point-in-time de features de contexto.
      features_full:    buildFeatureSnapshot(analysis, probs),
