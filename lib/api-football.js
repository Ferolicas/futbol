...
export async function analyzeMatch(fixture, { date: requestDate, force } = {}) {
  ...
  const modelCtx = await buildModelCtx(pgPool, {
    ...
    round: fixture.league.round, referee: fixture.fixture?.referee, homeId, awayId,
    homeRank: homePosition, awayRank: awayPosition, cutoff: new Date(),
  });
  ...
  const scored = modelToScored(pr.markets, { validationFamilies: pr.validationFamilies }); // frecuencia intacta; validación solo diagnóstica
    ...
    if (!Object.keys(playerMarkets).length && analysis.lineups?.available && Array.isArray(analysis.lineups.data)) {
      ...
      if (startXI.length) {
        playerMarkets = await buildPlayerMarkets(pgPool, startXI, {
          cutoff: new Date(), season: modelCtx.season,
