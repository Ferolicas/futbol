...
// SOLO estas 4 casas (decisión explícita del usuario — ninguna otra). El filtro es
// por NOMBRE (normalizeBkName); API-Football trae el nombre del bookmaker, no
// dependemos de IDs numéricos. (Ref IDs API-Football: bet365=8, bwin=6, 1xbet=11,
// betano=32 — informativo; el match real es por nombre.)
const ALLOWED_BOOKMAKERS = ['bwin', 'bet365', '1xbet', 'betano'];
...
const BET_NAMES = {
  ...
  // Asian Handicap
  // OJO: equivalencia ESTRICTA (matchesBetName usa includes() sobre esta lista,
  // que son nombres EXACTOS). Los tres mercados AH NUNCA deben compartir nombre →
  // cada uno cae en su propio objeto (asianHandicap / 1H / 2H), sin colisión.
  asianHandicap: ['Asian Handicap'],
  ahFirstHalf:   ['Asian Handicap First Half', 'Asian Handicap (1st Half)'],
  ahSecondHalf:  ['Asian Handicap (2nd Half)', 'Asian Handicap Second Half'],
...
export function extractOdds(oddsData) {
  ...
  // ── Asian Handicap (full / 1ª mitad / 2ª mitad) — SIN colisión ──
  // Clasificación ROBUSTA por tokens de mitad (no por nombre exacto): cualquier bet
  // cuyo nombre contenga 'half'/'1st'/'2nd'/'first'/'second' NUNCA entra al objeto
  // full-time. Así "Asian Handicap (2nd Half)" jamás contamina asianHandicap, sin
  // depender de que el nombre coincida carácter a carácter con una lista.
  // key = línea firmada: "Home -1.5" → home_m1_5, "Away +0.5" → away_p0_5.
  const ahKey = v => {
    const raw = (v.value || '').toString().trim();
  ...
  const classifyAH = (betName) => {
    ...
    if (!n.includes('handicap')) return null;            // no es handicap asiático
    ...
    if (n.includes('half')) return null;                 // mitad NO identificada → fuera del full
    return 'full';
  };
  // Enruta los values de los bets AH de un array → {full, h1, h2} (máx por clave).
  const ahFromBets = (bets) => {
    const out = { full: {}, h1: {}, h2: {} };
    for (const bet of bets || []) {
      const cls = classifyAH(bet.name); if (!cls) continue;
      const tgt = cls === 'h1' ? out.h1 : cls === 'h2' ? out.h2 : out.full;
      for (const v of bet.values || []) {
        const k = ahKey(v); if (!k) continue;
        const o = parseFloat(v.odd); if (!isFinite(o) || o <= 1) continue;
        if (!tgt[k] || o > tgt[k]) tgt[k] = o;
      }
    }
    return out;
  };
  {
    const full = {}, h1 = {}, h2 = {};
    for (const bk of bookmakers) {
      const e = ahFromBets(bk.bets);
      for (const [k, o] of Object.entries(e.full)) if (!full[k] || o > full[k]) full[k] = o;
      for (const [k, o] of Object.entries(e.h1)) if (!h1[k] || o > h1[k]) h1[k] = o;
      for (const [k, o] of Object.entries(e.h2)) if (!h2[k] || o > h2[k]) h2[k] = o;
    }
    if (Object.keys(full).length) result.asianHandicap = full;
    if (Object.keys(h1).length) result.asianHandicap1H = h1;
    if (Object.keys(h2).length) result.asianHandicap2H = h2;
  }
  ...
  // Mapa {key: mejorOdd} de UN bookmaker para una familia (no agregado).
  ...
  result.allBookmakerOdds = bookmakers.map(bk => {
    ...
    // Asian Handicap por bookmaker — mismo router classifyAH → full/1H/2H separados.
    const ah = ahFromBets(bk.bets);
    if (Object.keys(ah.full).length) entry.asianHandicap = ah.full;
    if (Object.keys(ah.h1).length) entry.asianHandicap1H = ah.h1;
    if (Object.keys(ah.h2).length) entry.asianHandicap2H = ah.h2;
    const players = {};
