...
export async function analyzeMatch(fixture, { date: requestDate, force } = {}) {
  ...
  // Stage 2.2: pasar league IDs de cada partido del historial para que
  // extractMatchStatsData pueda aplicar el filtro same-league (≥3 same-league
  // del histórico → filtrar a esos; sino mezclar todo). Mismo threshold que
  ...
  // Importante: usar el MISMO orden de fixtureIds, NO filtrar nulls aqui —
  // si filtraramos antes de obtener los league IDs, perderiamos alineamiento.
  // Si una fixture no tiene stats, su matchStat es null/undefined; pero su
  // league ID sigue siendo valido para el filtro. extractMatchStatsData
  // se encarga de saltar entradas null gracias al filter(Boolean) interno.
  const homeMatchStatsAligned = homeFixtureIds.map(fid => matchStatsMap[fid] || null);
  const awayMatchStatsAligned = awayFixtureIds.map(fid => matchStatsMap[fid] || null);
  const homeLeagueIds = (results.homeLastFive || []).map(f => f.league?.id ?? null);
  const awayLeagueIds = (results.awayLastFive || []).map(f => f.league?.id ?? null);
