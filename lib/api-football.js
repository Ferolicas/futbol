...
export async function analyzeMatch(fixture) {
  ...
  for (const fid of uniqueFixtureIds) {
    try {
      ...
      if (!fromCache) apiCalls++;
    } catch { matchStatsMap[fid] = []; }
  }

  // 11. Match players for last 5 of each team (shooters, scorers, usual XI)
  ...
  for (const fid of uniqueFixtureIds) {
    try {
      ...
      matchPlayersMap[fid] = data;
      if (!fromCache) apiCalls++;
      console.log(`[ANALYSIS] matchPlayers ${fid}: ${Array.isArray(data) ? data.length + ' teams' : 'null'}, fromCache=${fromCache}`);
    } catch (e) {
      matchPlayersMap[fid] = [];
      console.log(`[ANALYSIS] matchPlayers ${fid} ERROR: ${e.message}`);
    }
  }
