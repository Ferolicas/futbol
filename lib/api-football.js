...
async function cachedApiCall(cacheKey, endpoint) {
  ...
  if (cached !== null && !(Array.isArray(cached) && cached.length === 0)) {
    ...
    await redisSet(`api:${cacheKey}`, cached, 7 * 86400);
  ...
  try {
    ...
    if (!Array.isArray(data) || data.length > 0) {
      await Promise.all([
        cacheEndpoint(cacheKey, data),
        redisSet(`api:${cacheKey}`, data, 7 * 86400),
      ]);
    }
...
export async function analyzeMatch(fixture, { date: requestDate, force } = {}) {
  ...
  } catch (err) { console.error('[api-football:theOddsApi]', err.message); results.theOddsApiData = null; }
