...
function currentSeason() {
  const now = new Date();
  // Pro plan: current season (e.g. 2025 for 2025-2026 season)
...
export async function analyzeMatch(fixture, { date: requestDate, force } = {}) {
  ...
  // 2. Home last 5
  results.homeLastFive = await fetchLast5(homeId, season, finishedStatuses);
  console.log(`[ANALYSIS] Home last5 (${homeId}): ${results.homeLastFive.length} matches`);

  // 3. Away last 5
  results.awayLastFive = await fetchLast5(awayId, season, finishedStatuses);
  ...
  // 8b. Enrich with The Odds API if available (cached odds from cron)
  try {
    const { getFromSanity: getSanity } = await import('./sanity');
    const oddsCache = await getSanity('oddsCache', `odds-${fixtureId}`);
    if (oddsCache?.odds) {
      results.theOddsApiData = oddsCache.odds;
    }
  } catch { results.theOddsApiData = null; }
  ...
  const homeLastFiveEnriched = enrichLastFiveMatches(results.homeLastFive || [], homeId);
  const awayLastFiveEnriched = enrichLastFiveMatches(results.awayLastFive || [], awayId);
  ...
  // Save to Sanity (global cache — analysis data is shared)
  const saved = await cacheAnalysis(fixtureId, analysis);
  if (!saved) {
    console.error(`[ANALYSIS] FAILED to save fixture ${fixtureId} to Sanity`);
  }
...
function enrichLastFiveMatches(matches, teamId) {
