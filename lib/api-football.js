...
function extractPlayerHighlights(homeFixtureIds, awayFixtureIds, matchPlayersMap, homeId, awayId, homeTeamName, awayTeamName) {
  ...
  // Filtros de candidatos — modo ESTRICTO (instrucción del producto).
  // Para SCORER, SHOOTER (tiros a puerta) y SHOTSTOTAL (remates):
  //   El jugador debe registrar ≥1 evento en CADA UNO de los partidos del
  //   historial. Una media de "1 gol por partido" pero con un 4-0-0-0-1
  //   no califica; queremos que el evento ocurra en TODOS los partidos.
  //   Razon: el apostador necesita certeza de recurrencia, no promedio.
  //
  // Para ASSISTERS, FOULERS, BOOKERS: media menos punitiva (60%) — son
  // eventos mas raros por naturaleza; exigir 10/10 dejaria el grupo vacio.
  const MIN_HISTORY = 8;                 // partidos minimos requeridos
  const STRICT_HIT_RATIO = 1.0;          // 100% — todos los partidos
  const NORMAL_HIT_RATIO = 0.6;          // 60% — fouls/cards/assists
