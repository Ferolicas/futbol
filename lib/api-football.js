...
async function getTournamentContext(leagueId, season) {
  ...
  try {
    ...
    console.error('[tournament-context]', e.message);
    return null;
  }
}
...
export async function analyzeMatch(fixture, { date: requestDate, force } = {}) {
  ...
  try {
    ...
    console.error('[api-football:odds]', err.message);
    throw err;
  }
  ...
  if (earlyRedFids.size > 0) {
    console.log(`[ANALYSIS] excluded ${earlyRedFids.size} fixtures with early red cards from stats (home kept ${home.fids.length}/${homeFixtureIds.length}, away ${away.fids.length}/${awayFixtureIds.length})`);
  }
  ...
  if (dataQuality === 'no-data') {
    console.warn(`[ANALYSIS] sin hechos previos fixture=${fixtureId}: homeFinished=0 awayFinished=0`);
  }
  ...
  } catch (e) { console.error(`[ANALYSIS] playerMarkets ${fixtureId}:`, e.message); }
  ...
  console.log('[model-engine] fixture=%s: %d mercados, %d recomendados (h2h=%s jugador=%s nTeams=%s)',
    fixtureId, Object.keys(scored).length, combinadaResult.selections.length, pr.applied?.h2h, pr.applied?.player, modelCtx.nTeams);
  ...
  try {
    ...
    console.error(`[ANALYSIS] finalVerdict ${fixtureId}:`, error.message);
  ...
  try {
    ...
    console.error(`[ANALYSIS] cache save THREW ${fixtureId}:`, e.message);
  }
