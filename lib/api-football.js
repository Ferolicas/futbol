...
export async function analyzeMatch(fixture, { date: requestDate, force } = {}) {
  ...
  // Capa CONTEXTUAL (meta-modelo de 3 niveles). Inerte si
  // CONTEXTUAL_MODEL_ENABLED!=='true' o sin modelos activos → deja la isotónica
  // intacta. Recalibra (ambos lados) solo los mercados con modelo que superó al
  // baseline, y adjunta confidence por mercado.
  try {
  ...
  // Compute combinada server-side. Si el partido pertenece a un torneo
  // knockout, inyectamos contexto de bracket (P3) — campeón/finalista/semis
  // de los 2 equipos. El bracket se simula una vez por liga/día (cacheado en
  // Redis) para no correr Monte Carlo en cada análisis.
  ...
  const teamNames = {
    home: analysis.homeTeam, away: analysis.awayTeam,
    homeId, awayId,
  };
  analysis.combinada = buildCombinada(
