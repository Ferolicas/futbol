...
import { redisIncr, redisGet } from './redis';
...
// Get cached fixtures ignoring staleness (for batch analysis chain)
export async function getCachedFixturesRaw(date) {
  const doc = await getFromSanity('footballFixturesCache', date);
  return doc?.fixtures || null;
}
...
export async function getCachedAnalysis(fixtureId, requestDate, { strict } = {}) {
  ...
  // Allow analysis from today OR yesterday (timezone differences between client/server)
  if (doc.date !== today) {
    const yesterday = new Date(new Date(today).getTime() - 86400000).toISOString().split('T')[0];
    if (doc.date !== yesterday) return null;
  }
  ...
  const ver = doc.cacheVersion || doc._cacheVersion || 0;
  // strict = batch re-analysis needs latest version; normal = show any valid analysis
  if (strict && ver < 7) return null;
  if (!strict && ver < 1) return null;
  return doc;
}

export async function cacheAnalysis(fixtureId, data) {
  try {
    return await saveToSanity('footballMatchAnalysis', String(fixtureId), {
    ...
    console.error(`[CACHE] Failed to save analysis for fixture ${fixtureId}:`, e.message);
    return false;
  }
}
...
export async function getApiCallCount() {
  ...
  const redisKey = `apicalls:${today}`;
  // Try Redis first (atomic, ~2ms)
  const redisCount = await redisGet(redisKey);
  if (redisCount !== null) return Number(redisCount);
  // Fallback to Sanity
...
export async function incrementApiCallCount() {
  const today = new Date().toISOString().split('T')[0];
  const redisKey = `apicalls:${today}`;
  // Atomic Redis INCR (~2ms, no race condition)
  const count = await redisIncr(redisKey, 86400);
  if (count !== null) return count;
  // Fallback to Sanity if Redis unavailable
  const docId = `apiCalls-${today}`;
  const doc = await getFromSanity('appConfig', docId);
  const newCount = (doc?.count || 0) + 1;
  await saveToSanity('appConfig', docId, { date: today, count: newCount });
  return newCount;
}
...
// Set fresh=true to bypass CDN and read from Sanity origin (use after re-analyze)
export async function getAnalyzedMatchesFull(fixtureIds, { fresh } = {}) {
  if (!fixtureIds || fixtureIds.length === 0) return { analyzedOdds: {}, analyzedData: {} };
  const readFn = fresh
    ? (await import('./sanity')).getFromSanityFresh
    : getFromSanity;
  const docs = await Promise.all(
    fixtureIds.map(id => readFn('footballMatchAnalysis', String(id)))
  );
  const analyzedOdds = {};
  const analyzedData = {};
  fixtureIds.forEach((id, i) => {
