...
export async function getCachedAnalysis(fixtureId, requestDate, { strict } = {}) {
  ...
  if (cached) {
    ...
    if (unwrapped.date === today) return { ...unwrapped, combinada: sanitizeFootballCombinada(unwrapped.combinada) };
    const yesterday = new Date(new Date(today).getTime() - 86400000).toISOString().split('T')[0];
    if (unwrapped.date === yesterday) return { ...unwrapped, combinada: sanitizeFootballCombinada(unwrapped.combinada) };
    return null;
  }
  ...
  try {
    ...
    const doc = {
      ...
      combinada: sanitizeFootballCombinada(data.combinada || inner.combinada || null),
...
function extractAnalysisFields(doc) {
  return {
    ...
    odds: doc.odds, combinada: sanitizeFootballCombinada(doc.combinada),
