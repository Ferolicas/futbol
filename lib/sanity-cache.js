 ...
 */
import { redisGet, redisMGet, redisSet, redisIncr, redisIncrBy } from './redis';
...
// v24 añade el veredicto oficial aislado, pero NO cambia el motor v23. Mantener
// lectura de v23 evita ocultar análisis finalizados mientras el worker completa
// el nuevo bloque en segundo plano.
export const FOOTBALL_CACHE_VERSION = 24;
const MIN_CACHE_VERSION = 23;
...
export async function cacheAnalysis(fixtureId, data) {
  ...
  try {
    ...
    if (res?.error) {
      ...
      dbOk = true;
      console.log('[cacheAnalysis] upsert ok:', safeLogFields({ fixtureId, date, dataQuality: data.dataQuality || 'good' }));
    }
  } catch (e) {
    dbErr = e.message;
