...
// v21 cambia únicamente la política de publicación, no los cálculos de v20:
// por eso v20 puede reutilizarse y se sanea en cada frontera de salida. Esto
// evita dejar una jornada vacía o gastar cuota API en un recálculo idéntico.
// Versiones <20 sí pertenecen a contratos estadísticos anteriores.
export const FOOTBALL_CACHE_VERSION = 21;
const MIN_CACHE_VERSION = 20;
...
export async function cacheAnalysis(fixtureId, data) {
  ...
  try {
    ...
    redisOk = false;
    console.error(`[cacheAnalysis] Redis save failed for ${fixtureId}:`, e.message);
  }
  ...
  try {
    ...
    if (res?.error) {
      ...
      console.error(`[cacheAnalysis] PG upsert error fid=${fixtureId} date=${date}:`, dbErr);
    } else {
      dbOk = true;
      console.log(`[cacheAnalysis] ✓ upsert fid=${fixtureId} date=${date} dataQuality=${data.dataQuality || 'good'}`);
    }
  } catch (e) {
    dbErr = e.message;
    console.error(`[cacheAnalysis] PG upsert THREW fid=${fixtureId} date=${date}:`, e.message);
  }
