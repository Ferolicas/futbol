...
function getClient() {
  ...
  _client = new IORedis({
    ...
    db: parseInt(process.env.LOCAL_REDIS_DB || process.env.REDIS_DB || '0', 10),
    maxRetriesPerRequest: 3,
    lazyConnect: false,
    enableReadyCheck: true,
  });
  _client.on('error', (err) =>
    console.error('[REDIS] connection error:', safeLogValue(err)),
  );
  return _client;
}
...
export async function redisGet(key) {
  try {
    ...
    console.error('[REDIS] GET error:', safeLogFields({ key, error: e?.message }));
    return null;
  }
}
...
export async function redisMGet(keys) {
  ...
  try {
    ...
    console.error('[REDIS] MGET error:', safeLogValue(e));
...
export async function redisSet(key, value, ttlSeconds) {
  try {
    ...
    console.error('[REDIS] SET error:', safeLogFields({ key, error: e?.message }));
    return false;
  }
}
...
export async function redisIncr(key, ttlSeconds) {
  try {
    ...
    console.error('[REDIS] INCR error:', safeLogFields({ key, error: e?.message }));
    return null;
  }
}
...
export async function redisIncrBy(key, n, ttlSeconds) {
  try {
    ...
    console.error('[REDIS] INCRBY error:', safeLogFields({ key, error: e?.message }));
    return null;
  }
}
...
export async function redisAcquireRateSlot(key, intervalMs) {
  try {
    ...
    console.error('[REDIS] rate slot error:', safeLogFields({ key, error: e?.message }));
    return null;
  }
}
...
export async function redisListPush(key, value, maxLen, ttlSeconds) {
  ...
  try {
    ...
    console.error('[REDIS] LPUSH error:', safeLogFields({ key, error: e?.message }));
    return false;
  }
}
...
export async function redisListRange(key, start = 0, stop = -1) {
  try {
    ...
    console.error('[REDIS] LRANGE error:', safeLogFields({ key, error: e?.message }));
    return [];
  }
}
...
export async function redisDel(key) {
  try {
    ...
    console.error('[REDIS] DEL error:', safeLogFields({ key, error: e?.message }));
    return false;
  }
}
