...
const REC_MIN_PROB = 0.80;   // la app recomienda solo cumplimiento real ≥80%; Telegram exige ≥95%
...
function validationFor(probability, family, validationFamilies) {
  ...
  const band = probability >= 0.95 ? 'elite95' : 'high';
  const segment = metric?.[band];
  const target = probability >= 0.95 ? 0.95 : 0.80;
