...
function entry(prob_final, level, n, conf, hits = null, family = null, diagnostics = {}) {
  ...
  // La probabilidad publicada es 100% del enfrentamiento específico (H2H +
  // historial propio de cada equipo, ya cruzados antes de llegar aquí). El
  // promedio de la familia entre TODOS los equipos (calibration/validation)
  // queda solo como diagnóstico informativo — jamás corrige prob_final ni
  // decide "recommended". Mezclarlo diluye el valor exclusivo del
  // enfrentamiento: si el equipo falla, tiene que ser porque se salió de SU
  // propio patrón, no porque el consenso general infló el número.
  const calibration = validationFor(rawProbability, family, diagnostics.calibrationFamilies);
  const validation = validationFor(rawProbability, family, diagnostics.validationFamilies);
  const validationDecision = assessValidation(validation);
  return {
    prob: rawProbability,
    prob_raw: rawProbability,
    prob_calibrated: rawProbability,
    prob_final: rawProbability,
    level: level || 'empirical',
    n: n || 0,
    hits: hits == null ? null : Number(hits),
    confidence: conf != null ? conf : confFromN(n),
    recommended: rawProbability >= REC_MIN_PROB,
...
function modelToScored(markets, { calibrationFamilies = null, validationFamilies = null } = {}) {
  ...
  for (const [key, mk] of Object.entries(markets)) {
    ...
    // ── 1X2 y resultados derivados (ganador por mitad, más córners/tiros/faltas) ──
    if (mk.kind === 'result') {
      const RESULT_KEYS = {
        '1x2': ['home_win', 'draw', 'away_win'],
        winner_1h: ['winner_1h_home', 'winner_1h_draw', 'winner_1h_away'],
        winner_2h: ['winner_2h_home', 'winner_2h_draw', 'winner_2h_away'],
        most_corners: ['most_corners_home', 'most_corners_draw', 'most_corners_away'],
        most_shots: ['most_shots_home', 'most_shots_draw', 'most_shots_away'],
        most_fouls: ['most_fouls_home', 'most_fouls_draw', 'most_fouls_away'],
        most_corners_1h: ['most_corners_1h_home', 'most_corners_1h_draw', 'most_corners_1h_away'],
        most_corners_2h: ['most_corners_2h_home', 'most_corners_2h_draw', 'most_corners_2h_away'],
      };
      const [homeKey, drawKey, awayKey] = RESULT_KEYS[key] || [];
      if (!homeKey) continue;
      set(homeKey, makeEntry(mk.home, mk.level, mk.n, mk.homeConf ?? mk.conf, null, homeKey));
      set(drawKey, makeEntry(mk.draw, mk.level, mk.n, mk.drawConf ?? mk.conf, null, drawKey));
      set(awayKey, makeEntry(mk.away, mk.level, mk.n, mk.awayConf ?? mk.conf, null, awayKey));
      continue;
    }
