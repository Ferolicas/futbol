...
// Contrato visual: nunca redondear 94,x a 95. Por debajo del tope se conserva
// una décima (las probabilidades del motor tienen precisión suficiente); desde
// 95 hacia arriba se muestra 95 para evitar presentar un 100% como garantía.
function displayPct(value) {
  if (value == null || !Number.isFinite(Number(value))) return null;
  const pct = Math.max(0, Number(value));
  if (pct >= 95) return 95;
  return Math.floor((pct + 1e-9) * 10) / 10;
}
...
function validationFor(probability, family, validationFamilies) {
  ...
  if (!hasValidation) return {
    available: false, family: family || null, eligible: false,
    daily90Validated: false,
    elite95Validated: false,
  };
  ...
  // El gate jamás calibra ni cambia la frecuencia. Solo comprueba que esa misma
  // familia sostuvo fuera de muestra el porcentaje mostrado (95 es el tope visual).
  const target = Math.min(0.95, probability);
  const avgActual = segment?.avg_actual == null ? null : Number(segment.avg_actual);
  const eligible = Number(segment?.n || 0) > 0 && avgActual != null && avgActual + 1e-12 >= target;
  return {
    available: !!metric,
    family: family || null,
    band,
    ...
    avgActual,
    eligible,
    daily90Validated: probability >= 0.90 && eligible,
    elite95Validated: probability >= 0.95 && eligible,
  };
}
...
function entry(prob_final, level, n, conf, hits = null, family = null, validationFamilies = null) {
  ...
  return {
    ...
    // La validación nunca modifica p. Solo decide si una frecuencia calculada
    // merece etiqueta comercial de recomendación fuera de muestra.
    recommended: p >= REC_MIN_PROB && validation.eligible,
    validation,
  };
}
