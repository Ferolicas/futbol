...
function playerMarketsToSelections(playerMarkets) {
  ...
  for (const pm of Object.values(playerMarkets)) {
    ...
    const push = (cat, prob, line, label) => { if (prob == null) return; out.push({ id: `${cat}-${id}${line != null ? '-' + line : ''}`, category: `${cat}-${id}`, scope: 'player', playerId: id, playerName: name, name: label, probability: pct(prob), rawProbability: prob * 100, _line: line != null ? line : undefined }); };
