...
function entry(prob_final, level, n, conf, hits = null, family = null, validationFamilies = null) {
  ...
  const validation = validationFor(rawProbability, family, validationFamilies);
  const calibratedProbability = calibrateProbability(rawProbability, validation);
  ...
  return {
    ...
    validation: { ...validation, decision: validationDecision },
  };
}
  ...
  for (const [key, mk] of Object.entries(markets)) {
    ...
    if (mk.kind === 'ou') {
      ...
      for (const ln of (mk.lines || [])) {
        ...
        set(`${base}_under${N}_5`, entry(ln.prob == null ? null : round(1 - ln.prob), ln.level, ln.n, ln.underConf ?? ln.conf, ln.hits == null ? null : ln.n - ln.hits, lineDiagnosticKey(key, 'under', ln.line), validationFamilies));
      }
      continue;
    }
    ...
    // ── 1X2 ──
    if (mk.kind === 'result') {
      set('home_win', entry(mk.home, mk.level, mk.n, mk.homeConf ?? mk.conf, null, '1x2_home', validationFamilies));
      set('draw',     entry(mk.draw, mk.level, mk.n, mk.drawConf ?? mk.conf, null, '1x2_draw', validationFamilies));
      set('away_win', entry(mk.away, mk.level, mk.n, mk.awayConf ?? mk.conf, null, '1x2_away', validationFamilies));
      continue;
    }
    ...
    if (mk.kind === 'bool') {
      if (key === 'btts') {
        ...
        set('btts_no', entry(mk.prob == null ? null : round(1 - mk.prob), mk.level, mk.n, mk.inverseConf ?? mk.conf, mk.hits == null ? null : mk.n - mk.hits, 'btts_no', validationFamilies));
    ...
    if (mk.kind === 'multi') {
      ...
      if (key === 'double_chance') {
        set('dc_1x', entry(mk['1X'], 'derived', mk.n || n1x2, mk.conf1X ?? c1x2, null, 'dc_1x', validationFamilies));
        set('dc_12', entry(mk['12'], 'derived', mk.n || n1x2, mk.conf12 ?? c1x2, null, 'dc_12', validationFamilies));
        set('dc_x2', entry(mk['X2'], 'derived', mk.n || n1x2, mk.confX2 ?? c1x2, null, 'dc_x2', validationFamilies));
      } else if (key === 'odd_even') {
        set('goals_odd',  entry(mk.odd,  'derived', mk.n, mk.oddConf ?? confFromN(mk.n), null, 'goals_odd', validationFamilies));
        set('goals_even', entry(mk.even, 'derived', mk.n, mk.evenConf ?? confFromN(mk.n), null, 'goals_even', validationFamilies));
        ...
        for (const k of ['m0.5', 'm1.5', 'p0.5', 'p1.5']) {
          ...
          set(family, entry(mk[k], 'derived', n1x2, c1x2, null, family, validationFamilies));
        }
      } else if (key === 'handicap_home_eu') {
        for (const k of ['m1', 'p1']) set(`eh_home_${k}`, entry(mk[k], 'derived', n1x2, c1x2, null, `eh_home_${k}`, validationFamilies));   // hándicap europeo (3-way) local ∓1
      }
      continue;
    }
    ...
    if (mk.kind === 'list' && key === 'exact_score') {
      for (const e of (mk.lines || [])) {
        ...
        set(`cs_${h}_${a}`, entry(round(e.prob), 'derived', mk.n, confFromN(mk.n), null, `cs_${h}_${a}`, validationFamilies));   // marcador exacto h-a → resolveOddField cs_* → correctScore
      }
      ...
      for (const [bucket, p] of Object.entries(mk.totalProbabilities || {})) {
        set(`exact_goals_${bucket}`, entry(round(p), 'derived', mk.n, confFromN(mk.n), null, `exact_goals_${bucket}`, validationFamilies));
      }
      continue;
    }
  }
