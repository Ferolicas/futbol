...
export function marketLabel(key, teamNames) {
  ...
  // ── Hándicap (asiático ah_ / europeo eh_), perspectiva del LOCAL ──
  // m = menos (−), p = más (+); línea media (m0_5 → 0.5) o entera (m1 → 1).
  const hc = key.match(/^(ah|eh)_home_(m|p)(\d+)(?:_(\d+))?$/);
  if (hc) {
    const tipo = hc[1] === 'ah' ? 'asiático' : 'europeo';
    const signo = hc[2] === 'm' ? '-' : '+';
    const linea = hc[4] != null ? `${hc[3]}.${hc[4]}` : hc[3];
    return `Hándicap ${tipo} — ${H} ${signo}${linea}`;
  }
  // ── Marcador exacto (cs_<golesLocal>_<golesVisita>) ──
  const cs = key.match(/^cs_(\d+)_(\d+)$/);
  if (cs) return `Marcador exacto — ${H} ${cs[1]}-${cs[2]} ${A}`;
  // ── Escalares ──
  const S = {
    ...
    home_goals_odd: `${H} — Goles: impar`, home_goals_even: `${H} — Goles: par`,
    away_goals_odd: `${A} — Goles: impar`, away_goals_even: `${A} — Goles: par`,
    home_exact_goals_0: `${H} — Exactamente 0 goles`, home_exact_goals_1: `${H} — Exactamente 1 gol`, home_exact_goals_2: `${H} — Exactamente 2 goles`, home_exact_goals_3plus: `${H} — 3 o más goles`,
    away_exact_goals_0: `${A} — Exactamente 0 goles`, away_exact_goals_1: `${A} — Exactamente 1 gol`, away_exact_goals_2: `${A} — Exactamente 2 goles`, away_exact_goals_3plus: `${A} — 3 o más goles`,
