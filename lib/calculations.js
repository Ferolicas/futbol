...
export function computeAllProbabilities(analysis) {
  ...
  const { lambdaMul, cardMul } = roundLambdaFactor(roundType);
  const lambdaHome = Math.max(0.3, Math.min(5.0, injuryAdjusted.lambdaHome * lambdaMul));
  const lambdaAway = Math.max(0.3, Math.min(5.0, injuryAdjusted.lambdaAway * lambdaMul));
