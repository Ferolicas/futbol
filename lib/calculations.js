...
const HALF_SPLIT_1H = 0.45;
const HALF_SPLIT_2H = 0.55;
...
export function calculateHalfGoalsProbabilities(lambdaHome, lambdaAway) {
  const λFull = (lambdaHome || 0) + (lambdaAway || 0);
  const λ1H = λFull * HALF_SPLIT_1H;
  const λ2H = λFull * HALF_SPLIT_2H;
  return {
    firstHalf: {
...
export function calculatePerTeamHalfGoalsProbabilities(lambdaHome, lambdaAway) {
  const λH1 = (lambdaHome || 0) * HALF_SPLIT_1H;
  const λH2 = (lambdaHome || 0) * HALF_SPLIT_2H;
  const λA1 = (lambdaAway || 0) * HALF_SPLIT_1H;
  const λA2 = (lambdaAway || 0) * HALF_SPLIT_2H;
  const block = (λ) => ({
    over05: clamp(pct(1 - poissonCDF(0, λ)), 5, 95),
    over15: clamp(pct(1 - poissonCDF(1, λ)), 5, 95),
...
export function calculateHalfWinnerProbabilities(lambdaHome, lambdaAway) {
  const half = (lH, lA) => calculateWinProbabilitiesFromLambda(lH, lA);
  return {
    firstHalf:  half(lambdaHome * HALF_SPLIT_1H, lambdaAway * HALF_SPLIT_1H),
    secondHalf: half(lambdaHome * HALF_SPLIT_2H, lambdaAway * HALF_SPLIT_2H),
  };
}
...
export function computeAllProbabilities(analysis) {
  ...
  // 1H/2H goles y ganador (siempre se calculan, derivan de lambdas)
  const halfGoals    = calculateHalfGoalsProbabilities(lambdaHome, lambdaAway);
  const halfWinner   = calculateHalfWinnerProbabilities(lambdaHome, lambdaAway);
  const perTeamHalfGoals = calculatePerTeamHalfGoalsProbabilities(lambdaHome, lambdaAway);
