...
export function calculateCornerProbabilities(avgTotal, perMatchValues) {
  ...
  return {
    over85:  clamp(Math.round((1 - poissonCDF(8,  λ)) * od * 100), 5, 95),
    over95:  clamp(Math.round((1 - poissonCDF(9,  λ)) * od * 100), 5, 95),
    over105: clamp(Math.round((1 - poissonCDF(10, λ)) * od * 100), 5, 95),
  };
}
...
export function calculatePerTeamProbabilities(
  ...
  function cornerOverProbs(mean, od) {
    ...
    return {
      over05: clamp(Math.round((1 - poissonCDF(0, λ)) * od * 100), 5, 95),
      over15: clamp(Math.round((1 - poissonCDF(1, λ)) * od * 100), 5, 95),
      over25: clamp(Math.round((1 - poissonCDF(2, λ)) * od * 100), 5, 95),
      over35: clamp(Math.round((1 - poissonCDF(3, λ)) * od * 100), 5, 95),
      over45: clamp(Math.round((1 - poissonCDF(4, λ)) * od * 100), 5, 95),
      over55: clamp(Math.round((1 - poissonCDF(5, λ)) * od * 100), 5, 95),
    };
  }
