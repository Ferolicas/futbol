...
/**
 * Multiplicador heuristico para lambdas segun tipo de round.
 * Basado en patrones empiricos del futbol top:
 *   - Finales: equipos juegan cerrado, menos goles (-8%).
 *   - Semis: igual de tense, -4%.
 *   - Cuartos: leve cierre, -2%.
 *   - Qualifying: David vs Goliath, mas goles asimétricos (+5%).
 *   - Resto: sin ajuste.
 *
 * Returns { lambdaMul, cardMul } con factor para multiplicar.
 */
function roundLambdaFactor(roundType) {
  const map = {
    final:           { lambdaMul: 0.92, cardMul: 1.15 },  // menos goles, mas tarjetas
    semi:            { lambdaMul: 0.96, cardMul: 1.10 },
    quarter:         { lambdaMul: 0.98, cardMul: 1.05 },
    round_of_16:     { lambdaMul: 1.00, cardMul: 1.02 },
    round_of_32:     { lambdaMul: 1.00, cardMul: 1.00 },
    qualifying:      { lambdaMul: 1.05, cardMul: 0.95 },  // mas goles, menos tarjetas
    group_decisive:  { lambdaMul: 1.03, cardMul: 1.05 },  // todo o nada
    group:           { lambdaMul: 1.00, cardMul: 1.00 },
    leg_1st:         { lambdaMul: 0.95, cardMul: 1.00 },  // primer leg cauto
    leg_2nd:         { lambdaMul: 1.00, cardMul: 1.05 },  // 2do leg depende agregado
    regular:         { lambdaMul: 1.00, cardMul: 1.00 },
  };
...
/**
 * Heuristico de "importancia del partido" basado en:
 *   - Fase final de temporada (ultimo 15% de jornadas)
 *   - Posicion del equipo (top 1 ya casi campeon → rotacion; bottom 3
 *     ya casi descendido → desmotivacion; lucha por puesto europeo →
 *     +motivacion)
 *
 * Returns multiplicadores para lambdaHome y lambdaAway.
 *
 * Conservador: solo aplica si tenemos matchday + total matchdays + position.
 * Si falta cualquier dato, devuelve 1.0 (no ajusta).
 */
function applyImportanceAdjustment(lambdaHome, lambdaAway, leagueId, leagueRound, homePosition, awayPosition) {
  const totalMd = LEAGUE_TOTAL_MATCHDAYS[leagueId];
  const currentMd = parseRegularSeasonMatchday(leagueRound);
  if (!totalMd || !currentMd) {
    return { lambdaHome, lambdaAway, importance: null };
  }

  const seasonProgress = currentMd / totalMd;
  const isLateSeason = seasonProgress >= 0.85; // últimas 15% de jornadas
  const isMidSeason  = seasonProgress >= 0.30 && seasonProgress < 0.85;

  if (!isLateSeason) {
    // Mid-season: nadie esta en rotacion sistematica todavia.
    return { lambdaHome, lambdaAway, importance: { stage: isMidSeason ? 'mid' : 'early' } };
  }

  // Categorias por posicion (asumiendo liga de ~20 equipos; se ajusta proporcionalmente)
  function classify(pos) {
    if (!pos) return 'unknown';
    if (pos <= 2) return 'champion_race';      // top 2 lucha titulo
    if (pos <= 4) return 'cl_race';            // 3-4 lucha Champions
    if (pos <= 7) return 'europa_race';        // 5-7 lucha Europa
    if (pos >= 18) return 'relegation';        // 18+ relegado o descenso
    if (pos >= 15) return 'relegation_fight';  // 15-17 lucha descenso
    return 'midtable';                         // 8-14 nada que jugarse
  }

  function multiplierFor(category) {
    switch (category) {
      case 'champion_race':   return 1.05;  // motivacion alta + posible rotacion balanceada
      case 'cl_race':         return 1.06;  // lucha + ofensivos
      case 'europa_race':     return 1.04;
      case 'relegation_fight':return 1.05;  // pelea de descenso, mas tense
      case 'relegation':      return 0.92;  // ya bajado, desmotivacion
      case 'midtable':        return 0.93;  // nada que jugarse, suplencias
      default:                return 1.0;
    }
  }
  ...
  const homeMul = multiplierFor(homeCat);
  const awayMul = multiplierFor(awayCat);

  return {
    lambdaHome: Math.max(0.3, Math.min(5.0, lambdaHome * homeMul)),
    lambdaAway: Math.max(0.3, Math.min(5.0, lambdaAway * awayMul)),
    importance: {
      stage: 'late',
      progress: +seasonProgress.toFixed(2),
      home: { category: homeCat, mul: homeMul },
      away: { category: awayCat, mul: awayMul },
    },
  };
}
...
export function computeAllProbabilities(analysis) {
  ...
  // Multiplicador heuristico — finales tienen ~8% menos goles, qualifying
  // tiene ~5% mas. Mas tarjetas en knockout (presion).
  ...
  // ASIMETRICAMENTE (el equipo que va perdiendo el global ataca mas, el que
  // gana se cierra). Los multiplicadores acumulan con los del roundType.
  ...
  const afterRound = {
    lambdaHome: Math.max(0.3, Math.min(5.0, injuryAdjusted.lambdaHome * homeAttackMul)),
    lambdaAway: Math.max(0.3, Math.min(5.0, injuryAdjusted.lambdaAway * awayAttackMul)),
  };

  // Stage 5: ajuste por importancia del partido (rotacion en final de
  // temporada para equipos ya campeon/descendido/midtable; +motivacion para
  // lucha por puesto europeo/descenso).
  const importanceAdj = applyImportanceAdjustment(
    afterRound.lambdaHome,
    afterRound.lambdaAway,
    leagueId,
    leagueRound,
    homePosition,
    awayPosition,
  );
  const lambdaHome = importanceAdj.lambdaHome;
  const lambdaAway = importanceAdj.lambdaAway;
