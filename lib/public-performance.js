  ...
  const { rows } = await pool.query(
    `WITH latest_outputs AS (
     ...
     LEFT JOIN LATERAL (
       ...
       WHERE r.sport='american-football' AND fixture_id::text=r.fixture_id ORDER BY updated_at DESC NULLS LAST LIMIT 1
     ) afm ON TRUE
...
export async function getPublicPerformance(filters, pool = pgPool) {
  ...
  const allRows = query ? [...legacy, ...ledger].filter((row) =>
    [row.league, row.homeTeam, row.awayTeam, row.marketName].some((value) => String(value || '').toLocaleLowerCase('es').includes(query))) : [...legacy, ...ledger];
  return summarizePublicPerformance(allRows, filters);
}
