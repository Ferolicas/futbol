...
async function loadLedgerRows({ from, to, sport }, pool) {
  ...
  return rows.map((row) => ({
    ...
    marketName: row.output?.name || row.output?.pick || row.market_key,
...
export function buildMarketPerformance(rows) {
  ...
  for (const row of rows) {
    ...
    const marketName = String(row.marketName || row.output?.name || row.output?.pick || row.market_key || 'Mercado sin nombre').trim();
...
export function summarizePublicPerformance(rows, { page = 1, pageSize = 12 } = {}) {
  ...
  const orderedCertified = certified.slice().reverse();
  const totalPages = Math.max(1, Math.ceil(orderedCertified.length / pageSize));
  ...
  return {
    ...
    recommendations: orderedCertified.slice(offset, offset + pageSize).map((row) => ({
      sport: row.sport, kickoff: row.kickoff, league: row.league,
      matchName: `${row.homeTeam} vs ${row.awayTeam}`, marketName: row.marketName,
      probability: row.probability, odd: row.odd, outcome: row.outcome, seal: row.seal,
    })),
    pagination: { page: safePage, pageSize, totalItems: certified.length, totalPages },
  };
}
...
export async function getPublicPerformance(filters, pool = pgPool) {
  ...
  const [legacy, ledger] = await Promise.all([
    loadLegacyRows(filters, startRows[0]?.kickoff || null, pool),
    loadLedgerRows(filters, pool),
  ]);
  const legacyMultisport = await loadLegacyMultisportRows(filters, startRows[0]?.kickoff || null, pool);
  ...
  const allRows = rows
    .filter((row) => !filters.league || contains(row.league, filters.league))
    .filter((row) => !filters.market || contains(row.marketName, filters.market))
    .filter((row) => !filters.team || contains(`${row.homeTeam} ${row.awayTeam}`, filters.team))
    .filter((row) => !query || [row.league, row.homeTeam, row.awayTeam, row.marketName].some((value) => contains(value, query)));
  return {
    ...summarizePublicPerformance(allRows, filters),
    options: {
