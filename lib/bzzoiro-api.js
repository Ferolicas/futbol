  ...
  for (const bm of bzzoiroMatches) {
    ...
    if (bestMatch && bestScore >= 80) {
      updates[bestMatch.fixture.id] = {
        ...
        elapsed: bm.elapsed || bm.minute || null,
        bzzoiroId: bm.id,
      };
    }
  }
...
export function applyLiveUpdates(fixtures, updates) {
  return fixtures.map(fix => {
    ...
    const statusMap = {
      inprogress: fix.fixture.status.elapsed <= 45 ? '1H' : '2H',
      halftime: 'HT',
      finished: 'FT',
      notstarted: 'NS',
      cancelled: 'CANC',
    ...
    const mappedStatus = statusMap[(update.status || '').toLowerCase()] || fix.fixture.status.short;
