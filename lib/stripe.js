...
// Amount is computed in real-time from the exchange rate API
export async function createPSEPayment({ plan, userId, email, name }) {
  ...
  // Get real-time USD amount, then convert to COP centavos for Stripe
  const usdAmount = plan === 'plataforma'
    ? planConfig.firstMonthPrice / 100   // $15
    : planConfig.initialPrice / 100;     // $100

  const { amount: copAmount } = await convertAmount(usdAmount, 'COP');
  // Stripe requires integer centavos; round to nearest 100 COP (=1 centavo)
  const amount = Math.round(copAmount) * 100;
