...
export function buildProbabilitiesFromContext(scored, inputs, analysis) {
  ...
  // si su promedio subyacente NO es null (mismo criterio que totalCornersAvg).
  // Ligas sin cobertura de stats (p.ej. la uruguaya: fixtures/statistics →
  // results:0) dejan estos promedios en null → esa categoría no debe emitir
  // recomendaciones ni entrar al acordeón de seleccionables (ver buildContextCombinada).
  const dataAvailability = {
    corners:  homeAvg.cornersFor != null && awayAvg.cornersFor != null,
    cards:    homeAvg.yellows    != null && awayAvg.yellows    != null,
    shots:    homeAvg.shotsFor   != null && awayAvg.shotsFor   != null,
    sot:      homeAvg.sot        != null && awayAvg.sot        != null,
    fouls:    homeAvg.fouls      != null && awayAvg.fouls      != null,
    offsides: homeAvg.offsides   != null && awayAvg.offsides   != null,
  };
...
export function buildContextCombinada(scored, odds, teamNames, playerHighlights, probabilities) {
  const allBks = odds?.allBookmakerOdds;
  // Veto por categoría SIN datos reales en el crudo: si el promedio de córners/
  // tarjetas/tiros/sot/faltas/offsides es null (categoría no cubierta por la API),
  // sus mercados NO entran ni a recomendados ni a seleccionables. Goles/resultado
  // (dataCategoryOf → null) nunca se vetan. avail viene de buildProbabilitiesFromContext.
  const avail = probabilities?.cornerCardData?.dataAvailability || {};
  const blockedByData = (key) => { const c = dataCategoryOf(key); return c != null && avail[c] === false; };
  const markets = [];
