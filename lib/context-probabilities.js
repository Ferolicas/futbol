...
export function buildProbabilitiesFromContext(scored, inputs, analysis) {
  ...
  // Hándicap asiático — shape que espera el acordeón: { home:{hp0_5,hm1_5,…}, away:{…} }.
  // El acordeón ya filtra por cuota real (hasOdd); aquí solo emitimos la prob del
  // motor para CADA línea scoreada, así Apuesta del Día y acordeón muestran lo MISMO.
  const buildAH = () => {
    const home = {}, away = {};
    for (const k of Object.keys(scored)) {
      const m = k.match(/^ah_(home|away)_([mp]\d+_\d+)$/);
      if (!m) continue;
      const v = pf(k); if (v == null) continue;
      (m[1] === 'home' ? home : away)[`h${m[2]}`] = v;
    }
    return (Object.keys(home).length || Object.keys(away).length) ? { home, away } : null;
  };
...
function resolveOddField(key) {
  ...
  if (ah) return ['asianHandicap', `${ah[1]}_${ah[2]}`];
  return null;
}
...
// atribución), NUNCA en los agregados (que son solo descriptivos). Devuelve la MEJOR
// cuota entre los bookmakers que sí la ofrecen, con su nombre. null = no recomendable.
