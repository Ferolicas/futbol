...
// Un promedio cuenta como DATO REAL solo si es no-null Y > 0. CLAVE: las ligas
// sin cobertura de stats (p.ej. la uruguaya: fixtures/statistics → results:0)
// guardan las stats vacías como 0 al finalizar el partido, así que el promedio
// colapsa a 0 (no a null). Un promedio de 0 córners/tarjetas/tiros significa que
// NO hubo datos, no que el valor real sea cero → no debe contar como disponible.
const posAvg = (v) => v != null && isFinite(v) && v > 0;
...
export function buildProbabilitiesFromContext(scored, inputs, analysis) {
  ...
  // Disponibilidad de datos REALES por categoría. Una categoría tiene datos solo
  // si ALGÚN promedio de la categoría es no-null Y > 0 (ver posAvg). Un 0 NO
  // cuenta: las ligas sin cobertura (uruguaya) guardan stats vacías como 0 al
  // finalizar y el promedio colapsa a 0. Esta categoría no debe emitir
  // recomendaciones ni entrar al acordeón (ver buildContextCombinada, que vuelve a
  // derivar esto de los promedios para ser robusto a probabilities persistidas).
  const dataAvailability = {
    corners:  posAvg(homeAvg.cornersFor) || posAvg(awayAvg.cornersFor),
    cards:    posAvg(homeAvg.yellows) || posAvg(homeAvg.reds) || posAvg(awayAvg.yellows) || posAvg(awayAvg.reds),
    shots:    posAvg(homeAvg.shotsFor) || posAvg(awayAvg.shotsFor),
    sot:      posAvg(homeAvg.sot) || posAvg(awayAvg.sot),
    fouls:    posAvg(homeAvg.fouls) || posAvg(awayAvg.fouls),
    offsides: posAvg(homeAvg.offsides) || posAvg(awayAvg.offsides),
  };
  const cornerCardData = {
    // hasRealData YA NO hardcodeado: cierto solo si hay córners O tarjetas reales.
...
function resolveOddField(key) {
  const direct = {
    ...
    dnb_home: ['dnb', 'home'], dnb_away: ['dnb', 'away'],
    ...
    wbh_home: ['winBothHalves', 'home'], wbh_away: ['winBothHalves', 'away'],
    weh_home: ['winEitherHalf', 'home'], weh_away: ['winEitherHalf', 'away'],
    hsh_1h: ['highestScoringHalf', '1h'], hsh_2h: ['highestScoringHalf', '2h'], hsh_draw: ['highestScoringHalf', 'draw'],
    home_goals_odd: ['homeOddEven', 'odd'], home_goals_even: ['homeOddEven', 'even'],
    away_goals_odd: ['awayOddEven', 'odd'], away_goals_even: ['awayOddEven', 'even'],
    home_exact_goals_0: ['homeExactGoals', '0'], home_exact_goals_1: ['homeExactGoals', '1'], home_exact_goals_2: ['homeExactGoals', '2'], home_exact_goals_3plus: ['homeExactGoals', '3plus'],
    away_exact_goals_0: ['awayExactGoals', '0'], away_exact_goals_1: ['awayExactGoals', '1'], away_exact_goals_2: ['awayExactGoals', '2'], away_exact_goals_3plus: ['awayExactGoals', '3plus'],
...
// Familia de DATOS a la que pertenece un market_key — para vetar categorías sin
// datos reales en el crudo. Goles / resultado (1X2) / BTTS / doble-oportunidad /
// first-goal / exact-goals / odd-even → null: NUNCA se vetan (siempre hay datos
// de goles y resultado). Las claves de sot ('total_sot') no contienen 'shot', así
// que no colisionan con la familia de tiros.
function dataCategoryOf(key) {
  if (/corner/.test(key))   return 'corners';   // total_corners, *_corners, most_corners_*
  if (/card/.test(key))     return 'cards';      // total_cards, *_cards, red_card_*
  if (/sot/.test(key))      return 'sot';        // total_sot, total_sot_1h/2h
  if (/shot/.test(key))     return 'shots';      // total_shots, *_shots, most_shots_*
  if (/foul/.test(key))     return 'fouls';      // total_fouls, *_fouls, most_fouls_*
  if (/offside/.test(key))  return 'offsides';   // total_offsides, *_offsides
  return null;
}

export function buildContextCombinada(scored, odds, teamNames, playerHighlights, probabilities) {
  const allBks = odds?.allBookmakerOdds;
  // Veto por categoría SIN datos reales en el crudo. Se DERIVA aquí de los
  // promedios de cornerCardData (que SIEMPRE viajan en probabilities), con la
  // regla "no-null Y > 0" (posAvg) — NO se confía en el campo dataAvailability,
  // que puede faltar si las probabilities provienen de una persistencia previa.
  // Un promedio 0 = sin datos (ligas sin cobertura guardan stats vacías como 0).
  // Goles/resultado (dataCategoryOf → null) NUNCA se vetan. Si no llega ningún
  // promedio (ccd vacío), todas las categorías de stats quedan vetadas (fail-safe).
  const ccd = probabilities?.cornerCardData || {};
  const avail = {
    corners:  posAvg(ccd.totalCornersAvg) || posAvg(ccd.homeCornersAvg) || posAvg(ccd.awayCornersAvg),
    cards:    posAvg(ccd.totalCardsAvg) || posAvg(ccd.homeYellowsAvg) || posAvg(ccd.awayYellowsAvg) || posAvg(ccd.homeRedsAvg) || posAvg(ccd.awayRedsAvg),
    shots:    posAvg(ccd.homeShotsAvg) || posAvg(ccd.awayShotsAvg),
    sot:      posAvg(ccd.homeSotAvg) || posAvg(ccd.awaySotAvg),
    fouls:    posAvg(ccd.homeFoulsAvg) || posAvg(ccd.awayFoulsAvg),
    offsides: posAvg(ccd.homeOffsidesAvg) || posAvg(ccd.awayOffsidesAvg),
  };
  ...
  for (const [key, r] of Object.entries(scored)) {
    ...
    if (blockedByData(key)) continue;    // categoría sin datos reales → fuera
  ...
  // SELECCIONABLES (acordeón "Selecciona para tu combinada"): NO exige el gate
  // recommended (≥90%). Incluye TODA línea con prob_final ≥70% y cuota real ≥1.20 en
  // bet365/bwin (con equivalencia de línea entera). Esto es lo que el usuario marca
  // para armar SU combinada manual. La "Combinada del Día" (auto) usa `selections`.
  const SELECTABLE_MIN_PROB = 0.70;
  const teamSelectable = [];
  // Embudo de observabilidad (Fase 3): cuántos mercados puntúa el motor, cuántos
  // ≥70%, y cuántos de esos NO tienen cuota real en bet365/bwin. Permite ver dónde
  // se recortan los mercados del acordeón sin adivinar (se lee vía /api/fixtures).
  let fScored = 0, fGe70 = 0, fGe70NoOdds = 0, fNoData = 0;
  for (const [key, r] of Object.entries(scored)) {
    if (key.startsWith('ah_')) continue;
    fScored++;
    if (blockedByData(key)) { fNoData++; continue; } // categoría sin datos reales → fuera
    const pf = r.prob_final;
    if (pf == null || pf < SELECTABLE_MIN_PROB) continue;
    fGe70++;
    const o = oddFor(key, odds);
    if (!o) { fGe70NoOdds++; continue; }
    teamSelectable.push({
      id: key, category: categoryOf(key), scope: 'context',
      name: marketLabel(key, teamNames), probability: clampPct(pf * 100),
      odd: o.odd, bookmaker: o.bookmaker, level: r.level, recommended: !!r.recommended,
    });
  }
  // playerSel ya viene filtrado a freq≥70 (PROB_THRESHOLD) con bookmaker atribuido.
  const selectable = [...teamSelectable, ...playerSel]
    .filter(m => m.odd > MIN_ODD - 1e-9)
    .sort((a, b) => b.probability - a.probability);

  const _funnel = {
    scored: fScored, ge70: fGe70, ge70_sinCuota: fGe70NoOdds, sinDatos: fNoData,
    teamSelectable: teamSelectable.length, playerSelectable: playerSel.length,
  ...
  // CB1 FIX: la probabilidad de que TODAS las selecciones acierten es el PRODUCTO
  // (asumiendo independencia), no el promedio. El promedio sobrestimaba el acierto
  // de la combinada (5×80% mostraba 80% cuando el real es 0.8⁵≈33%). Coherente con
  // que combinedOdd también es producto.
