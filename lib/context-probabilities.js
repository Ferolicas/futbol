...
// Disponibilidad de datos REALES por categoría. FUENTE DE VERDAD: los arrays
// *PerMatch del cornerCardData que arma extractMatchStatsData (lib/api-football.js).
// processMatch SOLO hace push cuando la estadística está presente en el crudo
// (saltea con `continue` si el valor llega null), así que un array VACÍO ⇒ esa
// categoría no tiene cobertura en la liga (p.ej. la uruguaya, con fixtures/statistics
// vacío). NO se usa el promedio: con statistics vacías colapsa a 0 y MIENTE (lo
// marcaría disponible). Pares (home, away): basta con que UNO tenga muestra.
...
export function buildProbabilitiesFromContext(scored, inputs, analysis) {
  ...
  // Disponibilidad de datos REALES por categoría — derivada de los arrays *PerMatch
  // del cornerCardData que arma extractMatchStatsData (viaja en `analysis`). Señal
  // fiable: array vacío ⇒ sin cobertura de esa estadística. (Los promedios de
  // teamAverages NO sirven: con statistics vacías colapsan a 0 y mienten.)
...
export function buildContextCombinada(scored, odds, teamNames, playerHighlights, probabilities, statsData) {
  const allBks = odds?.allBookmakerOdds;
  // Veto por categoría SIN datos reales en el crudo. FUENTE DE VERDAD: los arrays
  // *PerMatch de statsData (el cornerCardData de extractMatchStatsData, que
  // api-football pasa como 6º arg). Array vacío ⇒ categoría sin cobertura → fuera de
  // selections y selectable. Si no llega statsData (p.ej. reconstrucción desde
  // probabilities persistidas), se cae al dataAvailability ya calculado en
  // buildProbabilitiesFromContext (misma señal). Goles/resultado (dataCategoryOf →
  // null) NUNCA se vetan; categoría sin señal ⇒ vetada (fail-safe).
  const avail = statsData
    ? availabilityFromStats(statsData)
    : (probabilities?.cornerCardData?.dataAvailability || {});
  if (process.env.CONTEXT_VETO_DEBUG === 'true') {
    const n = (a) => Array.isArray(a) ? a.length : -1;
    console.log('[veto] avail=%j perMatch(home/away) corners=%s/%s cards=%s/%s shots=%s/%s sot=%s/%s',
      avail, n(statsData?.homeCornersPerMatch), n(statsData?.awayCornersPerMatch),
      n(statsData?.homeCardsPerMatch), n(statsData?.awayCardsPerMatch),
      n(statsData?.homeShotsPerMatch), n(statsData?.awayShotsPerMatch),
      n(statsData?.homeSotPerMatch), n(statsData?.awaySotPerMatch));
  }
