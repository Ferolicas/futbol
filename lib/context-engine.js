...
function _resetMlCache() { _mlCache = null; _mlCacheAt = 0; }
...
function scoreContext(contextOut, sctx = {}) {
  ...
  const useMl = mlEnabled && mlModels && mlModels.size > 0;
  ...
  for (const [key, r] of Object.entries(contextOut)) {
    ...
    let ruptureMl = 0;
    if (useMl && prob > 0 && mlModels.has(key)) {
      try {
...
module.exports = {
  ...
  buildRuptureFeatures, ruptureContext, recentPPG, loadRuptureModels, _resetMlCache, ML_FEATURE_ORDER,
