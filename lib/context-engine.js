...
// Features del modelo AGREGADO de contexto adverso (a nivel partido, no mercado).
// Estima el efecto de key_injury/knockout sobre la FIABILIDAD de la predicción
// (error medio del modelo por-mercado) controlando por la forma. Señales adversas
// = knockout, key_injury; controles = home_ppg, away_ppg. MISMO orden en train↔runtime.
const ADVERSE_FEATURE_ORDER = ['home_ppg', 'away_ppg', 'knockout', 'key_injury'];
...
// Carga (cacheada) el modelo AGREGADO de contexto adverso (una sola fila).
let _advCache = null, _advCacheAt = 0;
async function loadAdverseModel(pool) {
  const now = Date.now();
  if (_advCache !== null && now - _advCacheAt < ML_CACHE_TTL) return _advCache || null;
  try {
    const { rows } = await pool.query(
      `SELECT weights FROM prediction_models
       WHERE sport='football' AND active=TRUE AND model_type='adverse-aggregate'
       ORDER BY version DESC LIMIT 1`
    );
    _advCache = rows[0]?.weights || false; // false = cargado pero no existe
  } catch { _advCache = false; }
  _advCacheAt = Date.now();
  return _advCache || null;
}
// Uplift de ruptura por contexto adverso (nivel partido): cuánta fiabilidad EXTRA
// pierde la predicción por knockout/key_injury, vs el contrafactual sin esas señales.
// Diferencia de probabilidades de error → en [0,1], sin constantes mágicas.
function adverseRupture(adverseModel, rc) {
  if (!adverseModel || !rc) return 0;
  const advF = { home_ppg: rc.home_ppg, away_ppg: rc.away_ppg, knockout: rc.knockout, key_injury: rc.key_injury };
  const ep = predictWithModel(adverseModel, advF);
  const eb = predictWithModel(adverseModel, { ...advF, key_injury: 0, knockout: 0 });
  if (ep == null || eb == null || !isFinite(ep) || !isFinite(eb)) return 0;
  return Math.max(0, Math.min(1, ep - eb));
}
function _resetMlCache() { _mlCache = null; _mlCacheAt = 0; _advCache = null; _advCacheAt = 0; }
...
function scoreContext(contextOut, sctx = {}) {
  ...
  const useMl = mlEnabled && ((mlModels && mlModels.size > 0) || adverseModel);
  // Precómputo del contexto ML una sola vez para el partido (se reusa en todos
  // los mercados con modelo activo).
  const rc = useMl ? ruptureContext({ homeTeamRecords, awayTeamRecords, todayCtx, beforeMs: Date.now() }) : null;
  // Ruptura por contexto adverso (nivel PARTIDO) → idéntica en todos los mercados.
  // Modelo agregado: aprende con todas las muestras el efecto de key_injury/knockout
  // aunque el modelo por-mercado no lo haya captado.
  const ruptureAdv = (useMl && adverseModel) ? adverseRupture(adverseModel, rc) : 0;
  const out = {};
  for (const [key, r] of Object.entries(contextOut)) {
    ...
    // ML total = soft-OR(modelo por-mercado, modelo agregado de contexto adverso).
    const ruptureMlTotal = ruptureAdv > 0 ? (1 - (1 - ruptureMl) * (1 - ruptureAdv)) : ruptureMl;
    const rupture = ruptureMlTotal > 0 ? (1 - (1 - ruptureH2H) * (1 - ruptureMlTotal)) : ruptureH2H;

    const prob_final = +(prob * (1 - VETO_ALPHA * rupture)).toFixed(6);
    ...
    out[key] = {
      ...
      rupture_adverse: +ruptureAdv.toFixed(4),
...
module.exports = {
  ...
  loadAdverseModel, adverseRupture, ADVERSE_FEATURE_ORDER,
