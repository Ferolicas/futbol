...
// ── Shrink bayesiano de CALIBRACIÓN (Fase 7) — gated por sctx.baseRates ───────
// p_cal = (n·p + k·base)/(n+k). Pondera la frecuencia OBSERVADA por su muestra n:
// n chico → manda la base del mercado; n grande → manda lo observado. Sin
// isotónica. Validado por backtest sobre ~30k fixtures: ECE global 10.2%→0.5%.
// k POR FAMILIA: categóricos (1X2/btts/primer/timing) fuerte — el H2H es ruidoso
// con n bajo; conteo (goles/córners/tarjetas/tiros) suave — el ADN ya discrimina.
// La base la calcula scripts/compute-market-base-rates.js (→ prediction_models).
const SHRINK_K_BY_FAMILY = { resultado: 14, resultado_1h: 18, resultado_2h: 18, btts: 14, primer_gol: 12, timing_gol: 14, most_x: 10, roja: 8 };
const shrinkKFor = (fam) => (SHRINK_K_BY_FAMILY[fam] != null ? SHRINK_K_BY_FAMILY[fam] : 5);
...
// Carga (cacheada) las TASAS BASE por mercado para el shrink de calibración →
// objeto plano { market_key: rate }. Mismo patrón/TTL que los modelos ML; vive
// en prediction_models (model_type='base-rate', weights={rate,n}). Si no hay
// filas → null (el motor opera sin shrink, comportamiento previo).
const BASE_RATES_MODEL_TYPE = 'base-rate';
let _baseRatesCache = null, _baseRatesCacheAt = 0;
async function loadMarketBaseRates(pool) {
  const now = Date.now();
  if (_baseRatesCache !== null && now - _baseRatesCacheAt < ML_CACHE_TTL) return _baseRatesCache;
  let obj = null;
  try {
    const { rows } = await pool.query(
      `SELECT market_key, weights FROM prediction_models
       WHERE sport='football' AND active=TRUE AND model_type=$1`, [BASE_RATES_MODEL_TYPE]
    );
    if (rows.length) {
      obj = {};
      for (const r of rows) { const v = r.weights && r.weights.rate; if (v != null && isFinite(v)) obj[r.market_key] = v; }
      if (!Object.keys(obj).length) obj = null;
    }
  } catch { obj = null; }
  _baseRatesCache = obj;
  _baseRatesCacheAt = Date.now();
  return _baseRatesCache;
}
function _resetBaseRatesCache() { _baseRatesCache = null; _baseRatesCacheAt = 0; }
...
// recency (opcional, Fase 7c): { halfLifeDays, refMs }. Pondera cada registro por
// antigüedad — peso = 0.5^(díasDeAntigüedad/halfLife). La tasa pasa a ser media
// ponderada (forma reciente pesa más) y nEff = Σpesos (muestra EFECTIVA, < n).
function rateFromRecords(records, def, recency = null) {
  let n = 0, hits = 0, wSum = 0, wHit = 0;
  for (const r of records) {
    const a = r.actuals;
    if (!a || !def.gate(a)) continue;
    n++;
    const hit = def.outcome(a) ? 1 : 0; hits += hit;
    if (recency && recency.refMs && r.date) {
      const ageDays = Math.max(0, (recency.refMs - new Date(r.date).getTime()) / 86400000);
      const w = Math.pow(0.5, ageDays / (recency.halfLifeDays || 180));
      wSum += w; wHit += w * hit;
    }
  }
  if (recency && wSum > 0) return { n, hits, rate: wHit / wSum, nEff: wSum };
  return { n, hits, rate: n ? hits / n : null, nEff: n };
}

// L2 ADN por mercado. Pool de registros según scope (= promedio ponderado).
function l2ForMarket(key, def, homeHome, awayAway, recency = null) {
  const scope = marketScope(key, def);
  const recs = scope === 'home' ? homeHome
             : scope === 'away' ? awayAway
             : homeHome.concat(awayAway); // total → pooled (ponderado por muestra)
  const { n, hits, rate, nEff } = rateFromRecords(recs, def, recency);
  return n > 0 ? { prob: rate, level: 'adn', n, hits, nEff, exceptions: [] } : null;
}
 ...
 */
function computeContext({ homeId, awayId, meetings = [], homeRecords = [], awayRecords = [], recency = null }) {
  ...
  for (const [key, def] of Object.entries(MARKET_DEFS)) {
    ...
    // El H2H NO se pondera por recencia: la historia mutua de ESTOS dos equipos
    // es valiosa aunque sea vieja (nEff = n para que el shrink la trate igual).
    const h2h = h2hForMarket(meetings, key, homeId, awayId, null);
    if (h2h.n > 0) {
      out[key] = { prob: h2h.rate, level: 'h2h', n: h2h.n, nEff: h2h.n, hits: Math.round(h2h.rate * h2h.n), exceptions: h2h.exceptions };
      continue;
    }
    // L2 — ADN (ponderado por recencia si se pasó: forma reciente pesa más).
    const l2 = l2ForMarket(key, def, homeHome, awayAway, recency);
    if (l2) out[key] = l2;
...
function scoreContext(contextOut, sctx = {}) {
  const {
    ...
    familyModels = null, baseRates = null, shrinkK = null,
  } = sctx;
  ...
  for (const [key, r] of Object.entries(contextOut)) {
    const probNorm = normP.has(key) ? normP.get(key) : r.prob;   // 1X2/ternas → normalizada
    // ── Shrink bayesiano de CALIBRACIÓN hacia la tasa base del mercado ────────
    // Gated: solo si el caller pasó baseRates (flag CONTEXT_SHRINK_ENABLED). Toda
    // la cadena de abajo (ML, ruptura, prob_final y los DERIVADOS que leen .prob)
    // parte de este `prob` calibrado → una sola intervención calibra el universo.
    let prob = probNorm;
    const nEff = (r.nEff != null ? r.nEff : r.n);   // muestra efectiva (recencia)
    if (baseRates && baseRates[key] != null && nEff > 0) {
      const kk = (shrinkK != null ? shrinkK : shrinkKFor(marketFamily(key)));
      prob = (nEff * probNorm + kk * baseRates[key]) / (nEff + kk);
    }
    ...
    // prob_final SIN shrink (auditoría + valida el path real en el backtest).
    const probAdjRaw = useFamily ? applyFamilyShift(familyModels, key, probNorm, kiNow, koNow) : probNorm;
    const prob_final_raw = +(probAdjRaw * (1 - VETO_ALPHA * rupture)).toFixed(6);
    ...
    out[key] = {
      ...r,
      prob: +prob.toFixed(6), prob_raw: r.prob, prob_pre_shrink: +probNorm.toFixed(6), normalized: normP.has(key),
      prob_adj: +probAdj.toFixed(6), adverse_shift: +(probAdj - prob).toFixed(4),
      prob_final, prob_final_raw, rupture_score: +rupture.toFixed(4),
  ...
  // Coherencia (Fase 7b): la escalera P(>k.5) DEBE ser monótona no-creciente en k
  // (P(>0.5) ≥ P(>1.5) ≥ …). El shrink/datos pueden romper esa monotonía → masas
  // P(=k) negativas que se perdían con Math.max(0,·). Se clampa la escalera a
  // monótona ANTES de derivar, así P(=k) ≥ 0 SIEMPRE y la distribución suma 1
  // EXACTO (par/impar y nº exacto de goles quedan coherentes, no aproximados).
  const distFromLadder = (prefix, maxK = 12) => {
    ...
    const lad = [Math.min(1, Math.max(0, pov(0)))];          // P(>0.5)
    for (let k = 1; k <= maxK; k++) {
      const b = pov(k); if (b == null) break;
      lad[k] = Math.min(lad[k - 1], Math.max(0, b));         // monótona ↓
    }
    const P = [Math.max(0, 1 - lad[0])];                      // P(=0) = 1 − P(>0.5)
    for (let k = 1; k < lad.length; k++) P[k] = lad[k - 1] - lad[k]; // P(=k) ≥ 0
    P[lad.length] = lad[lad.length - 1];                      // cola P(≥ length) → suma=1
    return P;
  };
  ...
  // Gana alguna mitad (P = 1 − (1−P1ª)(1−P2ª); independencia entre mitades).
  if (out.winner_1h_home && out.winner_2h_home)
    out.weh_home = mkDerived(1 - (1 - (out.winner_1h_home.prob || 0)) * (1 - (out.winner_2h_home.prob || 0)), out.winner_1h_home, 'win_either_half');
  if (out.winner_1h_away && out.winner_2h_away)
    out.weh_away = mkDerived(1 - (1 - (out.winner_1h_away.prob || 0)) * (1 - (out.winner_2h_away.prob || 0)), out.winner_1h_away, 'win_either_half');

  // ── Par/impar + nº exacto de goles POR EQUIPO (mismo patrón exacto) ───────
  for (const [side, fam] of [['home', 'home_goals'], ['away', 'away_goals']]) {
    const base = out[`${fam}_over0_5`] || out[`${fam}_over1_5`];
    const dist = distFromLadder(fam, 8);
    if (!base || !dist) continue;
    let ev = 0, od = 0;
    for (let k = 0; k < dist.length; k++) (k % 2 === 0 ? (ev += dist[k]) : (od += dist[k]));
    const s = ev + od;
    if (s > 0) {
      out[`${side}_goals_even`] = mkDerived(ev / s, base, 'team_odd_even');
      out[`${side}_goals_odd`] = mkDerived(od / s, base, 'team_odd_even');
    }
    for (let k = 0; k <= 2; k++) if (dist[k] != null) out[`${side}_exact_goals_${k}`] = mkDerived(dist[k], base, 'team_exact_goals');
    const p3 = out[`${fam}_over2_5`]?.prob; // P(>2.5) = P(>=3)
    if (p3 != null) out[`${side}_exact_goals_3plus`] = mkDerived(p3, base, 'team_exact_goals');
  }
  return out;
}
...
async function loadContextInputs(pool, homeId, awayId) {
  ...
  const recOf = (teamId) => (f) => {
    ...
    if (!a) return null;
    // `date` para el ponderado por recencia del ADN (Fase 7c). Inofensivo si OFF.
    return { venue: (f.teams?.home?.id === teamId ? 'home' : 'away'), actuals: a, date: f?.fixture?.date };
  };
...
module.exports = {
  ...
  loadMarketBaseRates, _resetBaseRatesCache, shrinkKFor, SHRINK_K_BY_FAMILY,
