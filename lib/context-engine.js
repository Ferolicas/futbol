...
function scoreContext(contextOut, sctx = {}) {
  const { meetings = [], ctx = {}, modalXIByTeam = null, todayCtx = {}, homeId, homeRecords, awayRecords } = sctx;
  ...
  for (const [key, r] of Object.entries(contextOut)) {
    ...
    // Línea con sentido (cerca de la media real) → si no, no recomendable aunque
    // dé ≥90% (es una línea trivial; la cuota la tiraría igual en la combinada).
    ...
    out[key] = {
      ...
      prob_final, rupture_score: +rupture.toFixed(4), confidence, tradeable, recommended, causes,
    };
  }
  return out;
}
