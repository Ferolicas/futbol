...
function scoreContext(contextOut, sctx = {}) {
  ...
  for (const [key, r] of Object.entries(contextOut)) {
    ...
    const recommended = isRecommended(prob_final, rupture, r.level, r.n, confidence);
    out[key] = {
      ...r,
      prob: +prob.toFixed(6), prob_raw: r.prob, normalized: normP.has(key),
      prob_final, rupture_score: +rupture.toFixed(4), confidence, recommended, causes,
    };
  }
  return out;
}
...
module.exports = {
  ...
  persistFixtureContext, loadContextInputs,
  VETO_ALPHA, VETO_TAU, CONF_N0, REC_THRESHOLD, CAUSE_WEIGHT,
  MIN_H2H_N, MIN_ADN_CONFIDENCE, EXHAUSTIVE_TRIPLES,
};
