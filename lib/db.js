...
// PostgREST / supabase-js devuelve DATE como 'YYYY-MM-DD' literal.
// pg-node por defecto convierte DATE a JS Date (medianoche en TZ del proceso),
// lo que rompe comparaciones tipo `row.date === '2026-05-14'` en el codigo
// que asume strings ISO (sanity-cache.js, /api/fixtures, etc.).
// Forzamos a devolver el string crudo para preservar la semantica de Supabase.
types.setTypeParser(types.builtins.DATE, (val) => val);

// BIGINT (INT8, OID 20): pg-node lo devuelve como string por defecto porque
// BigInt > Number.MAX_SAFE_INTEGER (2^53) no cabe en Number sin perdida.
// PERO en cfanalisis los BIGINT son IDs de api-sports (fixture_id, team_id,
// player_id) y serials auto-generados — todos muy por debajo de 2^53.
// Sin este parser, baseball_match_analysis.fixture_id sale como string y
// rompe `Map.has(f.id)` cuando f.id viene de la API como number → todo el
// dashboard de baseball mostraba "no analizado" aunque la BD lo tuviera.
// Mismo problema afectaria a futbol si subieramos sus IDs a BIGINT.
...
function buildPool() {
  ...
  return new Pool({
    ...
    // Un socket PostgreSQL medio abierto no puede congelar una cola para
    // siempre. Ocho minutos protege jobs periódicos; puede ampliarse por env
    // para tareas maratón si alguna consulta individual lo necesita.
    // Límite del cliente pg. No usar `statement_timeout` aquí: pg lo envía
    // como startup parameter y PgBouncer lo rechaza antes de abrir la sesión.
    query_timeout: Number(process.env.DATABASE_QUERY_TIMEOUT_MS || 8 * 60_000),
  });
}
...
class PgQueryBuilder {
  ...
  async _doSelect() {
    ...
    // head=true → solo count, sin traer filas.
    ...
    // M1 FIX: cuando se pide count + datos, lanzar AMBAS queries en PARALELO
    // (antes eran secuenciales → 2× RTT a Postgres). Si no hay count, solo datos.
    let rows;
    if (this._countMode) {
      const countSql = `SELECT count(*)::bigint AS c FROM ${ident(this.table)} ${whereSql}`.trim();
