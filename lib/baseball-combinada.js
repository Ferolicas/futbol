...
const isCalibratedValue = (selection, minimumExpectedValue = 0.05) => (
  selection?.validationStatus === 'calibrated'
  && Number.isFinite(Number(selection?.expectedValue))
  && Number(selection.expectedValue) + Number.EPSILON >= minimumExpectedValue
);
...
export function buildBaseballApuestaDelDia(analyzedGames, {
  ...
  for (const g of analyzedGames || []) {
    ...
    for (const s of catalog) {
      ...
      if (!isCalibratedValue(s, 0.10)) continue;
      ...
      candidates.push({
        ...
        priority,
        line: s.line ?? s._line,
        _line: s._line ?? s.line,
        side: s.side,
        expectedValue: Number(s.expectedValue),
  ...
  const ranked = candidates
    ...
    .sort((a, b) => (b.priority - a.priority)
      || (Number(b.expectedValue) - Number(a.expectedValue))
...
export function buildCustomBaseballCombinada(selectedMarkets, gamesById) {
  ...
  for (const [fid, markets] of Object.entries(selectedMarkets || {})) {
    ...
    // En Gratis el servidor entrega una única selección ya cruzada con cuota
    // Bet365 real y omite el catálogo Pro. Releer esa misma selección mantiene
    // la protección contra estado viejo sin exigir el payload bloqueado.
    const freeSelection = game.analysis?.access === 'free'
      ? game.analysis?.freePreview?.selection
      : null;
    const currentCatalog = freeSelection
      ? [freeSelection]
      : Array.isArray(game.analysis?.combinada?.selectable)
        ? game.analysis.combinada.selectable
        : [];
    for (const [key] of Object.entries(markets).slice(0, 1)) {
      ...
      if (!isBettable(m) || !isCalibratedValue(m)) continue;
      selections.push({
        matchName,
