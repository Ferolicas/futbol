 ...
 *              fiabilidad >= 90.
 *   Béisbol  → todos los mercados calculados de ganador, carreras
 *              (partido/equipo/entradas), hándicap, hits, bateadores y
 *              lanzadores; probabilidad >= 70 y fiabilidad >= 90, aunque no
 *              exista cuota.
 *
...
export const BASEBALL_PREMIUM_RULES = Object.freeze({
  minProbability: 70,
  minReliability: 90,
  groups: ['ganador', 'carreras', 'handicap', 'hits', 'bateadores', 'lanzadores', 'especiales'],
});
...
export function classifyBaseballPremiumOption(selection) {
  ...
  if (category === 'moneyline' || /-moneyline$/.test(category)) return 'ganador';
  ...
  if (/^player-strikeouts-/.test(category)) return 'lanzadores';
  if (/^player-/.test(category)) return 'bateadores';
  if (/^special-/.test(category)) return 'especiales';
  return null;
}
...
export function buildBaseballPremiumCatalog(row) {
  ...
  for (const [metric, players] of Object.entries(probabilities.players || {})) {
    const family = metric === 'strikeouts' ? 'lanzadores' : 'bateadores';
