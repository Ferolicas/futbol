...
export function buildBaseballPremiumCatalog(row) {
  ...
  const periods = model.periods && Object.keys(model.periods).length
    ? model.periods
    : (probabilities.periods && Object.keys(probabilities.periods).length
      ? probabilities.periods
    ...
    for (const [metric, scopes] of Object.entries(period?.statistics || {})) {
      ...
      for (const scope of ['total', 'home', 'away']) {
        ...
        for (const [line, values] of Object.entries(scopes?.[scope] || {})) {
          for (const side of ['over', 'under']) add({
            id: `${periodKey}-stat-${metric}-${scope}-${line}-${side}`,
            category: `${periodKey}-stat-${metric}-${scope}-${line}`,
            family: 'hits',
            name: `${scopeName}: ${side === 'over' ? 'más' : 'menos'} de ${line} hits en ${label}`,
            entry: values?.[side], side, line: Number(line),
          });
        }
      }
