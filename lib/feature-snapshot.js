...
//
// Lo usan los scripts históricos (scripts/backfill-features.js y
// reenrich-features.js) que reconstruyen un `analysis` equivalente con datos del
// MOMENTO del partido. (La ruta en vivo _savePrediction se retiró con la purga
// Dixon-Coles; el ML actual entrena desde raw_api_payloads, no desde este snapshot.)
//
...
function buildFeatureSnapshot(analysis, probs = {}) {
  ...
  return {
    ...
    // Goles esperados = promedio GENERAL real del equipo (motor de contexto),
    // NO lambdas Dixon-Coles (purgado). En fútbol DC ya no existe; estas keys
    // salen de las frecuencias reales o quedan null si no hay contexto.
    base: {
      expGoalsHome: r2(probs.homeGoals?.avgScored ?? a.calculatedProbabilities?.homeGoals?.avgScored),
      expGoalsAway: r2(probs.awayGoals?.avgScored ?? a.calculatedProbabilities?.awayGoals?.avgScored),
    },
  };
}
