...
import { BASEBALL_RECOMMENDATION_MIN_PROBABILITY } from './recommendation-policy.js';
...
// Líneas ESTÁNDAR que el almacenamiento (flattenProbabilitiesForStorage) y la
// calibración (calibration_baseball_v1) esperan SIEMPRE. El modelo las computa
// además de las adaptativas para que esos mercados nunca queden null/sin datos.
// BUG QUE RESUELVE: las líneas adaptativas (8.0, 9.0) no coincidían con las
// fijas que persiste flatten (8.5, 9.5) → p_total_over_85/95 y team_total_*_35
// salían null → calibración "insufficient" para siempre.
const TOTAL_STD_LINES = [7.5, 8.5, 9.5, 10.5];
const F5_STD_LINES = [4.5, 5.5];
const TEAM_TOTAL_STD_LINES = [3.5, 4.5];

// Une líneas adaptativas + estándar, sin duplicados, ordenadas.
function mergeLines(adaptive, std) {
  const set = new Set([...(adaptive || []), ...std]);
  return [...set].filter(l => l > 0).sort((a, b) => a - b);
}
...
// =====================================================================
// TEAM PITCHING STRENGTH (factor del pitcheo del equipo) — fallback cuando
// `pitcherMatchup` (starter individual desde MLB Stats API) no está disponible.
// Devuelve un factor 0.85-1.15 que escala las carreras esperadas DEL RIVAL:
...
function teamPitchingFactor(stats, leagueAvg = LEAGUE_AVG_RUNS_PER_GAME / 2) {
  ...
  // El campo `points.against.average.total` es la media de carreras encajadas
  // por partido. Si su pitcheo es bueno, este número está por debajo del
  // league average.
...
export function computeBaseballProbabilities({
  ...
  awayId,
  h2h,
  marketOdds,            // raw odds object from API (legacy api-baseball)
  marketMoneyline = null, // { home, away } decimal odds ya normalizado (The Odds API). Prioritario.
  parkFactor = 1.0,
  ...
  // Regularización hacia 0.5 (incertidumbre del béisbol). El modelo Poisson con
  // factores de pitcher fuertes produce probabilidades muy extremas (ej. 86-89%)
  // que NO reflejan la varianza real del béisbol — las casas rara vez exceden
  // ~70-72% incluso en los matchups más desequilibrados. Encogemos hacia 0.5
  // para que el favorito máximo realista ronde ~78%. Cuando hay odds de mercado,
  // el blend de abajo afina aún más; y la calibración isotónica corregirá el
  // resto a medida que se acumulen resultados reales. REG=0.78 es conservador.
  const MONEYLINE_REG = 0.78;
  pHome = 0.5 + (pHome - 0.5) * MONEYLINE_REG;
  pAway = 1 - pHome;

  // Blend with market (de-vig) when available.
  // Prioriza marketMoneyline normalizado (The Odds API); fallback al raw legacy.
  const mlMarket = marketMoneyline
    || (marketOdds ? extractMoneylineOdds(marketOdds) : null);
  const mlDevig = (mlMarket && mlMarket.home && mlMarket.away)
    ? deVigBinary(mlMarket.home, mlMarket.away) : null;
  if (mlDevig) {
  ...
  const totalLines = mergeLines(adaptiveBaseballLines(expectedTotal, 5, 4), TOTAL_STD_LINES);
  const totals = {};
  ...
  const f5TotalLines = mergeLines(adaptiveBaseballLines(f5Expected, 3, 2), F5_STD_LINES);
  const f5Totals = {};
  ...
  const homeTeamLines = mergeLines(adaptiveBaseballLines(lambdaHome, 3, 2), TEAM_TOTAL_STD_LINES);
  const awayTeamLines = mergeLines(adaptiveBaseballLines(lambdaAway, 3, 2), TEAM_TOTAL_STD_LINES);
...
// =====================================================================
// MARKET ODDS EXTRACTION (legacy multi-bookmaker format) — solo lo usa
// `extractMoneylineOdds` para el fallback raw `marketOdds`. The Odds API se
// extrae con `extractBaseballEventOdds` en lib/odds-api.js.
// =====================================================================
...
// El baremo público de Baseball es ≥70% tanto para el catálogo como para
// los destacados de partido. La Apuesta del Día conserva su gate independiente
// de 90% en baseball-combinada.js.
// MIN_ODD = 1.20 garantiza que apuestas combinadas tengan rendimiento mínimo.
const BASEBALL_SELECTABLE_THRESHOLD = BASEBALL_RECOMMENDATION_MIN_PROBABILITY;
const BASEBALL_SELECTIONS_THRESHOLD = BASEBALL_RECOMMENDATION_MIN_PROBABILITY;
const BASEBALL_MIN_ODD = 1.20;
// Compat retro: shape antiguo usaba `threshold` plano. Reportamos el del
// selectable (gate más laxo) para mantener consumidores existentes.
const BASEBALL_PROB_THRESHOLD = BASEBALL_SELECTABLE_THRESHOLD;
...
export function buildBaseballCombinada(probabilities, bestOdds, teamNames = {}) {
  ...
  // Dedup, filter, sort. Generamos DOS listas:
  //   selectable = todo lo que pasa el gate más laxo (≥70%) + cuota ≥1.20
  //   selections = destacados del partido con el mismo baremo público (≥70%)
  const deduplicated = deduplicateBaseballByCategory(all);
  const selectable = deduplicated
    .filter(m => m.probability >= BASEBALL_SELECTABLE_THRESHOLD)
    .sort((a, b) => b.probability - a.probability);
  const selections = selectable.filter(m => m.probability >= BASEBALL_SELECTIONS_THRESHOLD);

  if (selectable.length === 0) {
    return {
      selections: [], selectable: [], combinedOdd: null, combinedProbability: 0,
      hasRealOdds: false, threshold: BASEBALL_PROB_THRESHOLD,
      selectableThreshold: BASEBALL_SELECTABLE_THRESHOLD,
      selectionsThreshold: BASEBALL_SELECTIONS_THRESHOLD,
      totalMarkets: all.length, deduplicatedMarkets: deduplicated.length,
    };
  }

  // La cuota combinada y combinedProbability se calculan sobre `selections`;
  // si está vacía, sobre `selectable` (fallback para el chip resumen).
  const reportSet = selections.length > 0 ? selections : selectable;
  const combinedOdd = reportSet.reduce((acc, m) => acc * (m.odd || 1), 1);
  // CB1 FIX: producto (P de que TODAS acierten), no promedio — coherente con
  // buildBaseballApuestaDelDia (que ya usaba producto) y con combinedOdd.
  const combinedProbability = reportSet.reduce((acc, m) => acc * (m.probability / 100), 1) * 100;
  ...
  return {
    ...
    hasRealOdds: reportSet.every(s => s.odd),
...
function buildBaseballPlayerProbabilities(playerHighlights) {
  ...
  const built = Object.keys(players);
  // Decisivo: qué categorías de jugador se construyeron y con cuántos. Si solo
  // sale 'strikeouts', el lineup de bateadores vino vacío para ESE juego (MLB no
  // lo había posteado al analizar) — no es drop del modelo.
  console.log('[bb-props] buildProbs → categorías: %s (n: %s)',
    built.join(',') || 'NINGUNA', built.map(k => players[k].length).join('/') || '-');
  return built.length > 0 ? players : null;
}

// Los extractores legacy `extractBaseballPitcherMatchup` y
// `extractBaseballPlayerHighlights` se eliminaron al purgar api-baseball.
// Las implementaciones reales viven en lib/mlb-stats-api.js
// (`getMlbPitcherMatchup` y `extractBaseballPlayerHighlights`).
