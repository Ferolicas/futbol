...
async function fetchFromOrigin(id) {
  try {
    const res = await fetch(ORIGIN(id), {
      cache: 'no-store',
      redirect: 'error',
      signal: AbortSignal.timeout(8000),
    });
    if (!res.ok || res.headers.get('content-type')?.split(';')[0] !== 'image/png' || !res.body) return null;
    ...
    // Firma PNG; no persistir HTML u otro contenido aunque el origen mienta.
    if (body.length < 8 || !body.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]))) return null;
    return body;
  } catch {
...
async function writeAtomic(id, body) {
  await ensureDir();
  const tmp = path.join(PHOTO_DIR, `.${id}.${randomUUID()}.tmp`);
  try {
    await fs.writeFile(tmp, body, { flag: 'wx', mode: 0o644 });
