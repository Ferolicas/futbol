...
// TOPE DIARIO DURO de requests a The Odds API. The Odds API free = 500/mes
// (~16/día). 15/día = 450/mes con margen. Este guard es el punto ÚNICO por
// donde pasan TODAS las llamadas (cada fetchOddsForSport = 1 request por liga/
// sport), así que GARANTIZA no pasar de 500/mes sin importar cuántas ligas o
// cuántas veces se llame. Las cuotas son referencia (no críticas para predecir),
// así que cuando se agota el tope se reutilizan las cacheadas.
...
async function bumpDailyReqCount(n = 1) {
  try {
    const k = reqCountKey();
    const cur = Number(await redisGet(k)) || 0;
    await redisSet(k, cur + n, 48 * 3600);
  } catch {}
}
...
export async function fetchOddsForSport(sportKey, options = {}) {
  ...
  try {
    ...
    // Cuenta el request (1 por sport key) ANTES de validar la respuesta: The
    // Odds API ya descontó el request aunque devuelva error.
    await bumpDailyReqCount(1);
