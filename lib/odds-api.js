...
import { redisGet, redisIncrBy } from './redis';

const BASE_URL = 'https://api.the-odds-api.com/v4';

// TOPE DIARIO DURO de CRÉDITOS a The Odds API (1 crédito = 1 región × 1 mercado).
// Free = 500/mes (~16/día). 120 créditos/día ≈ 3600/mes — OJO: si estás en plan
// free, baja este número. Con regiones eu,uk (OA4) cada llamada cuesta 4 (fútbol)
// o 6 (baseball) créditos, así que 120 cubre ~20-30 llamadas/día. Las cuotas son
// referencia (no críticas), así que al agotar el tope se reutilizan las cacheadas.
const DAILY_REQUEST_CAP = 120;
...
export async function fetchOddsForSport(sportKey, options = {}) {
  ...
  // las cuotas cacheadas). Garantiza ≤450/mes entre fútbol y baseball.
  const usedToday = await getDailyReqCount();
  if (usedToday >= DAILY_REQUEST_CAP) {
    console.warn(`[ODDS-API] tope diario alcanzado (${usedToday}/${DAILY_REQUEST_CAP}) — skip ${sportKey}, uso cache`);
  ...
  try {
    ...
    const credits = String(regions).split(',').filter(Boolean).length
                  * String(markets).split(',').filter(Boolean).length;
