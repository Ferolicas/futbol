...
export async function fetchOddsForFixtures(fixtures) {
  ...
  // Cost: 1 per region per market. With regions=eu and markets=h2h,totals = 2 credits per sport
  // We use a single region to minimize cost
  for (const sportKey of sportKeys) {
    const { events, remaining, used } = await fetchOddsForSport(sportKey, {
      markets: 'h2h,totals',
      regions: 'eu', // Single region to save quota (1 credit per market = 2 per sport)
    });
