...
export async function createOrder({
  ...
  const firstName = String(details.firstName || payer.first_name || '').trim().slice(0, 32);
  const lastName = String(details.lastName || payer.last_name || '').trim().slice(0, 32);
  ...
  if (pmId === 'pse') {
    payerBody.phone = {
      area_code: String(details.phoneAreaCode || ''),
      number: String(details.phoneNumber || ''),
    };
    payerBody.address = {
