...
// ── Pago de UNA VEZ (PSE/Efecty) vía Orders API ──────────────────────────────
// Los métodos de transferencia bancaria (PSE/Nequi) y efectivo (Efecty) NO se
// pueden cobrar de forma recurrente: son pago por periodo. Creamos una order y,
// para PSE, MP devuelve la redirect_url al banco (paso final obligatorio del
// método; ver doc "Disponibilizar el pago"). external_reference = userId para
// casar el pago con la cuenta en el webhook.
export async function createOrder({ plan, formData, userId, backUrl, ipAddress }) {
  ...
  const financialInstitution =
    ...
    || fd.additional_info?.financial_institution
    || payer.financial_institution;

  const body = {
    type: 'online',
    total_amount: String(amountCop),
    ...
    transactions: {
      payments: [{
        ...
        payment_method: {
          id: pmId,
          type: fd.payment_type_id || 'bank_transfer',
          ...(financialInstitution ? { financial_institution: String(financialInstitution) } : {}),
        },
      }],
    },
    additional_info: { 'payer.ip_address': ipAddress || '0.0.0.0' },
    config: { online: { callback_url: backUrl } },
  };
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    throw new Error(`MP order ${res.status}: ${JSON.stringify(data).slice(0, 300)}`);
  }
  const redirectUrl = data?.transactions?.payments?.[0]?.payment_method?.redirect_url || null;
