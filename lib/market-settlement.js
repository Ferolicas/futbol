...
function scorePair(sport, game, live) {
  if (sport === 'football') {
    return {
      // En AET/PEN las casas liquidan los mercados ordinarios a 90 minutos.
      // El fulltime oficial gana sobre el marcador que ya incluye prórroga.
      home: finite(live?.score?.fulltime?.home ?? game?.score?.fulltime?.home ?? live?.goals?.home ?? game?.goals?.home),
      away: finite(live?.score?.fulltime?.away ?? game?.score?.fulltime?.away ?? live?.goals?.away ?? game?.goals?.away),
    };
  }
  if (sport === 'baseball') {
    return {
...
function compareEuropeanHandicap(home, away, side, line, complete) {
  if (!complete || home == null || away == null || finite(line) == null) return pending();
  const adjustedHome = home + Number(line);
  const actual = adjustedHome === away ? 'draw' : adjustedHome > away ? 'home' : 'away';
  return actual === side ? won(`${adjustedHome}-${away}`) : lost(`${adjustedHome}-${away}`);
}
...
function settleFootball(selection, game, live, status) {
  ...
  // Hándicap asiático canónico del motor: ah_home_p1_5 = local +1.5,
  // ah_home_m0_5 = local -0.5. El signo forma parte del ID porque el catálogo
  // conserva la línea exacta que publicó Bet365.
  const asianHandicap = id.match(/^ah_(home|away)_(m|p)(\d+(?:_\d+)?)$/);
  if (asianHandicap) {
    const magnitude = Number(asianHandicap[3].replace('_', '.'));
    const line = asianHandicap[2] === 'm' ? -magnitude : magnitude;
    return compareHandicap(score.home, score.away, asianHandicap[1], line, final);
  }

  // El hándicap europeo es de tres vías: si el marcador ajustado empata, una
  // selección Home/Away pierde (no es push como en el asiático).
  const europeanHandicap = id.match(/^eh_(home|away|draw)_(m|p)(\d+(?:_\d+)?)$/);
  if (europeanHandicap) {
    const magnitude = Number(europeanHandicap[3].replace('_', '.'));
    const line = europeanHandicap[2] === 'm' ? -magnitude : magnitude;
    return compareEuropeanHandicap(score.home, score.away, europeanHandicap[1], line, final);
  }
  ...
  const playerLine = id.match(/^(shotsTotal|shotsOn|fouls)-(\d+)-([\d.]+)$/);
  if (playerLine) {
    const stats = live?.player_stats?.[playerLine[2]]?.stats
      || live?.playerStats?.[playerLine[2]]?.stats
      || live?.player_stats?.[playerLine[2]]
      || live?.playerStats?.[playerLine[2]];
    const aliases = {
      shotsTotal: ['shots_total', 'shotsTotal', 'shots'],
      shotsOn: ['shots_on', 'shotsOn', 'shotsOnTarget'],
      fouls: ['fouls_committed', 'foulsCommitted', 'fouls'],
    }[playerLine[1]];
    const value = aliases.map((key) => finite(stats?.[key])).find((entry) => entry != null) ?? null;
    return compareLine(value, Number(playerLine[3]), 'over', final);
  }
...
function genericPeriodPair(game, periodKey) {
  ...
  // NCAA Basketball guarda dos mitades; NBA/NFL guardan cuatro cuartos.
  if (periodKey === 'firstHalf' || periodKey === 'halves') return pick(home.length <= 2 ? [0] : [0, 1]);
  if (periodKey === 'secondHalf') return pick(home.length <= 2 ? [1] : [2, 3]);
...
function baseballPeriod(live, periodKey, final) {
  ...
  const range = String(periodKey).match(/^first(3|5|7)$/i)?.[1];
  if (range) {
    const end = Number(range);
    return { pair: baseballInningPair(live, 1, end), complete: final || inning > end };
  }
  if (/^first4_?5$/i.test(String(periodKey))) {
    const firstFour = baseballInningPair(live, 1, 4);
    const fifth = baseballInningPair(live, 5, 5);
    const pair = {
      // 4.5 entradas termina tras la parte alta de la quinta: solo la carrera
      // visitante de esa entrada pertenece al mercado.
      home: firstFour.home,
      away: firstFour.away == null || fifth.away == null ? null : firstFour.away + fifth.away,
    };
    return { pair, complete: final || inning > 5 || (inning === 5 && live?.inning_half === 'bottom') };
  }
...
function baseballPlayerValue(live, playerId, metric) {
  const stats = live?.player_stats?.[String(playerId)]?.stats
    || live?.playerStats?.[String(playerId)]?.stats
    || live?.player_stats?.[String(playerId)]
    || live?.playerStats?.[String(playerId)]
    || null;
  ...
  const key = aliases[String(metric || '').toLowerCase()] || metric;
...
function settleMultisport(sport, selection, game, live, status) {
  ...
  const player = sport === 'baseball' && id.match(/^player-([a-zA-Z]+)-(\d+)-/);
  if (player && line != null) {
    const value = baseballPlayerValue(live, player[2], player[1]);
    // Si el jugador no tiene aparición oficial, la casa normalmente anula el
    // prop. No lo presentamos como derrota inventada.
    if (value == null && final) return voided();
    return compareLine(value, line, side, final);
  }
