...
// Fresh GROQ query bypassing CDN — guarantees post-write consistency
export async function queryFromSanityFresh(query, params = {}) {
...
// Patch specific fields on an existing document (does NOT replace the full doc)
export async function patchSanity(type, id, fields) {
  if (!client || !token) return false;
  const docId = `${type}-${id}`;
  try {
    ...
    console.error(`Sanity patch error [${docId}]:`, e.message);
    throw e;
  }
}
