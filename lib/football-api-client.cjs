...
async function enforceCriticalReserve(path, priority) {
  ...
  try {
    const raw = await getRedis().get(LAST_REMAINING_KEY);
    if (raw == null) return;
    const remaining = Number(raw);
    if (Number.isFinite(remaining) && remaining <= DAILY_CRITICAL_RESERVE) {
      throw new FootballApiError(
        `API-Football: reserva crítica protegida (${remaining} restantes)`,
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (Number.isFinite(remaining)) {
        try {
          await getRedis().set(LAST_REMAINING_KEY, String(remaining), 'EX', 26 * 3600);
