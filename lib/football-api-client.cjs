...
// Cuando la cuota diaria se agota, todos los endpoints devuelven el mismo error.
// API-SPORTS la reinicia a las 00:00 UTC; el circuito compartido despierta cinco
// segundos después. Puede sobrescribirse por env para una cuenta no estándar.
...
function dailyLimitCooldownMs(nowMs = Date.now()) {
  ...
  return Math.max(10_000, resetAt - nowMs);
}
