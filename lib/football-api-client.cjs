...
// Un cooldown compartido evita que cada job siga gastando sockets y reintentos;
// se prueba de nuevo al minuto para recuperar rápido tras reset o upgrade.
const DAILY_LIMIT_COOLDOWN_MS = Math.max(10_000, Number(process.env.FOOTBALL_API_DAILY_LIMIT_COOLDOWN_MS || 60_000));
...
async function openDailyLimitCircuit() {
  try {
    await getRedis().set(DAILY_LIMIT_KEY, '1', 'PX', DAILY_LIMIT_COOLDOWN_MS);
  } catch {}
}
