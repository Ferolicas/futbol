...
const DAILY_LIMIT_KEY = 'ratelimit:api-football:daily-exhausted';
const DAILY_PROBE_KEY = 'ratelimit:api-football:daily-probe';
const LAST_REMAINING_KEY = 'ratelimit:api-football:remaining';
const LAST_DAILY_LIMIT_KEY = 'ratelimit:api-football:daily-limit';
...
// La API puede renovar la cuota en una hora distinta a 00:00 UTC (depende de
// la cuenta/proveedor). Un circuito hasta la siguiente medianoche dejó el live
// bloqueado durante horas aunque /status ya mostraba 74.998 llamadas libres.
// Revalidamos como máximo cada 5 min; las prioridades críticas hacen una sonda
// distribuida una vez por minuto para recuperar el live apenas vuelva la API.
const CONFIGURED_DAILY_LIMIT_COOLDOWN_MS = Number(process.env.FOOTBALL_API_DAILY_LIMIT_COOLDOWN_MS || 0);
const DAILY_LIMIT_RECHECK_MS = Math.max(60_000, Number(process.env.FOOTBALL_API_DAILY_RECHECK_MS || 5 * 60_000));
const DAILY_PROBE_INTERVAL_MS = Math.max(20_000, Number(process.env.FOOTBALL_API_DAILY_PROBE_MS || 60_000));
// Reserva diaria para el negocio crítico: calendario, marcadores live y cierre
// de resultados. El análisis/enriquecido opcional se detiene antes de agotarla.
// `FOOTBALL_API_CRITICAL_RESERVE` es un TECHO, no una cifra rígida. La cuenta
// puede cambiar de plan (Mega 150k -> Pro 7.5k): reservar siempre 20k bloquearía
// todo el análisis aun con la suscripción activa. Conservamos por defecto el
// 15% del límite diario real, con suelo de 1.000 y techo de 20.000.
const DAILY_CRITICAL_RESERVE = Math.max(0, Number(process.env.FOOTBALL_API_CRITICAL_RESERVE || 20_000));
const DAILY_CRITICAL_RESERVE_MIN = Math.max(0, Number(process.env.FOOTBALL_API_CRITICAL_RESERVE_MIN || 1_000));
const parsedReserveRatio = Number(process.env.FOOTBALL_API_CRITICAL_RESERVE_RATIO || 0.15);
const DAILY_CRITICAL_RESERVE_RATIO = Number.isFinite(parsedReserveRatio)
  ? Math.max(0, Math.min(1, parsedReserveRatio))
  : 0.15;
const CRITICAL_PRIORITIES = new Set(['live', 'fixtures', 'results']);
function dailyLimitCooldownMs(nowMs = Date.now()) {
  if (Number.isFinite(CONFIGURED_DAILY_LIMIT_COOLDOWN_MS) && CONFIGURED_DAILY_LIMIT_COOLDOWN_MS > 0) {
    return Math.max(10_000, CONFIGURED_DAILY_LIMIT_COOLDOWN_MS);
  }
  const now = new Date(nowMs);
  const resetAt = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + 1, 0, 0, 5);
  return Math.max(10_000, Math.min(DAILY_LIMIT_RECHECK_MS, resetAt - nowMs));
}
...
function classifyApiErrorText(errorText) {
  const text = String(errorText || '').trim();
  if (/request limit for the day|daily (?:request )?(?:quota|limit)|quota[^.]*\bday\b|limit[^.]*per day/i.test(text)) {
    return { code: 'DAILY_LIMIT', retryable: false };
  }
  if (/rate|request|limit|too many/i.test(text)) {
    return { code: 'RATE_LIMIT', retryable: true };
  }
  return { code: 'API_BODY_ERROR', retryable: false };
}

async function dailyLimitCircuitRemaining() {
  try {
    const ttl = Number(await getRedis().pttl(DAILY_LIMIT_KEY));
...
function isCriticalPriority(priority) {
  return CRITICAL_PRIORITIES.has(String(priority || '').toLowerCase());
}

function criticalReserveForDailyLimit(limit, remaining = null) {
  if (DAILY_CRITICAL_RESERVE <= 0) return 0;
  const numericLimit = Number(limit);
  const numericRemaining = Number(remaining);
  // Tras el primer request exitoso conocemos el límite contractual exacto.
  // Mientras aún no exista, el remaining permite salir de una migración de
  // plan: se autoriza una llamada estándar y su respuesta ya fija el límite.
  const basis = Number.isFinite(numericLimit) && numericLimit > 0
    ? numericLimit
    : Number.isFinite(numericRemaining) && numericRemaining > 0 && numericRemaining <= DAILY_CRITICAL_RESERVE
      ? numericRemaining
      : null;
  if (basis == null) return DAILY_CRITICAL_RESERVE;
  const floor = Math.min(DAILY_CRITICAL_RESERVE_MIN, basis);
  return Math.min(
    DAILY_CRITICAL_RESERVE,
    basis,
    Math.max(floor, Math.floor(basis * DAILY_CRITICAL_RESERVE_RATIO)),
  );
}

async function reserveDailyProbe(priority) {
  if (!isCriticalPriority(priority)) return false;
  try {
    const result = await getRedis().set(DAILY_PROBE_KEY, '1', 'PX', DAILY_PROBE_INTERVAL_MS, 'NX');
    return result === 'OK';
  } catch {
    return false;
  }
...
async function enforceCriticalReserve(path, priority) {
  if (isCriticalPriority(priority) || DAILY_CRITICAL_RESERVE <= 0) return;
  try {
    const [rawRemaining, rawLimit] = await getRedis().mget(LAST_REMAINING_KEY, LAST_DAILY_LIMIT_KEY);
    if (rawRemaining == null) return;
    const remaining = Number(rawRemaining);
    const limit = rawLimit == null ? null : Number(rawLimit);
    const reserve = criticalReserveForDailyLimit(limit, remaining);
    if (Number.isFinite(remaining) && remaining <= reserve) {
      throw new FootballApiError(
        `API-Football: reserva crítica protegida (${remaining} restantes; reserva ${reserve})`,
        { code: 'DAILY_RESERVE', retryable: false, path },
      );
    }
  } catch (error) {
    if (error instanceof FootballApiError) throw error;
    // Redis caído: el limitador por minuto ya cae a modo conservador. No
    // bloqueamos toda la aplicación por no poder leer el contador restante.
  }
}

async function openDailyLimitCircuit() {
  try {
    await getRedis().set(DAILY_LIMIT_KEY, '1', 'PX', dailyLimitCooldownMs());
  } catch {}
}
...
async function footballApiRequest(path, options = {}) {
  ...
  const dailyCooldown = await dailyLimitCircuitRemaining();
  const probingDailyLimit = dailyCooldown > 0 && await reserveDailyProbe(priority);
  if (dailyCooldown > 0 && !probingDailyLimit) {
    throw new FootballApiError('API-Football: cuota diaria agotada (circuito en espera)', {
      code: 'DAILY_LIMIT', retryable: false, path, retryAfterMs: dailyCooldown,
    });
  }
  await enforceCriticalReserve(path, priority);
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (errorText) {
        const classification = classifyApiErrorText(errorText);
        if (classification.code === 'DAILY_LIMIT') await openDailyLimitCircuit();
        throw new FootballApiError(`API: ${errorText}`, {
          code: classification.code, retryable: classification.retryable, path,
        });
      }
      const remainingHeader = response.headers.get('x-ratelimit-requests-remaining');
      const dailyLimitHeader = response.headers.get('x-ratelimit-requests-limit');
      const remaining = remainingHeader == null ? null : Number(remainingHeader);
      const dailyLimit = dailyLimitHeader == null ? null : Number(dailyLimitHeader);
      if (Number.isFinite(remaining)) {
        try {
          const tx = getRedis().multi().set(LAST_REMAINING_KEY, String(remaining), 'EX', 26 * 3600);
          if (Number.isFinite(dailyLimit) && dailyLimit > 0) {
            tx.set(LAST_DAILY_LIMIT_KEY, String(dailyLimit), 'EX', 26 * 3600);
          }
          await tx.exec();
          if (remaining > 0 && probingDailyLimit) await getRedis().del(DAILY_LIMIT_KEY);
        } catch {}
      }
      if (remaining === 0) await openDailyLimitCircuit();
      return {
        payload,
...
module.exports = {
  ...
  DAILY_CRITICAL_RESERVE,
  DAILY_CRITICAL_RESERVE_MIN,
  DAILY_CRITICAL_RESERVE_RATIO,
