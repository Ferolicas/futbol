...
const CACHE_VERSION = 12; // probabilidad cruda para decisiones + cap 95 solo visual, sin gate
...
export function buildMultisportCombinada(prediction, odds, game, options = {}) {
  ...
  const add = ({ id, category, name, probability, odd, validationKey }) => {
    ...
    candidates.push({ id, category, market: category, name, pick: name, probability: p, rawProbability, odd: o, validationKey, statisticalRecommendation: true });
  };
