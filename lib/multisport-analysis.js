...
const CACHE_VERSION = 18; // Bet365 exacto + catálogo NBA/NFL >60% y fiabilidad >=90%
...
const MULTISPORT_MIN_PROBABILITY = 60;
const MULTISPORT_MIN_RELIABILITY = 90;
...
function playerProbability(player, line, side) {
  ...
  const evidence = side === 'under' && source && Number.isFinite(Number(source.n))
    ? { ...source, hits: Number(source.n) - Number(source.hits || 0), rawProbability }
    : source;
  return {
...
export function buildMultisportCombinada(prediction, odds, game, options = {}) {
  ...
  const selectableThreshold = options.selectableThreshold
    ?? (isBaseball ? BASEBALL_RECOMMENDATION_MIN_PROBABILITY : MULTISPORT_MIN_PROBABILITY);
  const highlightThreshold = options.highlightThreshold
    ?? (isBaseball ? BASEBALL_RECOMMENDATION_MIN_PROBABILITY : MULTISPORT_MIN_PROBABILITY);
  ...
  const reliabilityThreshold = options.reliabilityThreshold
    ?? (isBaseball ? BASEBALL_DAILY_MIN_PROBABILITY : MULTISPORT_MIN_PROBABILITY);
  const minimumReliability = options.minimumReliability
    ?? (isBaseball ? null : MULTISPORT_MIN_RELIABILITY);
  const candidates = [];
  const add = ({ id, category, marketLabel, name, probability, odd, validationKey, scope = 'match', line = null, side = null }) => {
    ...
    const passesProbability = isBaseball
      ? rawProbability >= selectableThreshold
      : rawProbability > selectableThreshold;
    ...
    if (!isBaseball && !meetsReliability(evidence.reliability, minimumReliability)) return;
    candidates.push({
      id,
      category,
  ...
  return {
    ...
    selectableThreshold,
    highlightThreshold,
    dailyThreshold: isBaseball ? BASEBALL_DAILY_MIN_PROBABILITY : MULTISPORT_MIN_PROBABILITY,
    minimumReliability,
    ...(isBaseball ? {
      winProbabilities: {
