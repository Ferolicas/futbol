...
// NBA/NCAA y NFL/NCAA comparten el umbral base de Baseball: probabilidad y
// fiabilidad (Beta-binomial), ambas exclusivas del enfrentamiento concreto —
// nada externo a los dos equipos/jugadores corrige el número ni bloquea la
// recomendación. El promedio de la familia entre TODOS los equipos
// (refreshPredictionLedgerCalibration) queda solo como diagnóstico
// informativo para /rendimiento.
...
export function buildMultisportCombinada(prediction, odds, game, options = {}) {
  ...
  const add = ({ id, category, marketLabel, name, probability, odd, validationKey, scope = 'match', line = null, side = null, pushProbability = 0, marketFairProbability = null }) => {
    ...
    // La probabilidad publicada es 100% la del equipo/jugador exacto — nada
    // externo (promedio de la familia entre otros equipos) la corrige.
    // empiricalProbability está en escala 0-100.
    const calibratedProbability = empiricalProbability / 100;
