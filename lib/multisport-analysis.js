...
import { getMultisportConfig, isIsoDate } from './multisport-config.js';
...
function validationAllows(prediction, key, threshold = 0.8) {
  ...
  return n > 0 && Number.isFinite(accuracy) && accuracy >= threshold;
}
...
export function buildMultisportCombinada(prediction, odds, game, options = {}) {
  ...
  const add = ({ id, category, name, probability, odd, validationKey }) => {
    ...
    if (!validationAllows(prediction, validationKey, (options.validationThreshold || 80) / 100)) return;
...
function storedGame(row) {
  return {
    ...
    league: { id: row.competition_id, name: row.league_name || null }, country: { name: 'USA' },
    status: {
...
export async function listSportFixtures(sport, date, options = {}) {
  ...
  try {
    ...
    stored = rows.map(storedGame).filter((game) => localDay(game.date, options.timeZone || 'UTC') === date);
  } catch {}
  let games = stored;
  if (!games.length) {
    ...
    const attempts = await Promise.allSettled(providerDates.map((providerDate) => (
      getSportGamesByDate(config.key, providerDate, options)
    )));
    const fetched = attempts.filter((attempt) => attempt.status === 'fulfilled').map((attempt) => attempt.value);
    if (!fetched.length) throw attempts.find((attempt) => attempt.status === 'rejected')?.reason || new Error('Proveedor no disponible');
    games = [...new Map(fetched.flat()
      .filter((game) => localDay(game.date, options.timeZone || 'UTC') === date)
