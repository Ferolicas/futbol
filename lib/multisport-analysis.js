...
const CACHE_VERSION = 14; // Baseball MLB-only y recomendaciones Bet365 desde 70%
...
export function buildMultisportCombinada(prediction, odds, game, options = {}) {
  ...
  const selections = selectable.filter((item) => item.rawProbability >= highlightThreshold).slice(0, 3);
