...
const CACHE_VERSION = 17; // Baseball: cuota solo pre-partido, con hora de captura
...
export function buildMultisportCombinada(prediction, odds, game, options = {}) {
  ...
  const selectableThreshold = options.selectableThreshold
    ?? (isBaseball ? BASEBALL_RECOMMENDATION_MIN_PROBABILITY : 80);
  const highlightThreshold = options.highlightThreshold
    ?? (isBaseball ? BASEBALL_RECOMMENDATION_MIN_PROBABILITY : 90);
  ...
  const reliabilityThreshold = options.reliabilityThreshold
    ?? (isBaseball ? BASEBALL_DAILY_MIN_PROBABILITY : 90);
  const candidates = [];
  const add = ({ id, category, marketLabel, name, probability, odd, validationKey, scope = 'match', line = null, side = null }) => {
    ...
    if (p == null || rawProbability == null || !o || rawProbability < selectableThreshold) return;
    ...
    candidates.push({
      ...
      ...sampleEvidence(probability, reliabilityThreshold),
      ...metadata,
  ...
  const selectable = (isBaseball ? uniqueCandidates : [...bestByCategory.values()])
  ...
  return {
    ...
    selectableThreshold,
    highlightThreshold,
    dailyThreshold: isBaseball ? BASEBALL_DAILY_MIN_PROBABILITY : 90,
    ...(isBaseball ? {
      winProbabilities: {
