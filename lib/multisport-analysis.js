...
import { recordPredictionRun, settlePredictionFixture } from './prediction-ledger.js';
...
const CACHE_VERSION = 21; // calibración+EV, corte estricto y ledger inmutable
...
// El ledger de calibración de multideporte (refreshPredictionLedgerCalibration,
// prediction-ledger.js) nació el 2026-09-08 pero dos bugs de escala (ahora
// corregidos) impidieron que se guardara ni un solo mercado hasta el
// 2026-09-22 — el catálogo llega vacío y "unvalidated" bloquea TODO, sin
// importar probabilidad o fiabilidad. Mientras no pase esta fecha (~2 meses,
// tiempo real para acumular ≥30 liquidaciones por línea exacta), se omite
// SOLO la exigencia de calibración del ledger — la probabilidad y la
// fiabilidad (Beta-binomial del equipo/jugador específico, nunca de la línea
// en general) se siguen exigiendo sin cambios. Se evalúa en cada análisis, no
// depende de que un cron corra: pasada la fecha, vuelve a exigirse solo. NO
// aplica a fútbol (usa su propio ledger, ya con historial real).
const MULTISPORT_LEDGER_WARMUP_UNTIL = new Date('2026-11-22T00:00:00Z').getTime();
function multisportLedgerCalibrationReady(now = Date.now()) {
  return now >= MULTISPORT_LEDGER_WARMUP_UNTIL;
}
// NBA/NCAA y NFL/NCAA comparten el umbral base de Baseball. La publicación
// exige además fiabilidad, validación temporal y EV positivo en add().
...
export function shouldRetryMissingBaseballOdds(game, analysis, options = {}) {
  ...
  // NFL: el apostador necesita margen real para entrar al partido, no un
  // reintento a última hora. Con minLeadMs>0 ningún pase toca un partido a
  // menos de esa distancia de su propio kickoff, sin importar a qué hora
  // corrió el cron — la garantía es por partido, no por horario fijo.
  const minLeadMs = Number(options.minLeadMs ?? 0);
  if (minLeadMs > 0 && kickoff.getTime() - now.getTime() < minLeadMs) return false;
...
function fairProbability(prices, outcome) {
  const offered = Object.fromEntries(Object.entries(prices || {})
    .map(([key, entry]) => [key, Number(typeof entry === 'object' ? entry?.odd : entry)])
    .filter(([, odd]) => Number.isFinite(odd) && odd > 1));
  const fair = deVigProbabilities(offered);
  return fair?.[outcome] ?? null;
}
...
export function buildMultisportCombinada(prediction, odds, game, options = {}) {
  ...
  const minimumReliability = options.minimumReliability ?? BASEBALL_DAILY_MIN_RELIABILITY;
  const validationCatalog = prediction?.engine?.validation?.validation
    || prediction?.engine?.validation
    || {};
  const candidates = [];
  const auditCollector = Array.isArray(options.auditCollector) ? options.auditCollector : null;
  const add = ({ id, category, marketLabel, name, probability, odd, validationKey, scope = 'match', line = null, side = null, pushProbability = 0, marketFairProbability = null }) => {
    const empiricalProbability = rawProbabilityValue(probability);
    const o = oddValue(odd);
    if (empiricalProbability == null || !o) return;
    ...
    const diagnostic = validationKey ? validationCatalog?.[validationKey] : null;
    const validation = diagnostic ? {
      available: true,
      n: Number(diagnostic.n || 0),
      avgPred: Number(diagnostic.avgPred),
      avgActual: Number(diagnostic.hitRate ?? diagnostic.avgActual),
    } : { available: false };
    // calibrateProbability (y clampProbability dentro) esperan fracción 0-1,
    // igual que avgPred/avgActual del catálogo de calibración (misma escala
    // que probability_raw en la BD). empiricalProbability está en 0-100 —
    // sin dividir, TODO valor >1 quedaba fijado (clamp) en 100%, sin importar
    // la probabilidad real del modelo (mismo bug de escala desde 2026-09-08).
    const calibratedProbability = calibrateProbability(empiricalProbability / 100, validation);
    const decision = recommendationDecision({
      probability: calibratedProbability,
      odd: o,
      pushProbability: Number(pushProbability) || 0,
      reliability: evidence.reliability,
      validation,
      marketFairProbability,
    }, {
      minimumProbability: selectableThreshold / 100,
      minimumReliability,
      minimumExpectedValue: 0.05,
      minimumMarketEdge: 0.02,
      enforceValidation: multisportLedgerCalibrationReady(),
    });
    const p = calibratedProbability >= .95
      ? 95
      : Math.floor((calibratedProbability * 100 + 1e-9) * 100) / 100;
    const rawProbability = calibratedProbability * 100;
    const evaluated = {
      id,
      category,
      ...
      probability: p,
      rawProbability,
      // `empiricalProbability` (línea ~275, rawProbabilityValue) YA está en
      // escala 0-100 — multiplicar de nuevo aquí producía valores como 8200
      // en vez de 82. prediction-ledger.js divide una sola vez entre 100 para
      // guardar probability_raw en [0,1] (CHECK de la BD); con el doble
      // multiplicador, TODA inserción de mercado de béisbol/NBA/NFL violaba
      // esa restricción y la fila entera se perdía (bug desde 2026-09-08,
      // commit e790e9c — coincide con la creación del ledger).
      ...
      ...evidence,
      ...metadata,
      validation: { ...validation, decision: assessValidation(validation) },
      ...
      pushProbability: Number(pushProbability) || 0,
      statisticalRecommendation: decision.eligible,
      rejectionReasons: decision.reasons,
    };
    auditCollector?.push(evaluated);
    if (decision.eligible) candidates.push(evaluated);
  };

  const moneylinePush = rawProbabilityValue(prediction.moneyline?.push) || 0;
  const moneylineFair = deVigProbabilities(Object.fromEntries(Object.entries(odds?.moneyline || {})
    .map(([key, entry]) => [key, Number(typeof entry === 'object' ? entry?.odd : entry)])
    .filter(([, odd]) => Number.isFinite(odd) && odd > 1))) || {};
  add({ id: 'ml-home', category: 'moneyline', marketLabel: 'Ganador del partido', name: `${game.teams.home.name} gana`, probability: prediction.moneyline?.home, odd: odds?.moneyline?.home, validationKey: 'moneyline_home', side: 'home', pushProbability: moneylinePush, marketFairProbability: moneylineFair.home });
  add({ id: 'ml-away', category: 'moneyline', marketLabel: 'Ganador del partido', name: `${game.teams.away.name} gana`, probability: prediction.moneyline?.away, odd: odds?.moneyline?.away, validationKey: 'moneyline_away', side: 'away', pushProbability: moneylinePush, marketFairProbability: moneylineFair.away });
  if (config.drawAllowed) add({ id: 'ml-draw', category: 'moneyline', marketLabel: 'Ganador del partido', name: 'Empate', probability: prediction.moneyline?.draw, odd: odds?.moneyline?.draw, validationKey: 'moneyline_draw', side: 'draw', marketFairProbability: moneylineFair.draw });

  for (const [line, values] of Object.entries(prediction.totals?.lines || {})) {
    const prices = odds?.totals?.[line];
    add({ id: `total-${line}-over`, category: `total-${line}`, marketLabel: `Total de ${config.scoreLabel}`, name: `Más de ${line} ${config.scoreLabel}`, probability: values.over, odd: prices?.over, validationKey: `total_over_${lineKey(line)}`, line: Number(line), side: 'over', marketFairProbability: fairProbability(prices, 'over') });
    add({ id: `total-${line}-under`, category: `total-${line}`, marketLabel: `Total de ${config.scoreLabel}`, name: `Menos de ${line} ${config.scoreLabel}`, probability: values.under, odd: prices?.under, validationKey: `total_under_${lineKey(line)}`, line: Number(line), side: 'under', marketFairProbability: fairProbability(prices, 'under') });
  }
  ...
  if (!isBaseball) {
    for (const team of ['home', 'away']) {
      ...
      for (const [line, values] of Object.entries(prediction.teamTotals?.[team] || {})) {
        const prices = odds?.teamTotals?.[team]?.[line];
        for (const side of ['over', 'under']) add({
          ...
          marketFairProbability: fairProbability(prices, side),
        });
      }
    }
    ...
    for (const [periodKey, periodPrediction] of Object.entries(prediction.periods || {})) {
      ...
      for (const [line, values] of Object.entries(periodPrediction.totals || {})) {
        const prices = periodOdds.totals?.[line];
        for (const side of ['over', 'under']) add({
          ...
          probability: values?.[side], odd: prices?.[side],
          validationKey: `${periodKey}_total_${side}_${lineKey(line)}`,
          line: Number(line), side, marketFairProbability: fairProbability(prices, side),
        });
      }
      for (const team of ['home', 'away']) {
        add({
        ...
        for (const [line, values] of Object.entries(periodPrediction.teamTotals?.[team] || {})) {
          const prices = periodOdds.teamTotals?.[team]?.[line];
          for (const side of ['over', 'under']) add({
  ...
  if (isBaseball) {
    ...
    for (const team of ['home', 'away']) {
      ...
      for (const [line, values] of Object.entries(prediction.teamTotals?.[team] || {})) {
        const prices = odds?.teamTotals?.[team]?.[line];
        add({ id: `team-total-${team}-${line}-over`, category: `team-total-${team}-${line}`, marketLabel: `Total de carreras de ${teamName}`, name: `${teamName}: más de ${line} carreras`, probability: values.over, odd: prices?.over, validationKey: `team_total_${team}_over_${lineKey(line)}`, scope: 'team', line: Number(line), side: 'over', marketFairProbability: fairProbability(prices, 'over') });
        add({ id: `team-total-${team}-${line}-under`, category: `team-total-${team}-${line}`, marketLabel: `Total de carreras de ${teamName}`, name: `${teamName}: menos de ${line} carreras`, probability: values.under, odd: prices?.under, validationKey: `team_total_${team}_under_${lineKey(line)}`, scope: 'team', line: Number(line), side: 'under', marketFairProbability: fairProbability(prices, 'under') });
      }
    }
    ...
    for (const [metric, scopes] of Object.entries(prediction.statistics || {})) {
      ...
      for (const scope of ['total', 'home', 'away']) {
        ...
        for (const [line, values] of Object.entries(scopes?.[scope] || {})) {
          const prices = metricOdds?.[scope]?.[line];
          for (const side of ['over', 'under']) {
            add({
        ...
        for (const [line, prices] of Object.entries(offeredPlayer.lines || {})) {
          for (const side of ['over', 'under']) {
            add({
              ...
              odd: prices?.[side],
              // La calibración se aprende por tipo de prop/lado/línea entre
              // jugadores comparables. Separarla por ID impediría alcanzar una
              // muestra útil y fingiría que 30 juegos de un solo atleta bastan.
              validationKey: `player_${metric}_${side}_${lineKey(line)}`,
  ...
  const selections = [...bestByCategory.values()]
    ...
    .sort((a, b) => b.rawProbability - a.rawProbability || b.odd - a.odd)
    .slice(0, 3);
  // Todas pertenecen al mismo partido: sin precio oficial SGP no se presume
  // independencia entre moneyline, totales, periodos o jugadores.
  return {
    selections,
    selectable,
    combinedOdd: null,
...
export async function analyzeSportGame(sport, game, options = {}) {
  ...
  const predictionCutoff = strictPregameCutoff(game.date, new Date());
  if (config.key === 'baseball' && String(game.league?.id || '1') === '1') {
    ...
    [pitcherMatchup, playerHighlights] = await Promise.all([
      ...
      extractBaseballPlayerHighlights({
        ...
        home: { ...game.teams.home, probablePitcherId: game.teams.home.probablePitcherId, probablePitcherName: game.teams.home.probablePitcherName },
        away: { ...game.teams.away, probablePitcherId: game.teams.away.probablePitcherId, probablePitcherName: game.teams.away.probablePitcherName },
      }, season, { cutoff: predictionCutoff }).catch(() => null),
    ]);
  ...
  const prediction = await computeMultisportEmpiricalPrediction(pgPool, {
    sport: config.key, fixture, odds, cutoff: predictionCutoff,
  });
  const playerProbabilities = buildEmpiricalPlayerProbabilities(playerHighlights, odds?.playerProps);
  const auditedMarkets = [];
  const genericCombinada = buildMultisportCombinada(prediction, odds, game, {
    playerProbabilities,
    auditCollector: auditedMarkets,
  });
  ...
  const generatedBeforeKickoff = Date.now() < new Date(game.date).getTime();
  const combinada = generatedBeforeKickoff ? genericCombinada : {
    ...genericCombinada,
  ...
  const predictionMeta = {
    ...
    horizon: fixture.context?.home?.starters?.length || fixture.context?.away?.starters?.length
      ? 'probable-lineup'
      : 'early',
    modelVersion: prediction.engine?.version || 0,
    engine: `${config.key}-empirical-weighting`,
    features: { context: fixture.context, engine: prediction.engine },
  };
  ...
  await recordPredictionRun({
    sport: config.key, fixtureId: game.id, game, payload: durablePayload,
    predictionMeta, analysisVersion: CACHE_VERSION, dataQuality: dq,
    source: 'multisport-analysis', marketOutputs: auditedMarkets,
  });
  await Promise.all([
    saveSportAnalysis(pgPool, config.key, game, durablePayload),
...
async function analyzeSportDateInner(sport, date, options = {}) {
  ...
  if (options.onlyMissingCurrent === true && eligibleGames.length) {
    ...
    games = selectGamesNeedingCurrentAnalysis(eligibleGames, existing, CACHE_VERSION, {
      retryMissingOdds: options.retryMissingOdds === true,
      now: options.now,
      maxOddsRetries: options.maxOddsRetries,
      minLeadMs: options.oddsRetryMinLeadMs,
    });
...
export async function listSportFixtures(sport, date, options = {}) {
  ...
  try {
    const seals = await sealedProofsForFixtures(config.key, currentAnalyses.map((analysis) => analysis.fixture_id));
    for (const analysis of currentAnalyses) {
      analysis.combinada = attachSealsToRecommendationContainer(
        analysis.combinada,
        seals.get(String(analysis.fixture_id)),
      );
    }
  } catch (error) {
    if (error?.code !== '42P01') console.warn(`[${config.key}:prediction-seals] ${error.message}`);
  }
