...
const CACHE_VERSION = 16; // Baseball: hándicap Bet365 canónico + resumen de victoria local/visitante
...
export function selectGamesNeedingCurrentAnalysis(games, analyses, cacheVersion = CACHE_VERSION, options = {}) {
  ...
  return (games || []).filter((game) => {
    ...
    if (!analysis || Number(analysis.cache_version || 0) < cacheVersion) return true;
    return options.retryMissingOdds === true
      && shouldRetryMissingBaseballOdds(game, analysis, options);
  });
}
