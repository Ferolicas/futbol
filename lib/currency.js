...
export async function getExchangeRate(from = 'USD', to = 'USD') {
  ...
  try {
    ...
    console.error('Currency conversion error:', safeLogFields({ error: error?.message }));
    // CU1 FIX: antes devolvía 1 → el caller cobraba el número en USD COMO SI
    // fuera la moneda local (ej. "15" en COP ≈ $0.004 en vez de $15). Ahora
    // devolvemos null para que convertAmount caiga a cobrar en la moneda FUENTE.
    return null;
  }
}
...
export async function convertAmount(amount, targetCurrency, sourceCurrency = 'USD') {
  ...
  if (src === tgt) {
    return { amount, currency: tgt, rate: 1, original: amount, fallback: false };
  }

  const rate = await getExchangeRate(src, tgt);
  // CU1 FIX: si no hay tasa fiable, NO convertir — cobrar en la moneda fuente
  // (importe correcto, solo en otra divisa) en vez de cobrar mal en la local.
  if (rate == null || !isFinite(rate) || rate <= 0) {
    return { amount, currency: src, rate: 1, original: amount, fallback: true };
  }
...
export function getCurrencyFromCountry(countryCode) {
  const map = {
    ...
    // Stripe no admite VES ni CUP como moneda de presentacion. Venezuela y
    // Cuba se muestran/cobran en USD para no enviar una divisa rechazada.
    VE: 'USD', EC: 'USD', BO: 'BOB', PY: 'PYG', CR: 'CRC', PA: 'USD', DO: 'DOP',
    GT: 'GTQ', HN: 'HNL', SV: 'USD', NI: 'NIO', CU: 'USD',
