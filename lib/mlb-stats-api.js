...
// así que para MLB usamos esta API que da TODO en tiempo real.
//
// Producto activo: sportId 1 = MLB, sportId 11 = Triple-A (Bet365 SÍ publica
// cuota real ahí: hándicap ±1.5 y una línea de total de carreras — confirmado
// en Bet365). El resto de MiLB (AA=12, A+=13, A=14, Rookie=16) sigue fuera:
// sin verificación de que Bet365 las ofrezca, no se piden.
//
// Endpoints usados:
...
// Categorías activas del producto. Los consumidores operativos derivan sus
// llamadas de este mapa, por lo que mantener solo MLB evita llamadas a MiLB.
export const MLB_SPORT_IDS = Object.freeze({
  1: 'MLB',
  11: 'Triple-A',
});

const MLB_HIT_EVENT_TYPES = new Set(['single', 'double', 'triple', 'home_run']);

export function isMlbHitPlay(play) {
  return MLB_HIT_EVENT_TYPES.has(String(play?.result?.eventType || '').toLowerCase());
}
...
// 45s: MLB Stats API es gratuita pero responde lento en endpoints "fat" como
// /v1.1/game/{pk}/feed/live o /schedule de días con muchos juegos. El timeout
// anterior de 15s producía aborts en baseball-live cuando había 10+ juegos
// simultáneos. 45s da margen sin bloquear el job (concurrency=1, every 60s).
...
export async function getMlbScheduleByDate(date, sportId = 1) {
  const sportIds = (Array.isArray(sportId) ? sportId : [sportId]).map(Number).filter(Number.isFinite);
  const path = `/v1/schedule?sportId=${sportIds.join(',')}&date=${date}` +
    `&hydrate=probablePitcher,linescore,team`;
  const data = await mlbFetch(path);
  const games = (data.dates?.[0]?.games) || [];
  return games.map((game) => normalizeScheduleGame(
    game,
    game.teams?.home?.team?.sport?.id || game.teams?.away?.team?.sport?.id || sportIds[0] || 1,
  ));
}

// Histórico oficial por rango. Se usa en backfills acotados por bloques para
// no depender del límite de fechas de API-Baseball ni hacer una llamada por
// jornada. MLB Stats no requiere clave.
export async function getMlbScheduleByRange(startDate, endDate, sportId = 1) {
  const sportIds = (Array.isArray(sportId) ? sportId : [sportId]).map(Number).filter(Number.isFinite);
  const path = `/v1/schedule?sportId=${sportIds.join(',')}&startDate=${startDate}&endDate=${endDate}`
    + '&hydrate=probablePitcher,linescore,team';
  const data = await mlbFetch(path);
  return (data.dates || []).flatMap((entry) => (
    entry.games || []
  ).map((game) => normalizeScheduleGame(
    game,
    game.teams?.home?.team?.sport?.id || game.teams?.away?.team?.sport?.id || sportIds[0] || 1,
  )));
}

function normalizeScheduleGame(g, sportId = 1) {
  ...
  return {
    ...
    sportName: MLB_SPORT_IDS[Number(sportId)] || g.sport?.name || `Béisbol ${sportId}`,
    dateUTC: g.gameDate,
...
export async function getMlbLiveGame(gamePk) {
  ...
  // Últimas 8 jugadas COMPLETAS (`isComplete: true`) — el job baseball-live
  // mantiene un cursor por gamePk en Redis (`bb:plays:cursor:{pk}`) con el
  // mayor atBatIndex emitido y procesa solo las nuevas, así detecta HR /
  // strikeouts / cambios de inning sin reprocesar. El tope de 8 cubre el
  // poll de 1 min con margen incluso en juegos de altísima actividad.
  const recentPlays = allPlays
    .filter(p => p?.about?.isComplete)
    .slice(-8)
    .map(p => {
      const runnersOnBaseBeforePlay = (p.runners || []).filter(r =>
        r.movement?.start && r.movement.start !== 'home' && r.movement.start !== '4B'
      ).length;
      return {
        atBatIndex: p.atBatIndex ?? null,
        eventType: p.result?.eventType || null,           // 'home_run', 'strikeout', 'single', ...
        event: p.result?.event || null,
        description: p.result?.description || '',
        inning: p.about?.inning ?? null,
        halfInning: p.about?.halfInning || null,           // 'top' | 'bottom'
        isComplete: !!p.about?.isComplete,
        outsAfter: p.count?.outs ?? null,                  // outs DESPUÉS de la jugada
        batter: p.matchup?.batter ? { id: p.matchup.batter.id, name: p.matchup.batter.fullName } : null,
        battingTeam: p.about?.halfInning === 'top' ? 'away' : 'home',
        runnersOnBaseBeforePlay,
        endTime: p.about?.endTime || null,
      };
    });

  // MLB no expone hits por entrada en el linescore. Se obtienen de todas las
  // apariciones al plato finalizadas mediante los `eventType` oficiales que
  // conceden hit: single, double, triple y home_run. Este desglose alimenta
  // los informes de 1.ª/primeras 3/primeras 5 sin estimar ni repartir el total.
  const hitsByInning = new Map();
  for (const play of allPlays) {
    const inning = Number(play?.about?.inning);
    if (!Number.isFinite(inning) || !isMlbHitPlay(play)) continue;
    const side = play?.about?.halfInning === 'top' ? 'away' : 'home';
    const current = hitsByInning.get(inning) || { home: 0, away: 0 };
    current[side] += 1;
    hitsByInning.set(inning, current);
  }
  ...
  return {
    ...
    innings: (ls.innings || []).map(i => ({
      ...
      homeHits: hitsByInning.get(Number(i.num))?.home ?? 0,
      awayHits: hitsByInning.get(Number(i.num))?.away ?? 0,
    })),
  };
}

// Boxscore completo oficial para ingesta histórica de jugadores. Se mantiene
// separado del payload live liviano porque solo se solicita al finalizar.
export async function getMlbGameBoxscore(gamePk) {
  if (!gamePk) return null;
  return mlbFetch(`/v1/game/${gamePk}/boxscore`);
}
...
export async function getMlbPitcherGameLog(pitcherId, season, cutoff = null) {
  if (!pitcherId) return null;
  const ck = `mlb:plog:p:v3:${pitcherId}:${season}`;
  let records = null;
  try { records = await redisGet(ck); } catch {}
  let data;
  if (!Array.isArray(records)) {
    try { data = await mlbFetch(`/v1/people/${pitcherId}/stats?stats=gameLog&group=pitching&season=${season}`); }
    catch { return null; }
    records = (data.stats?.[0]?.splits || []).map((s) => ({
      date: s.date || null,
      value: Number(s.stat?.strikeOuts) || 0,
    }));
    try { await redisSet(ck, records, TTL_PLAYERLOG); } catch {}
  }
  const cutoffDay = cutoff ? new Date(cutoff).toISOString().slice(0, 10) : null;
  return records.filter((row) => !cutoffDay || (row.date && row.date < cutoffDay)).map((row) => row.value);
}

// Game log de bateador → todas las métricas que el análisis completo y los
// props reales de Bet365 pueden cruzar, siempre partido a partido.
export async function getMlbBatterGameLog(batterId, season, cutoff = null) {
  if (!batterId) return null;
  const ck = `mlb:plog:b:v3:${batterId}:${season}`;
  let records = null;
  try { records = await redisGet(ck); } catch {}
  let data;
  if (!Array.isArray(records)) {
    ...
    records = (data.stats?.[0]?.splits || []).map((s) => ({ date: s.date || null, stat: s.stat || {} }));
    try { await redisSet(ck, records, TTL_PLAYERLOG); } catch {}
  }
  const cutoffDay = cutoff ? new Date(cutoff).toISOString().slice(0, 10) : null;
  const splits = records.filter((row) => !cutoffDay || (row.date && row.date < cutoffDay));
  const history = {
    ...
    runs:       splits.map(s => Number(s.stat.runs) || 0),
    walks:      splits.map(s => Number(s.stat.baseOnBalls) || 0),
    stolenBases: splits.map(s => Number(s.stat.stolenBases) || 0),
    battingStrikeouts: splits.map(s => Number(s.stat.strikeOuts) || 0),
  };
  return history;
}

// Lineup de un juego con nombres. DOS fuentes (la del boxscore solo se llena
// cuando el juego entra en Pre-Game/Live; antes está vacía → por eso "nunca"
// traía bateadores en el análisis previo):
//   1) /schedule?hydrate=lineups → lineup PROYECTADO/confirmado, aparece horas
//      antes (homePlayers/awayPlayers). Fuente primaria.
//   2) /game/{pk}/boxscore battingOrder → fallback (lineup oficial ya cargado).
export async function getMlbGameLineup(gamePk) {
  if (!gamePk) return { home: [], away: [], source: 'none' };
  const cacheKey = `mlb:lineup:${gamePk}`;
  try {
    const cached = await redisGet(cacheKey);
    if (cached) return cached;
  } catch {}
  const remember = async (value) => {
    try { await redisSet(cacheKey, value, 5 * 60); } catch {}
    return value;
  };

  // 1) Schedule hydrate=lineups
  try {
    const sch = await mlbFetch(`/v1/schedule?gamePk=${gamePk}&hydrate=lineups`);
    const lu = sch?.dates?.[0]?.games?.[0]?.lineups;
    if (lu) {
      const map = (arr) => (arr || []).map(p => ({ id: p.id ?? p.person?.id, name: p.fullName || p.name || p.person?.fullName || null })).filter(x => x.id && x.name);
      const home = map(lu.homePlayers), away = map(lu.awayPlayers);
      if (home.length || away.length) return remember({ home, away, source: 'schedule-lineups' });
    }
  } catch {}

  // 2) Boxscore battingOrder (cuando ya está cargado)
  try {
    const data = await mlbFetch(`/v1/game/${gamePk}/boxscore`);
    const side = (s) => {
    ...
    if (home.length || away.length) return remember({ home, away, source: 'boxscore' });
  } catch {}

  return remember({ home: [], away: [], source: 'none' });
}
 ...
 */
export async function extractBaseballPlayerHighlights(game, season, { cutoff = null } = {}) {
  if (!game) return null;
  const out = {
    strikeouts: [], hits: [], homeRuns: [], totalBases: [], rbis: [], runs: [],
    walks: [], stolenBases: [], battingStrikeouts: [], context: null,
  };

  // Ponches de los pitchers abridores.
  const pitchers = [
    { id: game.home?.probablePitcherId, name: game.home?.probablePitcherName, team: game.home?.name, teamId: game.home?.id },
    { id: game.away?.probablePitcherId, name: game.away?.probablePitcherName, team: game.away?.name, teamId: game.away?.id },
  ];
  await Promise.all(pitchers.map(async (p) => {
    if (!p.id) return;
    const log = await getMlbPitcherGameLog(p.id, season, cutoff).catch(() => null);
    if (log && log.length) {
      out.strikeouts.push({ id: p.id, name: p.name, teamName: p.team, teamId: p.teamId, starter: true, history: log, total: log.reduce((a, b) => a + b, 0) });
    }
  }));

  // Props de bateadores del lineup (proyectado o confirmado).
  const lineup = await getMlbGameLineup(game.gamePk).catch(() => ({ home: [], away: [], source: 'error' }));
  out.context = {
    source: lineup.source,
    homeStarters: lineup.home.map((player) => String(player.id)),
    awayStarters: lineup.away.map((player) => String(player.id)),
  };
  const batters = [
    ...lineup.home.map(b => ({ ...b, team: game.home?.name, teamId: game.home?.id })),
    ...lineup.away.map(b => ({ ...b, team: game.away?.name, teamId: game.away?.id })),
  ];
  console.log('[bb-props] gamePk=%s lineup source=%s batters=%d (home %d, away %d)', game.gamePk, lineup.source, batters.length, lineup.home.length, lineup.away.length);
  let withLog = 0;
  await Promise.all(batters.map(async (b) => {
    const log = await getMlbBatterGameLog(b.id, season, cutoff).catch(() => null);
    if (!log || !(log.hits || []).length) return;
    withLog++;
    const base = { id: b.id, name: b.name, teamName: b.team, teamId: b.teamId, starter: true };
    ...
    out.runs.push({ ...base, history: log.runs });
    out.walks.push({ ...base, history: log.walks });
    out.stolenBases.push({ ...base, history: log.stolenBases });
    out.battingStrikeouts.push({ ...base, history: log.battingStrikeouts });
  }));
  console.log('[bb-props] gamePk=%s pitchers=%d batters con game-log=%d/%d', game.gamePk, out.strikeouts.length, withLog, batters.length);
