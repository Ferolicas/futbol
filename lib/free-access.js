...
export function freeAnalysis(source, sport = 'football') {
  ...
  for (const pick of doc.combinada?.selectable || []) {
  ...
  return {
    ...
    freePreview: {
      selection: visible,
      // No canonical IDs, order, names, category, odds, sample or metadata.
      locked: options.filter(item => item.id !== selected?.id && item.probability > 70).map(item => ({ probability: item.probability })),
