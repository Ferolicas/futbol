...
const { deVigProbabilities, recommendationDecision } = predictionMath;
...
export function buildModelCombinada(scored, odds, teamNames, playerMarkets, probabilities, statsData) {
  ...
  for (const sel of playerMarketsToSelections(playerMarkets)) {
    ...
    if (!o) continue;
    // La probabilidad publicada es la del jugador tal cual la calculó el
    // motor — ninguna calibración externa a su propia evidencia la corrige.
    const calibratedProbability = rawProbability / 100;
