...
export function buildModelCombinada(scored, odds, teamNames, playerMarkets, probabilities, statsData) {
  ...
  // únicamente en el acordeón seleccionable. La cantidad de partidos jamás
  // veta el cálculo: estos son umbrales comerciales sobre la frecuencia real.
  ...
  const _funnel = {
    ...
    playerRecommended: playerRecommended.length, selectable: selectable.length,
  };

  if (!selected.length) {
    return { selections: [], selectable, _funnel, combinedOdd: null, combinedProbability: 0, highRisk: false, hasRealOdds: selectable.length > 0, threshold: 80, source: 'context-engine' };
  }
