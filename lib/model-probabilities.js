...
import { displayPct, playerMarketsToSelections } from './model-to-scored.js';
import {
  FOOTBALL_RECOMMENDATION_MIN_RELIABILITY,
...
const { calibrateProbability, deVigProbabilities, recommendationDecision } = predictionMath;

const MIN_ODD = 1.20;
const clampPct = (v) => displayPct(v);
...
export async function buildModelCtx(pool, { fixtureId, leagueId, season, round, referee, venueId, homeId, awayId, homeRank, awayRank, kickoff, cutoff }) {
  let nTeams = null;
  let schedule = {};
  try {
    const { rows } = await pool.query(
      `SELECT count(*)::int n FROM model.standings_snapshots
       WHERE competition_id=$1 AND season=$2
         AND as_of_date=(SELECT max(as_of_date) FROM model.standings_snapshots
                         WHERE competition_id=$1 AND season=$2
                           AND as_of_date <= ($3::timestamptz AT TIME ZONE 'UTC')::date)`,
      [leagueId, season, cutoff || kickoff || new Date()]);
    nTeams = rows[0]?.n || null;
  } catch { /* sin standings → nTeams null (degradación limpia) */ }
  try {
    const targetKickoff = new Date(kickoff || cutoff);
    const { rows } = await pool.query(
      `SELECT team_id,max(previous_kickoff) AS previous_kickoff,min(next_kickoff) AS next_kickoff
       FROM (
         SELECT target.team_id,
                CASE WHEN m.kickoff<$2 THEN m.kickoff END AS previous_kickoff,
                CASE WHEN m.kickoff>$2 AND m.fixture_id<>$3 THEN m.kickoff END AS next_kickoff
         FROM unnest($1::bigint[]) AS target(team_id)
         JOIN model.matches m ON target.team_id IN (m.home_team_id,m.away_team_id)
       ) schedule_rows GROUP BY team_id`,
      [[Number(homeId), Number(awayId)], targetKickoff, Number(fixtureId)],
    );
    const byTeam = new Map(rows.map((row) => [Number(row.team_id), row]));
    const days = (left, right) => left && right ? Math.max(0, (new Date(left) - new Date(right)) / 86400000) : null;
    schedule = {
      homeRestDays: days(targetKickoff, byTeam.get(Number(homeId))?.previous_kickoff),
      awayRestDays: days(targetKickoff, byTeam.get(Number(awayId))?.previous_kickoff),
      homeNextRestDays: days(byTeam.get(Number(homeId))?.next_kickoff, targetKickoff),
      awayNextRestDays: days(byTeam.get(Number(awayId))?.next_kickoff, targetKickoff),
    };
  } catch { /* agenda incompleta: el peso de descanso queda neutro */ }
  return {
    ...
    phase: phaseOf(round), referee: referee || null, venueId: venueId == null ? null : Number(venueId),
    homeRank: homeRank != null ? Number(homeRank) : null,
    awayRank: awayRank != null ? Number(awayRank) : null,
    nTeams, kickoff: kickoff || null, ...schedule, cutoff: cutoff || new Date(),
  };
}
...
async function teamAvg(pool, teamId, cutoff) {
  const { rows } = await pool.query(
    `SELECT count(*)::int n,
            count(corners_for)::int corners_n, count(yellow_for)::int cards_n,
            count(shots_for)::int shots_n, count(sot_for)::int sot_n,
            count(fouls_for)::int fouls_n, count(offsides_for)::int offsides_n,
  ...
  return {
    n: a.n || 0,
    coverage: {
      corners: Number(a.corners_n || 0), cards: Number(a.cards_n || 0),
      shots: Number(a.shots_n || 0), sot: Number(a.sot_n || 0),
      fouls: Number(a.fouls_n || 0), offsides: Number(a.offsides_n || 0),
    },
...
export async function buildModelDescriptives(pool, ctx) {
  ...
  for (const key of ['corners', 'cards', 'shots', 'sot', 'fouls', 'offsides']) {
    dataAvailability[key] = Number(homeAvg.coverage?.[key] || 0) + Number(awayAvg.coverage?.[key] || 0) > 0;
  }
  return { homeAvg, awayAvg, meetings: mt.rows[0]?.n || 0, dataAvailability };
}
...
export function buildCalculatedProbabilities(scored, descriptives, analysis) {
  ...
  const dataAvailability = descriptives.dataAvailability || availabilityFromStats(analysis?.cornerCardData);
...
function resolveOddField(key) {
  ...
  if (ou) {
    ...
    const field = {
      ...
      total_shots: 'shots', total_sot: 'sot', total_fouls: 'fouls', total_offsides: 'offsides',
      ...
      home_sot: 'homeSot', away_sot: 'awaySot',
      home_fouls: 'homeFouls', away_fouls: 'awayFouls', home_offsides: 'homeOffsides', away_offsides: 'awayOffsides',
...
// Diagnóstico de cuota sin aplicar las reglas de publicación. Permite distinguir
// tres situaciones que antes quedaban mezcladas bajo "sin cuota": mercado que el
// adaptador todavía no conoce, línea exacta que bet365/bwin no entregaron y cuota
// real entregada pero inferior al mínimo comercial. La ruta de producto sigue
// usando oddFor() y, por tanto, conserva intacto el gate >= 1.20.
export function inspectMarketOdd(key, odds) {
  ...
  if (Array.isArray(allBks)) {
    for (const bk of allBks) {
      const family = bk?.[field];
      if (!family) continue;
      for (const candidate of candidates) {
        const odd = Number(family[candidate]);
        if (!Number.isFinite(odd) || odd <= 1) continue;
        if (!best || odd > best.odd) best = { odd, bookmaker: bk.name || null };
        break;
      }
    }
  }

  // Red de seguridad para caches antiguos que solo conservaron el agregado.
  if (!best) {
    const aggregate = odds?.[field];
    if (aggregate) {
      for (const candidate of candidates) {
        const odd = Number(aggregate[candidate]);
        if (!Number.isFinite(odd) || odd <= 1) continue;
        best = { odd, bookmaker: 'bet365/bwin' };
        break;
      }
    }
  }
  ...
  return {
    status: best.odd >= MIN_ODD ? 'offered' : 'below_minimum',
    field,
    lineKey,
    candidates,
    odd: best.odd,
...
function oddFor(key, odds) {
  const inspected = inspectMarketOdd(key, odds);
  if (inspected.status !== 'offered') return null;
  let marketFairProbability = null;
  const oppositeCandidates = (inspected.candidates || []).map((candidate) => {
    if (candidate.startsWith('Over_')) return `Under_${candidate.slice(5)}`;
    if (candidate.startsWith('Under_')) return `Over_${candidate.slice(6)}`;
    return null;
  }).filter(Boolean);
  if (oppositeCandidates.length) {
    const sources = Array.isArray(odds?.allBookmakerOdds)
      ? odds.allBookmakerOdds.filter((bookmaker) => bookmaker?.name === inspected.bookmaker)
      : [];
    if (!sources.length && odds?.[inspected.field]) sources.push({ [inspected.field]: odds[inspected.field] });
    for (const source of sources) {
      const family = source?.[inspected.field];
      if (!family) continue;
      const oppositeOdd = oppositeCandidates.map((candidate) => Number(family[candidate]))
        .find((value) => Number.isFinite(value) && value > 1);
      if (!oppositeOdd) continue;
      const fair = deVigProbabilities([inspected.odd, oppositeOdd]);
      marketFairProbability = fair?.[0] ?? null;
      break;
    }
  }
  if (marketFairProbability == null) {
    // Solo normalizamos grupos realmente excluyentes. Doble oportunidad y
    // escaleras de hándicap se solapan, por lo que tratarlos como una
    // distribución sería otro error de independencia.
    const exclusiveFamilies = new Set([
      'matchWinner', 'oddEven', 'exactGoals', 'btts',
      'winner1H', 'winner2H', 'corners1x2', 'shots1x2', 'fouls1x2',
      'correctScore', 'cleanSheetHome', 'cleanSheetAway',
    ]);
    if (exclusiveFamilies.has(inspected.field)) {
      const sources = Array.isArray(odds?.allBookmakerOdds)
        ? odds.allBookmakerOdds.filter((bookmaker) => bookmaker?.name === inspected.bookmaker)
        : [];
      if (!sources.length && odds?.[inspected.field]) sources.push({ [inspected.field]: odds[inspected.field] });
      for (const source of sources) {
        const family = source?.[inspected.field];
        const selectedKey = (inspected.candidates || []).find((candidate) => Number(family?.[candidate]) === inspected.odd);
        if (!selectedKey) continue;
        const offered = Object.fromEntries(Object.entries(family || {})
          .map(([candidate, odd]) => [candidate, Number(odd)])
          .filter(([, odd]) => Number.isFinite(odd) && odd > 1));
        const fair = deVigProbabilities(offered);
        marketFairProbability = fair?.[selectedKey] ?? null;
        break;
      }
    }
  }
  return { odd: inspected.odd, bookmaker: inspected.bookmaker, marketFairProbability };
}
...
export function buildModelCombinada(scored, odds, teamNames, playerMarkets, probabilities, statsData) {
  const allBks = odds?.allBookmakerOdds;
  // El schema model es la fuente del cálculo. Las arrays visuales de últimos
  // partidos pueden venir incompletas aunque el histórico real sí exista.
  const avail = probabilities?.cornerCardData?.dataAvailability || (statsData ? availabilityFromStats(statsData) : {});
  const blockedByData = (key) => { const c = dataCategoryOf(key); return c != null && avail[c] !== true; };

  // Player props del MODELO: ≥80% entra como recomendación; 70–79% queda
  // únicamente en el acordeón seleccionable. La fiabilidad >=90% decide si
  // el resultado calculado puede publicarse como opción, pero nunca altera su
  // frecuencia ni lo elimina de las estadísticas calculadas.
  const PLAYER_RECOMMENDED_MIN_PROB = 80;
  const PLAYER_SELECTABLE_MIN_PROB = 70;
  const playerSelectable = [];
  let playerBelowReliability = 0;
  for (const sel of playerMarketsToSelections(playerMarkets)) {
    const rawProbability = Number(sel.rawProbability ?? sel.probability);
    if (rawProbability < PLAYER_SELECTABLE_MIN_PROB) continue;
    if (!meetsFootballReliability(sel.confidence)) { playerBelowReliability++; continue; }
    const o = playerOddFor(sel, allBks);
    if (!o) continue;
    // Los props conservan sus estadísticas aunque no superen la validación
    // temporal de su propia familia. Solo la recomendación queda cerrada.
    const calibratedProbability = calibrateProbability(rawProbability / 100, sel.validation);
    const decision = recommendationDecision({
      probability: calibratedProbability,
      odd: o.odd,
      reliability: reliabilityPercent(sel.confidence),
      validation: sel.validation,
    }, {
      minimumProbability: PLAYER_SELECTABLE_MIN_PROB / 100,
      enforceExpectedValue: false,
      enforceMarketEdge: false,
    });
    if (!decision.eligible
        || !meetsFootballExpectedValuePolicy(calibratedProbability, decision.expectedValue)) continue;
    playerSelectable.push({
      ...sel,
      probability: clampPct(calibratedProbability * 100),
      rawProbability: calibratedProbability * 100,
      empiricalProbability: rawProbability,
      odd: o.odd,
      bookmaker: o.bookmaker,
      confidence: reliabilityPercent(sel.confidence),
      expectedValue: decision.expectedValue,
      validationStatus: decision.validation.status,
      recommended: calibratedProbability >= PLAYER_RECOMMENDED_MIN_PROB / 100,
      dailyEligible: calibratedProbability >= 0.90
        && meetsFootballExpectedValuePolicy(calibratedProbability, decision.expectedValue),
    });
  }
  // El tamaño de muestra queda visible en la evidencia, pero no bloquea una
  // frecuencia real. Los umbrales de 80/90 son reglas de producto, no ajustes.
  const playerRecommended = playerSelectable.filter((sel) => Number(sel.rawProbability ?? sel.probability) >= PLAYER_RECOMMENDED_MIN_PROB);

  // Equipos recomendados (≥80% via scored.recommended) con cuota real.
  const teamSel = [];
  for (const [key, r] of Object.entries(scored)) {
    if (!r.recommended) continue;
    if (!meetsFootballReliability(r.confidence)) continue;
    if (blockedByData(key)) continue;
    const o = oddFor(key, odds);
    if (!o) continue;
    const confidence = reliabilityPercent(r.confidence);
    const decision = recommendationDecision({
      probability: r.prob_final,
      odd: o.odd,
      reliability: confidence,
      validation: r.validation,
      marketFairProbability: o.marketFairProbability,
    }, {
    ...
    if (!decision.eligible
        || !meetsFootballExpectedValuePolicy(r.prob_final, decision.expectedValue)) continue;
    teamSel.push({
      ...
      level: r.level, confidence, sampleN: r.n,
      rawProbability: r.prob_final * 100,
      empiricalProbability: r.prob_raw * 100,
      validation: r.validation,
      validationStatus: decision.validation.status,
      expectedValue: decision.expectedValue,
      marketFairProbability: decision.marketFairProbability,
      marketEdge: decision.marketEdge,
      dailyEligible: r.prob_final >= 0.90
        && meetsFootballExpectedValuePolicy(r.prob_final, decision.expectedValue),
    });
  }
  const selected = [...teamSel, ...playerRecommended]
    .filter(m => m.odd > MIN_ODD - 1e-9)
  ...
  let fScored = 0, fGe70 = 0, fBelowReliability = 0, fGe70NoOdds = 0, fNoData = 0;
  for (const [key, r] of Object.entries(scored)) {
    fScored++;
    ...
    fGe70++;
    if (!meetsFootballReliability(r.confidence)) { fBelowReliability++; continue; }
    const o = oddFor(key, odds);
    ...
    const decision = recommendationDecision({
      ...
      minimumProbability: SELECTABLE_MIN_PROB,
      enforceExpectedValue: false,
      enforceMarketEdge: false,
    });
    if (!decision.eligible || !meetsFootballExpectedValuePolicy(p, decision.expectedValue)) continue;
    teamSelectable.push({
      id: key, category: categoryOf(key), scope: 'context',
      ...
      confidence, sampleN: r.n,
      rawProbability: p * 100,
      empiricalProbability: r.prob_raw * 100,
      validation: r.validation,
      validationStatus: decision.validation.status,
      expectedValue: decision.expectedValue,
      marketFairProbability: decision.marketFairProbability,
      marketEdge: decision.marketEdge,
      dailyEligible: p >= 0.90 && meetsFootballExpectedValuePolicy(p, decision.expectedValue),
    });
  }
  const selectable = [...teamSelectable, ...playerSelectable]
    .filter(m => m.odd > MIN_ODD - 1e-9)
    .sort((a, b) => Number(b.expectedValue || 0) - Number(a.expectedValue || 0)
      || Number(b.rawProbability ?? b.probability) - Number(a.rawProbability ?? a.probability));

  const _funnel = {
    scored: fScored, ge70: fGe70, bajoFiabilidad90: fBelowReliability,
    ge70_sinCuota: fGe70NoOdds, sinDatos: fNoData,
    teamSelectable: teamSelectable.length, playerSelectable: playerSelectable.length,
    playerBelowReliability, playerRecommended: playerRecommended.length, selectable: selectable.length,
  };

  if (!selected.length) {
    return {
      selections: [], selectable, _funnel,
      combinedOdd: null, combinedProbability: null, correlationStatus: 'not-priced', highRisk: false,
      hasRealOdds: selectable.length > 0, threshold: 80,
      minimumReliability: FOOTBALL_RECOMMENDATION_MIN_RELIABILITY,
      source: 'context-engine',
    };
  }
  // Estas selecciones pertenecen al MISMO partido. Sin una cuota oficial de
  // Bet Builder/SGP o una distribución conjunta certificada, multiplicarlas
  // fingiría independencia. Conservamos las opciones, pero no publicamos una
  // probabilidad/cuota combinada inventada.
  return {
    selections: selected, selectable, _funnel,
    combinedOdd: null,
    combinedProbability: null,
    correlationStatus: 'same-fixture-not-priced',
    highRisk: false, hasRealOdds: true,
    threshold: Math.min(...selected.map(m => Number(m.rawProbability ?? m.probability))), source: 'context-engine',
    minimumReliability: FOOTBALL_RECOMMENDATION_MIN_RELIABILITY,
  };
}
