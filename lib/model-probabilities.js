...
export async function buildModelCtx(pool, { fixtureId, leagueId, season, round, referee, homeId, awayId, homeRank, awayRank, cutoff }) {
  let nTeams = null;
  try {
  ...
  return {
    ...
    phase: phaseOf(round), referee: referee || null,
...
function oddFor(key, odds) {
  ...
  return { odd: inspected.odd, bookmaker: inspected.bookmaker };
}
...
export function buildModelCombinada(scored, odds, teamNames, playerMarkets, probabilities, statsData) {
  ...
  for (const sel of playerMarketsToSelections(playerMarkets)) {
    ...
    playerSelectable.push({
      ...
      recommended: rawProbability >= PLAYER_RECOMMENDED_MIN_PROB,
      dailyEligible: rawProbability >= 90,
    });
  }
  ...
  for (const [key, r] of Object.entries(scored)) {
    ...
    teamSel.push({
      ...
      level: r.level, confidence: reliabilityPercent(r.confidence), sampleN: r.n,
  ...
  for (const [key, r] of Object.entries(scored)) {
    ...
    teamSelectable.push({
      ...
      confidence: reliabilityPercent(r.confidence), sampleN: r.n,
  ...
  if (!selected.length) {
    return {
      ...
      combinedOdd: null, combinedProbability: 0, highRisk: false,
  ...
  const combinedProbability = selected.reduce((acc, m) => acc * (Number(m.rawProbability ?? m.probability) / 100), 1) * 100;
  return {
