...
export function buildModelCombinada(scored, odds, teamNames, playerMarkets, probabilities, statsData) {
  ...
  for (const sel of playerMarketsToSelections(playerMarkets)) {
    ...
    }, { minimumProbability: PLAYER_SELECTABLE_MIN_PROB / 100, minimumExpectedValue: MIN_EXPECTED_VALUE });
    ...
    playerSelectable.push({
      ...
      dailyEligible: calibratedProbability >= 0.90 && decision.expectedValue >= 0.10,
    });
  }
  ...
  for (const [key, r] of Object.entries(scored)) {
    ...
    }, { minimumProbability: 0.80, minimumExpectedValue: MIN_EXPECTED_VALUE, minimumMarketEdge: 0.02 });
    if (!decision.eligible) continue;
    teamSel.push({
      ...
      dailyEligible: r.prob_final >= 0.90 && decision.expectedValue >= 0.10,
    });
  }
  ...
  for (const [key, r] of Object.entries(scored)) {
    ...
    const decision = recommendationDecision({
      ...
      validation: r.validation,
      marketFairProbability: o.marketFairProbability,
    }, { minimumProbability: SELECTABLE_MIN_PROB, minimumExpectedValue: MIN_EXPECTED_VALUE, minimumMarketEdge: 0.02 });
    if (!decision.eligible) continue;
    teamSelectable.push({
      id: key, category: categoryOf(key), scope: 'context',
      ...
      marketFairProbability: decision.marketFairProbability,
      marketEdge: decision.marketEdge,
      dailyEligible: p >= 0.90 && decision.expectedValue >= 0.10,
    });
  }
