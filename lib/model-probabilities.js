...
export function buildModelCombinada(scored, odds, teamNames, playerMarkets, probabilities, statsData) {
  ...
  for (const sel of playerMarketsToSelections(playerMarkets)) {
    if (sel.probability < PLAYER_SELECTABLE_MIN_PROB) continue;
    ...
    playerSelectable.push({
      ...
      // Aún no existe backtest point-in-time de props individuales. Se muestran
      // en el constructor, pero no pueden convertirse en Apuesta del Día.
      dailyValidated: false,
    });
  }
  // Los props siguen disponibles como estadística para el constructor, pero no
  // entran automáticamente hasta tener validación walk-forward propia.
  const playerRecommended = playerSelectable.filter((sel) => sel.probability >= PLAYER_RECOMMENDED_MIN_PROB);
  ...
  for (const [key, r] of Object.entries(scored)) {
    ...
    teamSel.push({
      ...
      dailyValidated: r.validation?.daily90Validated === true,
    });
  }
  const selected = [...teamSel].filter(m => m.odd > MIN_ODD - 1e-9).sort((a, b) => b.probability - a.probability);
  ...
  const selectable = [...teamSelectable, ...playerSelectable].filter(m => m.odd > MIN_ODD - 1e-9).sort((a, b) => b.probability - a.probability);
  ...
  const _funnel = {
    ...
    playerPendingValidation: playerRecommended.length, selectable: selectable.length,
  };
