...
export function buildModelCombinada(scored, odds, teamNames, playerMarkets, probabilities, statsData) {
  ...
  for (const [key, r] of Object.entries(scored)) {
    ...
    teamSel.push({
      ...
      dailyValidated: r.validation?.elite95Validated === true,
    });
  }
  const selected = [...teamSel, ...playerRecommended].filter(m => m.odd > MIN_ODD - 1e-9).sort((a, b) => b.probability - a.probability);
  ...
  const _funnel = {
    ...
    teamSelectable: teamSelectable.length, playerSelectable: playerSelectable.length, selectable: selectable.length,
  };
