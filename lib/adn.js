...
function buildActuals(fixtureObj, statsResp, eventsResp, halfStats) {
  ...
  const cardHome = teamHasStats(homeId) ? (yh || 0) + (rh || 0) : (cardEv.length ? cardEv.filter(e => e.team?.id === homeId).length : null);
  const cardAway = teamHasStats(awayId) ? (ya || 0) + (ra || 0) : (cardEv.length ? cardEv.filter(e => e.team?.id === awayId).length : null);
  ...
  // Stats de alta frecuencia: si el campo falta, es cobertura limitada y debe
  // quedar null, no cero. Solo los contadores que la API omite habitualmente
  // cuando el evento no ocurrió (tarjetas/offsides) usan cnt.
  const cH = sv(homeId, 'Corner Kicks'), cA = sv(awayId, 'Corner Kicks');
  const shH = sv(homeId, 'Total Shots'), shA = sv(awayId, 'Total Shots');
  const soH = sv(homeId, 'Shots on Goal'), soA = sv(awayId, 'Shots on Goal');
  const flH = sv(homeId, 'Fouls'), flA = sv(awayId, 'Fouls');
...
function recordFromRaw(fixtureObj, statsResp, teamId, opts = {}) {
  ...
  const statsArr = statsResp?.response || statsResp || [];
  const ownStats = (Array.isArray(statsArr) ? statsArr : []).find(s => s.team?.id === me);
  const hasOwnStats = !!(ownStats && Array.isArray(ownStats.statistics) && ownStats.statistics.length);
  const yc = statVal(statsResp, me, 'Yellow Cards');
  const rc = statVal(statsResp, me, 'Red Cards');
  // Una tarjeta omitida dentro de un bloque estadístico real equivale a cero;
  // si el bloque entero no existe, no hay evidencia y la métrica queda null.
  const cards = hasOwnStats ? (yc || 0) + (rc || 0) : null;
