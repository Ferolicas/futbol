...
function buildActuals(fixtureObj, statsResp, eventsResp, halfStats) {
  ...
  // Tarjetas totales por equipo (de stats; fallback a contar eventos).
  const yh = sv(homeId, 'Yellow Cards'), ya = sv(awayId, 'Yellow Cards');
  const rh = sv(homeId, 'Red Cards'),    ra = sv(awayId, 'Red Cards');
  ...
  // Stats agregadas a 90' (full match).
  ...
  const oH = sv(homeId, 'Offsides'), oA = sv(awayId, 'Offsides');
