...
export function buildCombinada(probabilities, odds, playerHighlights, teamNames) {
  ...
  // =========================================================================
  // Step 2: Filter to "probable" recommendations (>= 70%) WITH real odds
  // Requisito #9: no odds = no display. Markets without odds are excluded.
  // =========================================================================
  const probable = deduplicated.filter(m => m.probability >= 70 && m.odd && m.odd > 1);
  ...
  if (probable.length >= 2) {
    ...
    const relaxed = deduplicated
      .filter(m => m.probability >= 65 && m.odd && m.odd > 1)
