...
// calibrated probability >= 90, reliability >= 90, cuota >= 1.20 and EV >=10%.

const PROB_THRESHOLD = 70;   // baseline for any selection
const MIN_ODD        = 1.01; // anything ≤1.01 is meaningless

const clampProb = (p) => Math.max(0, Math.min(100, Number(p) || 0));
const displayProb = (p) => {
  const value = clampProb(p);
  if (value >= 95) return 95;
  return Math.floor((value + 1e-9) * 100) / 100;
};
const rawProb = (market) => Number(market?.rawProbability ?? market?.probability) || 0;
...
const TOTAL_CATEGORIES = {
  // total-goals, total-corners, total-cards ahora usan iterOuLines con ids
  // dinamicos (sufijo -over-K_5 / -under-K_5). Dedup se hace en runtime por
  // m.category (no por lista enumerada), separando over y under.
  'total-goals-over':    [],
  'total-goals-under':   [],
  'total-corners-over':  [],
  'total-corners-under': [],
...
function deduplicateByCategory(markets) {
  ...
  for (const m of markets) {
    ...
    let cat = idToCategory[m.id] || m.category;
    if (!cat) { uncategorized.push(m); continue; }

    // Para mercados con over/under en la misma categoria, dedup separado por
    // lado: "mas de 4 corners" y "menos de 11 corners" son apuestas distintas
    // y ambas pueden tener value. _side = 'over' | 'under' viene de iterOuLines.
    if (m._side) cat = `${cat}-${m._side}`;

    const existing = bestByCategory[cat];
    if (!existing ||
        rawProb(m) > rawProb(existing) ||
        (rawProb(m) === rawProb(existing) && (DIFFICULTY[m.id] || 0) > (DIFFICULTY[existing.id] || 0)) ||
        // Tie-break adicional: para mismo prob, preferir la linea mas extrema
        // (mas dificil sobre la media → mejor cuota).
        (rawProb(m) === rawProb(existing) && m._line != null && existing._line != null &&
         ((m._side === 'over' && m._line > existing._line) ||
          (m._side === 'under' && m._line < existing._line)))) {
...
/**
 * Construye selecciones de "torneo" (outright) para los equipos de ESTE
 * partido, a partir del resultado de /api/tournament-predict (simulateBracket).
 *
 * tournamentContext shape:
 *   { champion: { [teamId]: { team:{id,name,logo}, probability } },
 *     finalist: { ... }, semis: { ... } }
 *
 * Solo emite selección si la probabilidad ≥ PROB_THRESHOLD (el mismo umbral
 * del resto del sistema). La cuota es IMPLÍCITA justa (100/prob) porque no hay
 * cuota de bookmaker para outright en el payload de odds del partido — se marca
 * con `impliedOdds:true` para que el frontend la etiquete como estimada.
 *
 * Cada selección lleva categoría única por equipo+mercado para no colapsar en
 * el dedup (queremos ambos equipos si ambos califican).
 */
function buildTournamentSelections(tournamentContext, homeId, awayId) {
  if (!tournamentContext) return [];
  const out = [];
  const ids = [Number(homeId), Number(awayId)].filter(Boolean);

  const emit = (bucket, label, idPrefix) => {
    const map = tournamentContext[bucket];
    if (!map || typeof map !== 'object') return;
    for (const teamId of ids) {
      const entry = map[teamId] || map[String(teamId)];
      if (!entry) continue;
      const rawProbability = clampProb(entry.probability);
      if (rawProbability < PROB_THRESHOLD) continue;
      const impliedOdd = +(100 / rawProbability).toFixed(2);
      if (!isFinite(impliedOdd) || impliedOdd <= MIN_ODD) continue;
      const teamName = entry.team?.name || `Equipo ${teamId}`;
      out.push({
        id: `${idPrefix}-${teamId}`,
        category: `${idPrefix}-${teamId}`, // única → no colapsa en dedup
        scope: 'tournament',
        name: `${teamName} — ${label}`,
        probability: displayProb(rawProbability),
        rawProbability,
        odd: impliedOdd,
        impliedOdds: true,   // cuota estimada (sin bookmaker)
        isTournament: true,
      });
    }
  };

  // Orden de relevancia: campeón > finalista > semifinalista.
  emit('champion', 'Campeón del torneo', 'tournament-champion');
  emit('finalist', 'Llega a la final', 'tournament-finalist');
  emit('semis',    'Llega a semifinales', 'tournament-semis');

  return out;
}

export function buildCombinada(probabilities, odds, playerHighlights, teamNames, tournamentContext = null) {
  if (!probabilities) {
  ...
  const homeId   = teamNames?.homeId ?? null;
  const awayId   = teamNames?.awayId ?? null;
  const allMarkets = [];
  ...
  const push = (m, oddVal) => {
    ...
    const rawProbability = clampProb(m.rawProbability ?? m.probability);
    allMarkets.push({ ...m, odd, probability: displayProb(rawProbability), rawProbability });
  };

  // ── Helpers para mercados con lineas dinamicas (over/under) ────────────────
  // Declarados ANTES de los pushes de corners/cards/totales/per-team porque
  // todos ellos pasan por iterOuLines. Mover esto despues causaria TDZ
  // (ReferenceError: Cannot access 'iterOuLines' before initialization).
  //
  // lineFromKey: "over11_5" / "under11_5" → 11.5
  const lineFromKey = (key) => {
    const m = (key || '').match(/^(?:over|under)(\d+)(?:_(\d+))?$/i);
    if (!m) return null;
    return parseFloat(`${m[1]}.${m[2] || '0'}`);
  };

  // iterOuLines: para probs = { _lines, over11_5: 78, under11_5: 22, ... }
  // y odds = { Over_11_5: 1.85, Under_11_5: 1.95, ... }
  // emite push() por cada linea over y under con cuota disponible.
  // opts: { scope: 'team'|'total', team: 'home'|'away' } para per-team markets.
  const iterOuLines = (probs, odds, category, namePrefix, opts = {}) => {
    if (!probs || !odds) return;
    const scope = opts.scope || 'total';
    ...
    for (const key of Object.keys(probs)) {
      ...
      const id = `${category}-over-${String(line).replace('.', '-')}`;
      push({ id, category, scope, ...(team ? { team } : {}),
             name: `${namePrefix} — Más de ${line}`,
             probability: probs[key], _line: line, _side: 'over' }, oddVal);
    }
    // UNDER lines
    for (const key of Object.keys(probs)) {
      if (key.startsWith('_')) continue;
      ...
      const line = lineFromKey(key);
      if (line == null) continue;
      const oddKey = `Under_${String(line).replace('.', '_')}`;
      const oddVal = odds[oddKey];
      const id = `${category}-under-${String(line).replace('.', '-')}`;
      push({ id, category, scope, ...(team ? { team } : {}),
             name: `${namePrefix} — Menos de ${line}`,
             probability: probs[key], _line: line, _side: 'under' }, oddVal);
    }
  };
  ...
  // ── Goles totales del partido (data-driven, over y under) ──
  // calculateOverUnder ahora devuelve _lines con todas las lineas adaptativas.
  iterOuLines(probabilities.overUnder, odds?.overUnder, 'total-goals', 'Total partido — Goles');

  // ── Córners totales (rango amplio desde 4.5, over y under) ──
  iterOuLines(probabilities.corners,   odds?.corners,   'total-corners', 'Total partido — Córners');

  // ── Tarjetas totales del partido (rango amplio, over y under) ──
  iterOuLines(probabilities.cards,     odds?.cards,     'total-cards',   'Total partido — Tarjetas');

  // ── Per-team: goles, córners, tarjetas por equipo (lineas amplias + under) ──
  const pt = probabilities.perTeam || {};
  if (pt.home?.goals)   iterOuLines(pt.home.goals,   odds?.homeGoals,   'home-goals',   `Local (${homeName}) — Goles`,   { scope: 'team', team: 'home' });
  if (pt.away?.goals)   iterOuLines(pt.away.goals,   odds?.awayGoals,   'away-goals',   `Visitante (${awayName}) — Goles`,   { scope: 'team', team: 'away' });
  if (pt.home?.corners) iterOuLines(pt.home.corners, odds?.homeCorners, 'home-corners', `Local (${homeName}) — Córners`, { scope: 'team', team: 'home' });
  if (pt.away?.corners) iterOuLines(pt.away.corners, odds?.awayCorners, 'away-corners', `Visitante (${awayName}) — Córners`, { scope: 'team', team: 'away' });
  if (pt.home?.cards)   iterOuLines(pt.home.cards,   odds?.homeCards,   'home-cards',   `Local (${homeName}) — Tarjetas`, { scope: 'team', team: 'home' });
  if (pt.away?.cards)   iterOuLines(pt.away.cards,   odds?.awayCards,   'away-cards',   `Visitante (${awayName}) — Tarjetas`, { scope: 'team', team: 'away' });
  ...
  (ph.shotsTotalists || []).forEach(p => {
    // shotsTotal = total shots (incluye a puerta + fuera + bloqueados).
    // Diferenciar del label de shotsOnGoal ("remate a puerta") para que
    // el usuario sepa que mercado del bookmaker se esta recomendando.
    ...
    if (!best) {
      ...
      push({ id: `shotsTotal-${p.id}`, category: `shotsTotal-${p.id}`, scope: 'player',
             playerId: p.id, playerName: p.name, team: p.teamName,
             name: `${p.name} — Cualquier tiro (total)`, probability: probAnytime }, null);
      return;
    }
    const threshold = Math.ceil(best.line);
    const label = threshold === 1
      ? `${p.name} — Cualquier tiro (total)`
      : `${p.name} — ${threshold}+ tiros totales`;
    push({ id: `shotsTotal-${p.id}`, category: `shotsTotal-${p.id}`, scope: 'player',
           playerId: p.id, playerName: p.name, team: p.teamName,
           name: label, probability: best.freq, _line: best.line }, best.odd);
  });
  ...
  // (lineFromKey + iterOuLines fueron movidos al principio de buildCombinada
  //  para que esten disponibles para corners/cards/totales que ya los usan.)
  // ──────────────────────────────────────────────────────────────────────────
  ...
  // (Hándicap asiático eliminado del catálogo — no se genera ninguna selección.)
  ...
  // ── Stage 4.3: Mercados nuevos — roja, offside ──
  // Tarjeta roja: 'anyRed' = P(al menos una roja en el partido)
  const rc = probabilities.redCards;
  if (rc?.anyRed != null) {
    push({ id: 'total-red-card', category: 'total-red-card', scope: 'total',
           name: 'Total partido — Habrá tarjeta roja', probability: rc.anyRed },
         odds?.redCard?.yes);
  }
  // Per-team rojas
  const prc = probabilities.perTeamRedCards;
  if (prc?.home?.anyRed != null) {
    push({ id: 'home-red-card', category: 'home-red-card', scope: 'team', team: 'home',
           name: `Local (${homeName}) — Tarjeta roja`, probability: prc.home.anyRed },
         odds?.homeRedCard?.yes);
  }
  if (prc?.away?.anyRed != null) {
    push({ id: 'away-red-card', category: 'away-red-card', scope: 'team', team: 'away',
           name: `Visitante (${awayName}) — Tarjeta roja`, probability: prc.away.anyRed },
         odds?.awayRedCard?.yes);
  }

  // Offsides totales (dinamico — varias lineas)
  iterOuLines(probabilities.offsides,           odds?.offsides,    'total-offsides',
              'Total partido — Offsides');
  if (probabilities.perTeamOffsides?.home) {
    iterOuLines(probabilities.perTeamOffsides.home, odds?.homeOffsides, 'home-offsides',
                `Local (${homeName}) — Offsides`, { scope: 'team', team: 'home' });
  }
  if (probabilities.perTeamOffsides?.away) {
    iterOuLines(probabilities.perTeamOffsides.away, odds?.awayOffsides, 'away-offsides',
                `Visitante (${awayName}) — Offsides`, { scope: 'team', team: 'away' });
  }

  // ── Selecciones de torneo (outright) — P3 ──
  // Se agregan al pool ANTES del dedup. Cada una tiene categoría única
  // (tournament-*-{teamId}) así que el dedup no las colapsa entre sí ni con
  // los mercados del partido. Pasan el mismo filtro ≥PROB_THRESHOLD + odd.
  const tournamentSelections = buildTournamentSelections(tournamentContext, homeId, awayId);
  for (const ts of tournamentSelections) allMarkets.push(ts);

  // ── Dedup, sort, filter ──
  const deduplicated = deduplicateByCategory(allMarkets);
  const selected = deduplicated
    .filter(m => rawProb(m) >= PROB_THRESHOLD && m.odd > MIN_ODD)
    .sort((a, b) => rawProb(b) - rawProb(a));
  ...
  // CB1 FIX: producto (P de que TODAS acierten), no promedio. Coherente con combinedOdd.
  const combinedProbability = selected.reduce((acc, m) => acc * (rawProb(m) / 100), 1) * 100;
  const effectiveThreshold  = Math.min(...selected.map(rawProb));
