...
export function buildCombinada(probabilities, odds, playerHighlights) {
  ...
  if (playerHighlights?.scorers?.length > 0) {
    ...
    const scorerProb = Math.min(95, Math.round((matchesWithGoal / totalMatches) * 100));
    if (scorerProb >= 60) {
      allMarkets.push({
  ...
  // Hard cap: no probability above 95%
  selected.forEach(m => { m.probability = Math.min(95, m.probability); });
  ...
  const combinedProbability = selected.length > 0
    ? selected.reduce((acc, m) => acc * (m.probability / 100), 1) * 100
    : 0;
