...
const clampProb = (p) => {
  const value = Math.max(0, Number(p) || 0);
...
function deduplicateByCategory(markets) {
  ...
  for (const m of markets) {
    ...
    if (!existing ||
        ...
        (m.probability === existing.probability && m._line != null && existing._line != null &&
...
function buildTournamentSelections(tournamentContext, homeId, awayId) {
  ...
  const emit = (bucket, label, idPrefix) => {
    ...
    for (const teamId of ids) {
      ...
      if (!entry) continue;
      const prob = clampProb(entry.probability);
      if (prob < PROB_THRESHOLD) continue;
      const impliedOdd = +(100 / prob).toFixed(2);
