...
export function buildCombinada(probabilities, odds, playerHighlights, teamNames) {
  ...
  // ── Asistencias por jugador (nuevo, cache_version 8) ──
  ...
  // ── Tiros por jugador totales (mercado distinto a "tiros a puerta") ──
  // Player Shots Total: Bet365 lo ofrece como "X.5 tiros". Es Anytime style →
  // probability = frecuencia historica de >=2 tiros (umbral ≥ 1.5).
  (ph.shotsTotalists || []).forEach(p => {
    const prob = playerFreq(p.shotsTotal);
    const odd  = lookupPlayerOdd('shots', p.name);
    push({ id: `playershots-${p.id}`, category: `playershots-${p.id}`, scope: 'player',
           playerId: p.id, playerName: p.name, team: p.teamName,
           name: `${p.name} — Hacer un tiro`, probability: prob }, odd);
  });
