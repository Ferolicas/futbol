...
const PLAYER_AGG = `count(*), avg(minutes), avg(goals), avg(assists), avg(shots_total), avg(shots_on), avg(fouls_committed), avg(yellow), avg(rating), avg((is_starter)::int), (sum(goals)::real/NULLIF(sum(minutes) FILTER (WHERE goals IS NOT NULL),0)*90), avg((shots_on>0)::int), avg((goals>0)::int), avg(((coalesce(yellow,0)+coalesce(red,0))>0)::int) FILTER (WHERE yellow IS NOT NULL OR red IS NOT NULL), (sum(shots_total)::real/NULLIF(sum(minutes) FILTER (WHERE shots_total IS NOT NULL),0)*90), (sum(shots_on)::real/NULLIF(sum(minutes) FILTER (WHERE shots_on IS NOT NULL),0)*90), (sum(fouls_committed)::real/NULLIF(sum(minutes) FILTER (WHERE fouls_committed IS NOT NULL),0)*90), (sum(coalesce(yellow,0)+coalesce(red,0))::real/NULLIF(sum(minutes) FILTER (WHERE yellow IS NOT NULL OR red IS NOT NULL),0)*90), max(kickoff)::date`;

const isPool = (db) => typeof db?.connect === 'function' && typeof db?.totalCount === 'number';
async function atomic(pool, fn) {
...
async function buildTeamProfiles(pool, { teamIds = null, minN = 1, _atomic = false } = {}) {
  if (!_atomic && isPool(pool)) {
    return atomic(pool, (client) => buildTeamProfiles(client, { teamIds, minN, _atomic: true }));
  }
...
async function buildPlayerProfiles(pool, { playerIds = null, minN = 1, _atomic = false } = {}) {
  if (!_atomic && isPool(pool)) {
    return atomic(pool, (client) => buildPlayerProfiles(client, { playerIds, minN, _atomic: true }));
  }
