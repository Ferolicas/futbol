  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      const response = await fetch(`${API}${path}`, {
        // ESPN devuelve 403 a varios User-Agent explícitos (incluido el que
        // usábamos). El fetch nativo sin sobrescribirlo recibe 200.
...
export async function getEspnGamesByDate(competitionKey, date, options = {}) {
  const config = COMPETITIONS[competitionKey];
  if (!config) throw new Error(`Competición ESPN inválida: ${competitionKey}`);
  // ESPN responde 500 en algunas jornadas FCS cuando se pide limit=1000
  // (p. ej. 2025-12-13), aunque el mismo recurso es válido con 500. Ninguna
  // de estas competiciones se acerca a 500 partidos diarios.
  const params = new URLSearchParams({ dates: date.replaceAll('-', ''), limit: '500' });
