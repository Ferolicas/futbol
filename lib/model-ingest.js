...
async function ingestFixtures(pool, fixtureIds, { batch = 400 } = {}) {
  ...
  for (let i = 0; i < ids.length; i += batch) {
    ...
    try {
      for (const r of rows) {
        ...
        catch (e) { await client.query('ROLLBACK'); console.error(`[model-ingest] fixture ${fid} FALLÓ: ${e.message}`); }
  ...
  return { done, withStats, withPlayers };
}
  ...
  for (let i = 0; i < objs.length; i += batch) {
    ...
    try {
      for (const fx of slice) {
        ...
        await client.query('BEGIN');
        try { await processFixture(client, fx, relFor(rel, fid)); await client.query('COMMIT'); }
        catch (e) { await client.query('ROLLBACK'); console.error(`[model-ingest] obj fixture ${fid} FALLÓ: ${e.message}`); }
        done++;
      }
    } finally { client.release(); }
  }
...
module.exports = { ingestFixtures, ingestFixtureObjects, processFixture };
