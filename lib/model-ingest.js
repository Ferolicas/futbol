...
//   ingestFixtureObjects(pool, fxObjects) → ingiere filas matches+dims desde objetos fixture dados
//                                           (upcoming: aún no en el crudo; sin stats/players).
  ...
  for (let i = 0; i < objs.length; i += batch) {
    ...
    try {
      for (const fx of slice) {
        ...
        await client.query('BEGIN');
        try { await processFixture(client, fx, relFor(rel, fid)); await client.query('COMMIT'); done++; }
...
module.exports = {
  ...
  buildPlayerCoverage, coveredPlayerCounter, nonNegativeInt: int,
};
