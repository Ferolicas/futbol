...
export async function GET(request) {
  try {
    ...
    const [analysesRes, resultsRes, hiddenRes, favoritesRes] = await Promise.all([
      ...
      allFids.length ? supabaseAdmin.from('baseball_match_results').select('fixture_id, status, inning, inning_half, home_score, away_score, home_hits, away_hits, home_errors, away_errors, innings, home_stats, away_stats, outs, balls, strikes, bases, finished_at').in('fixture_id', allFids) : Promise.resolve({ data: [] }),
