            ...
            {a.lineups.data.map((team, idx) => (
              <Panel key={idx}>
                ...
                {(team.startXI || []).map((pl, i) => (
                  <div key={i} className="fak-lineup-row"><span className="is-num">{pl.player?.number}</span><PlayerFace id={pl.player?.id} size={24} ring={idx === 0 ? 'rgba(0,212,255,.45)' : 'rgba(236,72,153,.45)'} /><span className="is-name">{pl.player?.name}</span><span className="is-pos">{pl.player?.pos}</span></div>
                ))}
                {(team.substitutes || []).length > 0 && <span className="fak-kicker" style={{ marginTop: 6 }}>Suplentes</span>}
                {(team.substitutes || []).map((pl, i) => (
                  <div key={i} className="fak-lineup-row" style={{ opacity: .7 }}><span className="is-num">{pl.player?.number}</span><PlayerFace id={pl.player?.id} size={24} ring={idx === 0 ? 'rgba(0,212,255,.45)' : 'rgba(236,72,153,.45)'} /><span className="is-name">{pl.player?.name}</span><span className="is-pos">{pl.player?.pos}</span></div>
                ))}
              </Panel>
            ))}
...
// Foto del jugador (/api/player-photo) con círculo de respaldo si no hay imagen.
