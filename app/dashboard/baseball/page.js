...
function GameCard({ game, userTz, isFavorite, isAnalyzed, isExpanded,
  ...
  const current = { ...game, status: { ...game.status, short: effectiveGameStatus(game) },
    scores: { home: { total: finished ? game.scores?.home?.total ?? liveResult?.home_score : liveResult?.home_score ?? game.scores?.home?.total },
      away: { total: finished ? game.scores?.away?.total ?? liveResult?.away_score : liveResult?.away_score ?? game.scores?.away?.total } } };
  return <SharedSportCard game={current} sport="baseball" scoreLabel="carreras" timeZone={userTz}
    expanded={isExpanded} onToggle={onExpand} selected={selectedMarkets}
    onTogglePick={pick => onToggleMarket(pick.id, { ...pick, key: pick.id, cat: pick.marketLabel || pick.category, label: pick.name, probability: cap(pick.probability), rawProbability: Number(pick.rawProbability ?? pick.probability), odd: Number(pick.odd) })}
    favorite={isFavorite} onFavorite={id => onFavorite({ stopPropagation() {} }, id)}
    onDismiss={onDismiss} onViewFull={onViewFull} />;
}
